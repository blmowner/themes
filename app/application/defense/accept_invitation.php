...
function runnum5($column_name, $tblname) 
{ 
    global $db_klas2;
...
{	
	...
	if (sizeof($_POST['myCheckbox'])>0) {
		...
		{	
			...
			$lock_tables="LOCK TABLES pg_invitation_detail WRITE";
			...
			if ($refSessionTypeId[$val] == "DEF") {
				...
				if ($row_cnt9 > 0) {				
					...
					$lock_tables="LOCK TABLES pg_evaluation_detail WRITE";
					$db->query($lock_tables);
				...
				if ($row_cnt9 > 0) {				
					...
					$lock_tables="LOCK TABLES pg_work_evaluation_detail WRITE";
					$db->query($lock_tables);
			...
			else if ($refSessionTypeId[$val] == "VIV") {
				
				$sql1 = "SELECT id from pg_viva 
				where student_matrix_no = '$studentMatrixNo[$val]'
				and pg_thesis_id = '$theThesisId[$val]'
				and status in ('A', 'ARC', 'ARC1')
				and pg_calendar_id = '$theCalendarId[$val]'
				order by id desc";
				...
				if(!empty($id))
				{
					$lock_tables="LOCK TABLES pg_evaluation_viva_detail WRITE";
					$db->query($lock_tables);
			...
			if ($constvalue == 'Y' && $fromadmin !="" && $superemail != "" && $facultyemail != "")
			{
...
?>
		<form id="form2" name="form2" method="post" enctype="multipart/form-data">
			...
			<legend><strong>List of Thesis</strong></legend>
				...
				<table>
					<tr>
						<td>Note: Please provide <strong>Acceptance Remarks</strong> above if you are not able to accept the invitation.</td>
					</tr>
				</table>
				<br/>
				<table>					
