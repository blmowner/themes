...
// Program Name: defense_proposal_confirm.php
//
// Created by: Zuraimi
// Created Date: 06-August-2015
// Modified by: Zuraimi
...
if(isset($_POST['btnSearch']) && ($_POST['btnSearch'] <> "")) {
	...
	c.description as status_desc, a.pg_defense_id, a.ref_defense_marks_id, b.description, a.respond_by
	FROM pg_evaluation a
	LEFT JOIN ref_defense_marks b ON (b.id = a.ref_defense_marks_id)
	...
	ORDER BY a.confirmed_date ,a.student_matrix_no, a.reference_no DESC";
	...
	if (mysql_num_rows($result1) > 0){
		do {
			$evaluationIdArray[$no1] = $dba->f('id');
			...
			$defenseIdArray[$no1] = $dba->f('pg_defense_id');
			$defenseMarksIdArray[$no1] = $dba->f('ref_defense_marks_id');
			$defenseMarksDescArray[$no1] = $dba->f('description');
		...
		for ($i=0; $i<$no1; $i++){
			...
			if (mysql_num_rows($result9)>0) {
				...
				$evaluationIdArray[$no2] = $evaluationIdArray[$i];
				...
				$defenseMarksIdArray[$no2] = $defenseMarksIdArray[$i];
				$defenseMarksDescArray[$no2] = $defenseMarksDescArray[$i];
...
{
	$sql1 = "SELECT a.id, a.student_matrix_no, a.pg_thesis_id, a.pg_proposal_id, a.reference_no, a.status,
	c.description as status_desc, a.pg_defense_id, a.ref_defense_marks_id, b.description, a.respond_by, d.pg_calendar_id,
	DATE_FORMAT(a.confirmed_date,'%d-%b-%Y %h:%i %p') AS confirmed_date
	FROM pg_evaluation a
	LEFT JOIN ref_defense_marks b ON (b.id = a.ref_defense_marks_id)
	LEFT JOIN ref_proposal_status c ON (c.id = a.status)
	LEFT JOIN pg_defense d ON (d.id = a.pg_defense_id)
	WHERE a.status IN ('IN1','APP','DIS')
	AND a.respond_status = 'Y'
	AND a.archived_status IS NULL
	AND b.status = 'A'
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	function respConfirm () {
		...
		<legend><strong>List of Student</strong></legend>
			...
			<table border="1" style="border-collapse:collapse;" cellpadding="2" cellspacing="1" width="90%" class="thetable">			
				<tr>
					...
					<th width="20%" align="left">Evaluation Report<br>Reference No.</th>						
					<th width="15%" align="left">Status by Evaluation Committee</th>
					<th width="15%" align="left">Status by Schoolboard</th>
				...
				for ($i=0; $i<$no2; $i++){?>
					<tr>
						...
						LEFT JOIN pg_evaluation c ON (c.pg_defense_id = b.id) 
						LEFT JOIN pg_evaluation_detail d ON (d.pg_eval_id = c.id)
						LEFT JOIN ref_session_type e ON (e.id = a.ref_session_type_id)
						WHERE a.student_matrix_no = '$studentMatrixNoArray[$i]'
						AND c.pg_defense_id = '$defenseIdArray[$i]'
						...
						$venue = $dba->f('venue');		
						?>
						<td><label><?=$sessionTypeDesc?>: <?=$defenseDate?>, <?=$defenseSTime?>-<?=$defenseETime?><br/><?=$referenceNoArray[$i]?></label></td>
						<td><label><?=$defenseMarksDescArray[$i]?><br><?=$respondedDate?></label></td>
						<td><label><?=$statusDescArray[$i]?><br><?=$confirmedDateArray[$i]?></label></td>
						<td align="center">			
						<?if ($evaluationStatusArray[$i] == 'IN1') {?>										
							<a href="../defense/confirm_evaluation_detail.php?id=<?=$evaluationIdArray[$i]?>&mn=<?=$studentMatrixNoArray[$i]?>&tid=<?=$thesisIdArray[$i]?>&pid=<?=$proposalIdArray[$i]?>&ref=<?=$referenceNoArray[$i]?>&refd=<?=$referenceNoDefence?>&eid=<?=$respondByArray[$i]?>&cid=<?=$calendarIdArray[$i]?>&did=<?=$defenseIdArray[$i]?>">Update</a></td>
							<?
						}
						else {
							?>
								<a href="../defense/confirm_evaluation_detail_view.php?id=<?=$evaluationIdArray[$i]?>&mn=<?=$studentMatrixNoArray[$i]?>&tid=<?=$thesisIdArray[$i]?>&pid=<?=$proposalIdArray[$i]?>&ref=<?=$referenceNoArray[$i]?>&refd=<?=$referenceNoDefence?>&eid=<?=$respondByArray[$i]?>&cid=<?=$calendarIdArray[$i]?>&did=<?=$defenseIdArray[$i]?>">View</a></td>
							<?
						}?>
						</td>
					</tr>
		...
		<table>				
			<tr><td><br/>Notes:<br/>
				1. If you don't find the student and the session, it could be the Evaluation Committee is yet to complete the feedback.</a></td>
			</tr>
		</table>
	...
	?>
		...
		<br/>
		<table>				
			<tr><td>Possible Reason:-<br/>
				1. The Evaluation Panel is yet to complete their feedback.</td>
			</tr>
		</table>
		<?
	}?>	
