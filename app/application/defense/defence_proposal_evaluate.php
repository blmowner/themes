...
// Program Name: defense_proposal_evaluate.php
//
// Created by: Zuraimi
...
if(isset($_POST['btnSearch']) && ($_POST['btnSearch'] <> "")) {
	...
	$sql1 = "SELECT a.pg_defense_id, a.student_matrix_no, a.pg_thesis_id, a.pg_proposal_id, a.reference_no,
	c.pg_calendar_id
	FROM pg_evaluation a
	LEFT JOIN pg_evaluation_detail b ON (b.pg_eval_id = a.id)
	LEFT JOIN pg_defense c ON (c.id = a.pg_defense_id)
		...
		<legend><strong>List of Student</strong></legend>
			...
			<table border="1" style="border-collapse:collapse;" cellpadding="2" cellspacing="1" width="100%" class="thetable">			
				<tr>
					...
					<th width="20%" align="left">Evaluation Report<br>Reference No<br>Final Evaluation Status</th>						
					<th width="15%"  align="left">Evaluation Status by Member</th>					
				...
				for ($i=0; $i<$no2; $i++){?>
					<tr>
						...
						g.description as defense_marks_desc_detail, e.respond_status,
						e.ref_defense_marks_id, i.description as defense_marks_desc 
						FROM pg_defense a
						LEFT JOIN pg_defense_detail b ON (b.pg_defense_id = a.id)
						LEFT JOIN pg_calendar d ON (d.id = a.pg_calendar_id)
						LEFT JOIN pg_evaluation e ON (e.pg_defense_id = a.id)
						LEFT JOIN pg_evaluation_detail f ON (f.pg_eval_id = e.id)
						...
						LEFT JOIN ref_defense_marks i ON (i.id = e.ref_defense_marks_id)
						...
						$defenseMarksDescDetail=$dbg->f('defense_marks_desc_detail');
						$confirmedDate=$dbg->f('confirmed_date');
						$row_cnt2 = mysql_num_rows($result2);						
						?>						
						<td><label><?=$sessionTypeDesc?> - <?=$defenseDate?>, <?=$defenseSTime?> - <?=$defenseETime?></strong><br/><?=$referenceNoArray[$i]?><br><?=$defenseMarksDesc?></label></td>
						<td><label><?=$defenseMarksDescDetail?><br><?=$evaluationDetailDesc?><br><?=$respondedDate?></label></td>
						<td><label><?=$evaluationDesc?><br><?=$confirmedDate?></label></td>
						<td align="center">			
						<? if ($evaluationStatus == 'IN1' && $respondStatus!='Y') {
							if ($evaluationDetailStatus =='IN1' || $evaluationDetailStatus =='SV1') {
								if ($supervisorTypeId != 'XE') {?>										
									<a href="../defense/review_evaluation_detail.php?id=<?=$id?>&mn=<?=$studentMatrixNoArray[$i]?>&tid=<?=$thesisIdArray[$i]?>&pid=<?=$proposalIdArray[$i]?>&ref=<?=$referenceNoArray[$i]?>&refd=<?=$referenceNoDefence?>&rol=<?=$supervisorTypeId?>&cid=<?=$calendarIdArray[$i]?>&did=<?=$defenseIdArray[$i]?>">Update</a></td>
								<? }
								else {?>
								...
								<a href="../defense/review_evaluation_detail_view.php?id=<?=$id?>&mn=<?=$studentMatrixNoArray[$i]?>&tid=<?=$thesisIdArray[$i]?>&pid=<?=$proposalIdArray[$i]?>&ref=<?=$referenceNoArray[$i]?>&refd=<?=$referenceNoDefence?>&rol=<?=$supervisorTypeId?>&cid=<?=$calendarIdArray[$i]?>&did=<?=$defenseIdArray[$i]?>">View</a></td>
							<?}
						}
						else {
							?>
	...
	?>
		...
		<table>				
			<tr><td><br/>Notes:<br/>
				...
				2. If you have accepted the invitation, the defence proposal is pending submission by the Student.<br/>
				3. If you have rejected the invitation, the defence proposal would not be displayed in here.</td>
			</tr>
		</table>
		<?
	}?>	
