...
if(isset($_POST['btnSave']) && ($_POST['btnSave'] <> ""))
{
	...
	if(empty($msg))  
	{
		if ($defenseStatus=="") {
			...
			pg_achievement WRITE, pg_defense_publication WRITE, pg_defense_conference WRITE"; //lock the table
			...
			{	
				...
				(id, defense_id, reference_no, pg_thesis_id, pg_proposal_id, student_matrix_no, pg_chapter_id, pg_subchapter_id,  
			...
			$msg[] = "<div class=\"success\"><span>Your Progression Monitoring Report (Defence Proposal) has been saved successfully.</span></div>";
		}
...
if(isset($_POST['btnSubmit']) && ($_POST['btnSubmit'] <> ""))
{
	...
	if(empty($msg)) 
	{
		if ($defenseStatus=="") {
			...
			$msg[] = "<div class=\"success\"><span>Your Progression Monitoring Report (Defence Proposal) has been submitted to your Supervisor/Co-Supervisor successfully.</span></div>";		
		}
		else if ($defenseStatus=="SAV") {//SAV
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	function newPublication(pid, tid, pgid, ref) 
	{
		...
		if (ask) 
		{
			document.location.href = "../defense/edit_defense_publication.php?pid=" + pid + "&tid=" + tid + "&pgid=" + pgid + "&ref=" + ref;
			return true;
		}
		return false;
	}
	...
	function newConference(pid, tid, pgid, ref) 
	{
		...
		if (ask) 
		{
			document.location.href = "../defense/edit_defense_conference.php?pid=" + pid + "&tid=" + tid + "&pgid=" + pgid + "&ref=" + ref;
			return true;
		}
		return false;
	}
	...
	if ($row_cnt4 > 0) {
		if ($defenseStatus == "" || $defenseStatus == "SAV") {?>
			...
			<table width="52%">
				...
				<tr>
					...
					AND a.recomm_status IN ('REC', 'NRE')
					AND a.status = 'A'
					AND a.id NOT IN (
					...
					if ($row_cnt_supervisor>0) {
						do {
							...
							<tr>
								...
								if ($row_cnt12>0) {
								?>
								<td align="center"><a href="edit_defense_view_feedback.php?tid=<?=$thesisId;?>&pid=<?=$proposalId;?>&eid=<?=$employeeId;?>&id=<?=$id;?>&ref=<?=$referenceNo;?>&cid=<?=$calendarId?>">View</a></td>	
								<?}
								else {
									?>
									<td></td>
								...
								?>
									<td align="left"><label>Expecting Defence Proposal</label></td>
								<?}
		...
		$msg = "<div class=\"error\"><span>Defence Proposal submission would require proposal defence date. Please check your preference date with your main Supervisor.</span></div>";
		echo $msg;
		?>
