...
if(isset($_POST['btnSave']) && ($_POST['btnSave'] <> ""))
{
	...
	if(empty($msg)) {
		if ($defenseStatus=="") {
			...
			$lock_tables="LOCK TABLES pg_defense WRITE"; //lock the table
			$db->query($lock_tables);
			
			$sql9 = "UPDATE pg_defense
			...
			while (list ($key,$val) = @each ($supervisorIdArray)) 
			{
				...
				$lock_tables="LOCK TABLES pg_defense_detail WRITE"; //lock the table
				$db->query($lock_tables);
			...
			{	
				...
				$lock_tables="LOCK TABLES pg_achievement WRITE"; //lock the table
				$db->query($lock_tables);
			...
			$lock_tables="LOCK TABLES pg_defense_publication WRITE, pg_defense_conference WRITE"; //lock the table
			$db->query($lock_tables);
	...
	if(empty($msg)) {
		if ($defenseStatus=="") {
			...
			while (list ($key,$val) = @each ($supervisorIdArray)) 
			{
				...
				{
					include("../../../app/application/inbox/defense/submit_defense.php");
				}
				...
				{
					include("../../../app/application/email/defense/email_submit_defense.php");
				}			
				////////////////////end notification//////////////////////////////////
			}
			...
			$msg[] = "<div class=\"success\"><span>Your Progression Monitoring Report (Defense Proposal) has been submitted to your Supervisor/Co-Supervisor successfully.</span></div>";		
		}
		else if ($defenseStatus=="SAV") {//SAV
...
if ($check == true) {
	$sql10 = "SELECT a.id, a.reference_no, a.status as defense_status
	FROM pg_defense a
	WHERE a.student_matrix_no = '$user_id'
	AND a.pg_thesis_id = '$thesisId'
	AND a.pg_proposal_id = '$proposalId'
	...
	$defenseStatus1 = $dbg->f('defense_status');
	...
	if ($row_cnt10 > 0) { //Check defence proposal status
		if ($defenseStatus1 == "REC") {
			$sql8 = "SELECT a.status as defense_status, b.ref_defense_marks_id, b.proposed_marks_id, 
				...
				if ($respondStatus == "Y") {
					if ($confirmedStatus == "Y") {
						if ($evaluationStatus == "APP") {
							...
							if ($defenseMarksId == "SAT") {
								if ($proposedMarksId == "NSA") {
									$resubmitReport = true;
								}
								else if ($proposedMarksId == "FAI") {
									$resubmitReport = true;					
								}
							}
							else if ($defenseMarksId == "NSA") {
		...
		else if ($defenseStatus1 == "IN1") {
			$errorNo = 6;
			$resubmitReport = false;
			$msg[] = "<div class=\"error\"><span>Submission of new Defence Proposal Report is currently not allowed.</span></div>";
		}
		else if ($defenseStatus1 == "NRE") {
			$resubmitReport = true;
		}
	}
	else {
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	if ($resubmitReport == true) {
		if ($row_cnt4 > 0) {
			if ($defenseStatus == "" || $defenseStatus == "SAV") {?>
				...
				<table>
					<tr>
						<td><label>Notes:<br/>
						1. The recommended date to have Defence Proposal is after <?=$defenseDurationParam?> days starting from submission date.</label>
						</td>
					</tr>
				</table>			
				<?
		...
		<table>
			<tr>		
				<td><label>Possible Reason:-<br>
				1. Your Defence Proposal report has already been approved by Evaluation Committee and Academic Committee, please proceed with the <a href="../work/work_completion.php">Work Completion</a> report submission<br>
				</label></td>		
			</tr>
		</table>
		<?
		...
		<table>
			<tr>		
				<td><label>Possible Reason:-<br>
				1. Your Defence Proposal report has already been approved by Academic Committee, please proceed with the <a href="../work/work_completion.php">Work Completion</a> report submission<br>
				</label></td>		
			</tr>
		</table>
		<?
		...
		<table>
			<tr>		
				<td><label>Possible Reason:-<br>
				1. Your Defence Proposal report is currently pending approval by Academic Committee, please check again later.<br>
				</label></td>		
			</tr>
		</table>
		<?
		...
		<table>
			<tr>		
				<td><label>Possible Reason:-<br>
				1. Your Defence Proposal report is currently pending approval by Evaluation Committee, please check again later.<br>
				</label></td>		
			</tr>
		</table>
		<?
		...
		<table>
			<tr>		
				<td><label>Possible Reason:-<br>
				1. Your Defence Proposal report is currently in review by Evaluation Committee. Please check again later.</label></td>		
			</tr>
		</table>
		<?
		}
		...
		<table>
			<tr>		
				<td><label>Possible Reason:-<br>
				1. Your Defence Proposal report is currently in draft mode <a href="../defense/defense_proposal.php">[Edit Mode]</a>, please complete and submit it first</label></td>		
			</tr>
		</table>
		<?
		}
