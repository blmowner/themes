	...
	if(empty($msg)) 
	{
		...
		if ($roleStatus =="PRI") {
			...
			if ($recommendationStatus=="REC") {
				$curdatetime = date("Y-m-d H:i:s");	
				$newEvaluationId = runnum('id','pg_evaluation');
				$newReferenceNo = "E".runnum2('reference_no','pg_evaluation');
				
				$lock_tables="LOCK TABLES pg_evaluation WRITE"; //lock the table
				$db->query($lock_tables);
			
				$sql11 = "INSERT INTO pg_evaluation
				(id, pg_defense_id, reference_no, student_matrix_no, pg_thesis_id, pg_proposal_id, respond_status, confirmed_status,
				ref_defense_marks_id, status, remark_evaluation, insert_by, insert_date, modify_by, modify_date)
				VALUES ('$newEvaluationId', '$defenseId','$newReferenceNo','$studentMatrixNo','$thesisId','$proposalId', 'N', 'N',
				...
				if ($arr_length > 0) {
					for ($j=0;$j<$arr_length;$j++) {
						...
						$lock_tables="LOCK TABLES pg_evaluation_detail WRITE"; //lock the table
						$db->query($lock_tables);
						...
						$dbg;
						$resultSql3 = $dbg->query($sql3); 
						$dbg->next_record();
						
						$recommendedId = $dbg->f('id');
						$defenseDate = $dbg->f('viva_date');
						$defenseSTime = $dbg->f('viva_stime');
						$defenseETime = $dbg->f('viva_etime');	
						$venue = $dbg->f('venue');		
						$calendarStatus = $dbg->f('status');
						$recommStatus = $dbg->f('recomm_status');
						...
						{
							include("../../../app/application/inbox/defense/submit_defense_exa.php");
						}
						...
						{
							include("../../../app/application/email/defense/email_submit_defense_exa.php");
						}			
						////////////////////end notification//////////////////////////////////
			...
			$msg[] = "<div class=\"success\"><span>The confirmation on the defence proposal has been submitted successfully to Evaluation Panel for approval.</span></div>";	
		}
		else {
			$msg[] = "<div class=\"success\"><span>The confirmation on the defence proposal has been submitted successfully to Main Supervisor for review.</span></div>";	
		}
	...
	<?if ($row_cnt2>0) {
		if ($defenseDetailStatus1=="IN1") {?>
			...
			<table>
				...
				<input type="hidden" name="calendarId" id="calendarId" value="<?=$calendarId?>" /></td>					
			</tr>   				
			</table>				
			</br>
			<?
				...
				if ($row_cnt_supervisor>0) {
					...
					do {
						...
						<tr>
							...
							?>
							<td align="left"><a href="defense_view_feedback_staff.php?eid=<?=$employeeId?>&mn=<?=$studentMatrixNo?>&pid=<?=$proposalId;?>&tid=<?=$thesisId;?>&id=<?=$id;?>&ref=<?=$referenceNo;?>&cid=<?=$calendarId?>"><?=$defenseDetailDesc;?></a></td>								
