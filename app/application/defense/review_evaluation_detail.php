...
// Program Name: review_evaluation_detail.php
//
// Created by: Zuraimi
...
if(isset($_POST['btnSave']) && ($_POST['btnSave'] <> ""))
{
	...
	if(empty($msg)) 
	{
		if ($evaluationDetailStatus=="IN1") {
			...
			$lock_tables="LOCK TABLES pg_evaluation_detail WRITE, pg_evaluation_style WRITE,
			pg_evaluation_overall WRITE"; //lock the table
			...
			ref_defense_marks_id = '$defenseMarksCb', modify_by = '$user_id', modify_date = '$curdatetime'
...
if(isset($_POST['btnSubmit']) && ($_POST['btnSubmit'] <> ""))
{
	...
	if(empty($msg)) 
	{
		if ($evaluationDetailStatus=="IN1") {
			...
			$lock_tables="LOCK TABLES pg_evaluation_detail WRITE, pg_evaluation_style WRITE,
			pg_evaluation_overall WRITE, pg_evaluation WRITE"; //lock the table
			$db->query($lock_tables);
			...
			if ($supervisorTypeId =="EC") {
				...
				SET ref_defense_marks_id = '$defenseMarksCb', respond_by = '$user_id', 
				...
				$msg[] = "<div class=\"success\"><span>Defence Proposal Evaluation Report has been submitted successfully to Schoolboard for approval.</span></div>";
			}
			else {
				$msg[] = "<div class=\"success\"><span>Defence Proposal Evaluation Report has been submitted successfully to Chairman for review.</span></div>";
			}	
			$unlock_tables="UNLOCK TABLES"; //unlock the table;
			$db->query($unlock_tables);	
		}
			...
			$sql13 = "UPDATE pg_evaluation_detail
			SET major_revision = '$majorRevision', other_comment = '$otherComments', responded_date = '$curdatetime',
			ref_defense_marks_id = '$defenseMarksCb', status = 'IN2', modify_by = '$user_id', modify_date = '$curdatetime'
			WHERE id = '$evaluationDetailId'
			AND pg_eval_id = '$evaluationId'
			AND pg_employee_empid = '$user_id'
			AND status = 'SV1'
			...
			if ($supervisorTypeId =="EC") {
				...
				$msg[] = "<div class=\"success\"><span>Defence Proposal Evaluation Report has been submitted successfully to Evaluation Panel for approval.</span></div>";	
			}
			else {
				$msg[] = "<div class=\"success\"><span>Defence Proposal Evaluation Report has been submitted successfully to Chairman for review.</span></div>";	
			}
		...
		//if ($roleType == 'EC') {
			...
			$sqlotherstaff = "SELECT name, email from new_employee WHERE empid = 'S012013060023'";
			$dbcstaff2 = $dbc;
			...
			$sql4 = "SELECT id, description, remarks
			FROM ref_defense_marks
			WHERE STATUS = 'A'
			AND id = '$defenseMarksCb'								
			ORDER BY seq";
			$dba4 = $dba;
			$result_sql4 = $dba4->query($sql4); 
			$dba4->next_record();
			
			$recDesc = $dba4->f('description');
			
			$sqlmsg = "SELECT const_value FROM base_constant
			WHERE const_term = 'MESSAGE_EXA_TO_SCH'";
			$dbgmsj = $dbg;
			...
			{
				include("../../../app/application/inbox/defense/submit_defense_school.php");
			}
			
			////email notification/////
			$sqlemail = "SELECT const_value FROM base_constant
			WHERE const_term = 'EMAIL_EXA_TO_SCH'";
			$dbg1 = $dbg;
			...
			{
				//include("../../../app/application/email/defense/email_submit_defense_school.php");
			}			
			////////////////////end notification//////////////////////////////////
		}
	}
...
$sql2 = "SELECT b.id as evaluation_detail_id, a.pg_defense_id,
		...
		if ($evaluationDetailStatus1=="IN1" || $evaluationDetailStatus1=="SV1") {?>
			...
			<table>
				...
				<tr>
					...
					<td>:</td>
					<td><strong><?=$examinerDesc?></strong><input type="hidden" name="roleType" id="roleType" value="<?=$supervisorTypeId?>" /></td>
				</tr>				
				<tr>
					<td>Last Update</td>
					<td>:</td>
				...
				<?				
					...
					?>				
				<td><label><?=$sessionTypeDesc?> - <?=$defenseDate?>, <?=$defenseSTime?> to <?=$defenseETime?>, <?=$venue?></label>
				<input type="hidden" name="calendarId" id="calendarId" value="<?=$recommendedId?>" /></td>				
			</tr>   				
			</table>				
			</br>
			<?
				...
				if ($row_cnt_examiner>0) {
					...
					do {
						...
						<tr>
							...
							FROM pg_evaluation_detail a
							LEFT JOIN pg_evaluation b ON (b.id = a.pg_eval_id)
							...
							?>
							<td><a href="../defense/review_evaluation_partner.php?id=<?=$evaluationId?>&cid=<?=$calendarId?>&tid=<?=$thesisId;?>&pid=<?=$proposalId?>&eid=<?=$employeeId?>&mn=<?=$studentMatrixNo?>&ref=<?=$referenceNo?>&refd=<?=$referenceNoDefence?>&rol=<?=$partnerSupervisorTypeId?>&rolx=<?=$supervisorTypeId?>"><?=$partnerEvaluationDesc?></a><label></label></td>
						<?
