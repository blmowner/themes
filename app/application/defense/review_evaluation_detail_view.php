		...
		if ($row_cnt_examiner>0) {
			...
			do {
				...
				<tr>
					...
					?>
					<td><a href="../defense/review_evaluation_partner_view.php?id=<?=$evaluationId?>&cid=<?=$calendarId?>&tid=<?=$thesisId;?>&pid=<?=$proposalId?>&eid=<?=$employeeId?>&mn=<?=$studentMatrixNo?>&ref=<?=$referenceNo?>&refd=<?=$referenceNoDefence?>&rol=<?=$partnerSupervisorTypeId?>&rolx=<?=$supervisorTypeId?>"><?=$partnerEvaluationDesc?></a><label></label></td>
				<?
...
<legend><h3><strong>Work Achievement</strong></h3></legend>
	...
	<table>
		...
		<!--    <td><input type="button" name="btnPrint" value="Print PDF" onClick="javascript:document.location.href='../defense/pdf_defense_approval_report.php?id=<?=$id?>&mn=<?=$studentMatrixNo?>&tid=<?=$thesisId?>&pid=<?=$proposalId?>&ref=<?=$referenceNo?>&refd=<?=$referenceNoDefence?>&rol=<?=$supervisorTypeId?>&cid=<?=$calendarId?> &did=<?=$defenseId?>';" /></td>-->
			<td> <input type="button" name="btnView" id="btnView" value="Print PDF"onclick="javascript:open_win('pdf_defense_approval_report.php?id=<?=$id?>&mn=<?=$studentMatrixNo?>&tid=<?=$thesisId?>&pid=<?=$proposalId?>&ref=<?=$referenceNo?>&refd=<?=$referenceNoDefence?>&rol=<?=$supervisorTypeId?>&cid=<?=$calendarId?> &did=<?=$defenseId?>',800,700,0,0,0,1,0,1,1,0,5,'winupload'); "/></td>
