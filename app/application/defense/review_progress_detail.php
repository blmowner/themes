...
if(isset($_POST['btnRequestChange']) && ($_POST['btnRequestChange'] <> ""))
{
	...
	if(empty($msg)) 
	{
		if ($progressDetailStatus == 'IN1') {
			...
			$lock_tables="LOCK TABLES pg_progress_detail WRITE"; //lock the table
			...
			{	
				...
				$lock_tables="LOCK TABLES pg_discussion_detail WRITE"; //lock the table
				$db->query($lock_tables);
			...
			$lock_tables="LOCK TABLES pg_discussion_detail WRITE,
			file_upload_progress WRITE, pg_progress WRITE"; //lock the table
			$db->query($lock_tables);
		...
		elseif ($progressDetailStatus == 'SV1')  {
			...
			$lock_tables="LOCK TABLES pg_progress_detail WRITE, pg_discussion_detail WRITE,
			file_upload_progress WRITE, pg_progress WRITE"; //lock the table
			$db->query($lock_tables);
...
if(isset($_POST['btnApproved']) && ($_POST['btnApproved'] <> ""))
{
	...
	if(empty($msg)) 
	{
		if ($progressDetailStatus == 'IN1') {
			...
			$lock_tables="LOCK TABLES pg_progress WRITE"; //lock the table
			$db->query($lock_tables);
		...
		elseif ($progressDetailStatus == 'SV1')  {
			...
			$lock_tables="LOCK TABLES pg_progress_detail WRITE, pg_discussion_detail WRITE,
			pg_progress WRITE"; //lock the table
			$db->query($lock_tables);
	...
	if(empty($msg)) 
	{
		if ($progressDetailStatus == 'IN1') {
			...
			{	
				...
				$lock_tables="LOCK TABLES pg_discussion_detail WRITE,
				file_upload_progress WRITE"; //lock the table
				$db->query($lock_tables);
		...
		elseif ($progressDetailStatus == 'SV1')  {
			...
			$lock_tables="LOCK TABLES pg_progress_detail WRITE, pg_discussion_detail WRITE"; //lock the table
			$db->query($lock_tables);
...
<html xmlns="http://www.w3.org/1999/xhtml">
		...
		<table>
			...
			<table width="75%" border="1" cellpadding="3" cellspacing="1" style="border-collapse:collapse;" class="thetable">			
					<tr>
						...
						<th width="15%">Meeting Start Time</th>
						<th width="15%">Meeting End Time</th>
						<th width="15%">Meeting Mode</th>
					</tr>
	...
	else if ($progressDetailStatus1 == 'REQ')  
	{
		?>
			<table>
				<tr>
					<td>Monthly Progress Report <strong><?=$_POST['referenceNo']?></strong> has been submitted back to the student <strong><?=$_POST['sname']?></strong> for the required changes.</td>
				</tr>
			</table>							
			<table>
				<tr>
