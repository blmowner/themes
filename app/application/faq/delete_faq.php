...
if(isset($_REQUEST['btnDelete']) && ($_REQUEST['btnDelete'] <> ""))
{
	...
	if (sizeof($_POST['deleteBox'])== 0) {
		$msg[] = "<div class=\"error\"><span>Please select the required FAQ before proceed with the deletion.</span></div>";	
	}
	...
	if(empty($msg))  
	{
		...
		$msg[] = "<div class=\"success\"><span>The selected FAQ has been deleted successfully.</span></div>";
	}
}	
		...
		for ($i=0;$i<count($categoryCountArray);$i++) {
			...
			<?if ($categoryCountArray[$i] > 0) {			
				...
				<?for ($j=0;$j<$categoryCountArray[$i];$j++) {
					...
					?>
					<tr class="<?=$color?>">
						<td align="center"><input type="checkbox" name="deleteBox[]" value="<?=$faqIdArray[$i][$j];?>"/></td>
