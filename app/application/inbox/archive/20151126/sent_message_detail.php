	...
	<table>
		<tr>
			...
			<?if ($theRecipientStatus != 'NEW') {?>	
				<td bgcolor="#DBDBDB"><label><?=$theRecipientStatusDate?></label></td>			
			<?}
			else {
				?>
		...
		<tr>
			...
			<td bgcolor="#DBDBDB"><label><?=convertname($user_id)?></label></td>
			<input type="hidden" name="userId" id="userId" value = "<?=$user_id?>" /></input>
		</tr>
		<tr>
			<input type="hidden" name="messageId" id="messageId" value="<?=$theMessageId?>"></input>
			<?
			  ...
			  <?php
				...
				if ($row_cnt>0)
				{
					...
					{ 
						...
						?>
						<a href="download.php?fc=<?=$row["fu_cd"];?>&al=S" title="File Description: <?=$row["fu_document_filedesc"];?>"><?=$namefile;?>: 
