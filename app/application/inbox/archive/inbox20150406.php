...
?>
  <div id = "msg" class = 'msg'>
  	<table width="895" height="303" bordercolor="#FFFFFF">
  	  <tr bgcolor="#5B74A8">
  	    <td width="814" height="34"><span class="style4 style6"><span class="style4">From:
            <?= $from; ?>
        </span></span></td>
  	    <td width="69"><span class="style4 style6"><span class="style4">
  	      <?= $myFormatForView; ?>
        </span></span></td>
      </tr>
  	  <tr bgcolor="#5B74A8">
        <td height="21" colspan="2"><span class="style4 style6"><span class="style4">To:
          <?= $name; ?>
          <input type="hidden" name="textfield" value ="<?= $userID; ?>" />        
        </span></span></td>
      </tr>
		<tr bgcolor="#5B74A8">
		  <td height="21" colspan="2" bgcolor="#5B74A8"><p class="style3">Subject:
	        <?=$subject; ?>
	      </p></td>
	  </tr>
		<tr>
          <td height="185" bgcolor="#DBDBDB"><?=$message1?></td>
		  <td bgcolor="#5B74A8"><p><a href="reply.php?id=<?=$id?>&amp;subject=<?=$subject?>&amp;from=<?=$from?>&amp;userID=<?=$userID?>" class="style3">Reply Message </a></p>
		      <p><a href ="../../application/inbox/inbox.php?remove=<?=$id?> " class = "style4 remove" onclick = "return confirm('Do you wish to proceed?:')" ><strong>Delete message</strong></a></p></td>
	  </tr>
		<tr>
		<?php 
    	$sql_file = "SELECT inbox_id  
		...
		{
		 ?>
		  <td height="28" colspan="2" bgcolor="#B7B7FF"><a href="../../../include/download.php?id=<?php echo $inbox_id; ?>&table=file_upload_inbox&where=inbox_id&theArray=file_name+file_type+file_size+file_data" target="_blank">Download</a></td>
		 <? }
		 else
		 {
...
?>
 	...
 	if(isset($_GET['remove']))
	{
		...
		//$msg[] = "<div class=\"success\"><span>Delete Success!</span></div>";
...
<div class = "CSSTableGenerator">
	<table width="898">
		...
		<?
			...
			echo '<td><a style="color: #000000" href = "?msg='.$id.'">'.$from.'</a></td>';
			echo '<td><a style="color: #000000" href = "?msg='.$id.'">'.$subject.'</a></td>';
			echo '<td><a style="color: #000000" href = "?msg='.$id.'">'.$myFormatForView.'</a></td>';
			echo '<td><a style="color: #000000" href = "?msg='.$id.'">'.$open.'</a></td>';
			echo '</tr>'
	   ...
	   <legend>
	   <input type="submit" name="btnDelete" value="Delete" onclick = "return confirm('Do you wish to proceed?:')" class = "fancy-button-red" />
	   </legend>
...
if(isset($_POST['btnDelete']) && ($_POST['btnDelete'] <> ""))
{
	...
	if (isset($_REQUEST['id'])) /* checks weather $_GET['empids'] is set */
	{
		...
		header("location:inbox.php");/* Goes back to index.php */
		}
	}
	else 
	{	
