...
$xpic_desc = clean($xpic_desc, 50);
	...
	if (is_uploaded_file($xpic_file))
	{
		...
		$upload_id = runnum('fu_cd','file_upload_inbox');
		...
		{
			header("Location: new_message_receipt.php?data=$fileContent&status=T&userId=$user_id");
		}
		else 
		{
			header("Location: new_message_receipt.php?status=F&userId=$user_id");
		}
	}
?>
...
<body>
<form name="frmEmpUpdPic" method="post" action="new_message_upload.php?mid=<?=$_POST['messageId']?>" enctype="multipart/form-data">
