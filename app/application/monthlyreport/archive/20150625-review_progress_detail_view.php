	...
	<table border="0">
			...
			<table border="1" style="border-collapse:collapse;" cellpadding="2" cellspacing="1" width="100%" class="thetable">				
				...
				$sql_supervisor = "SELECT a.id, b.pg_employee_empid, c.ref_supervisor_type_id, d.description as supervisor_desc, 
						...
						AND a.pg_thesis_id = '$thesisId'
						AND a.pg_proposal_id = '$proposalId'
						AND b.pg_employee_empid NOT IN ('$user_id')
						AND a.archived_status is null
						AND b.archived_status is null
				...
				if ($row_cnt_supervisor>0) {
					...
					do {
						...
						<tr>
							...
							?>								
								<td><a href="review_progress_detail_staff.php?eid=<?=$employeeId?>&mn=<?=$studentMatrixNo?>&pid=<?=$proposalId;?>&tid=<?=$thesisId;?>&id=<?=$id;?>" title="Description of topic or Issues facing by Student - Read more..."><img src="../images/view.jpg" width="45" height="30" style="border:0px;" title="View feedback"></a>
								</td>	
							<?}
							else {
								?>
			...
			<table>
				<tr>
					<td><textarea name="supervisorIssues" class="ckeditor" ><?=$supervisorIssues; ?></textarea></td>
				</tr>
				<tr>
