...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<legend><strong>List of Student's Monthly Progress Report</strong></legend>
			...
			for ($i=$reportMonth[$currMonth]-$reportMonth[$startMonth];$i>=0;$i--) {?>
				<tr>
					...
					if ($expectedMonth1 == $currMonth1) {						
						?>
						<td align="left"><label><?=$expectedMonth2?> <span style="color:#FF0000">*</span></label></td>
					<?}
					else {						
						?>
						<td align="left"><label><?=$expectedMonth2?></label></td>
						<?
					}?>
		...
		<table>
			<tr>		
				<td><input type="button" name="btnBack" value="Back" onclick="javascript:document.location.href='list_progress_report_faculty.php';" /></td>
				<td><!-- <input type="button" name="btnPrint" value="Print" onclick="javascript:open_win('list_progress_report_html.php?tid=<?=$thesisId?>&pid=<?=$proposalId?>&mn=<?=$matrixNo;?>&eid=<?=$employeeId?>',1020,780,0,0,0,0,1,1,0,0,5,'print');return false;" /></td>standard high 1020-->
				<input type="button" name="btnPrint" id="btnPrint" value="Print"onclick="javascript:open_win('pdf_list_progress_report.php?tid=<?=$thesisId?>&pid=<?=$proposalId?>&mn=<?=$matrixNo;?>&eid=<?=$employeeId?>',800,700,0,0,0,1,0,1,1,0,5,'winupload'); "/></td>
