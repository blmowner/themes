...
// Program Name: submit_progress.php
//
// Created by: Zuraimi
...
if(isset($_POST['btnNewReport']) && ($_POST['btnNewReport'] <> ""))
{
	$progressIdNewReport = $_POST['progressIdNewReport'];
	$thesisIdNewReport = $_POST['thesisIdNewReport'];
	$proposalIdNewReport = $_POST['proposalIdNewReport'];
	$curdatetime = date("Y-m-d H:i:s");	
	$sql1_0 = "UPDATE pg_progress
	SET status='APP', archived_status = 'ARC', archived_date = '$curdatetime'
	WHERE id = '$progressIdNewReport'
	AND pg_thesis_id = '$thesisIdNewReport'
	AND pg_proposal_id = '$proposalIdNewReport'
	AND student_matrix_no = '$user_id'
	AND respond_status = 'Y'
	AND status IN ('APP')
	AND archived_status IS NULL";

	$result1_0 = $dba->query($sql1_0); 
	$dba->next_record();
	
	$sql1_1 = "UPDATE pg_discussion
	SET archived_status = 'ARC', archived_date = '$curdatetime'
	WHERE progress_id = '$progressIdNewReport'
	AND pg_thesis_id = '$thesisIdNewReport'
	AND pg_proposal_id = '$proposalIdNewReport'
	AND student_matrix_no = '$user_id'
	AND archived_status IS NULL";

	$result1_1 = $dba->query($sql1_1); 
	$dba->next_record();
	
	$sql1_2 = "UPDATE pg_progress_detail
	SET archived_status = 'ARC', archived_date = '$curdatetime'
	WHERE pg_progress_id = (SELECT id
		FROM pg_progress
		WHERE id = '$progressIdNewReport'
		AND pg_thesis_id = '$thesisIdNewReport'
		AND pg_proposal_id = '$proposalIdNewReport'
	...
	$result1_2 = $dba->query($sql1_2); 
	$dba->next_record();
	
	$sql1_3 = "UPDATE pg_discussion_detail
	SET archived_status = 'ARC', archived_date = '$curdatetime'
	...
	$result1_3 = $dba->query($sql1_3); 
	$dba->next_record();
	
	$sql1_4 = "UPDATE file_upload_progress
	SET archived_status = 'ARC', archived_date = '$curdatetime'
	WHERE progress_id = '$progressIdNewReport'
	AND pg_proposal_id = '$proposalIdNewReport'		
	AND archived_status IS NULL";

	$result1_4 = $dba->query($sql1_4); 
	$dba->next_record();
...
if(isset($_POST['btnSave']) && ($_POST['btnSave'] <> ""))
{
	...
	if(empty($msg)) 
	{
		if ($progressStatus=="") {
			...
			echo $sql0 = "SELECT id,
			MONTHNAME(STR_TO_DATE(MONTH(STR_TO_DATE('$reportMonth','%M'))+1, '%m')) AS next_month
			FROM pg_progress
			...
			$dba->next_record();
			echo $nextMonth = $dba->f('next_month');
			echo $row_cnt_sql0 = mysql_num_rows($result_sql0);		
			
			if ($row_cnt_sql0 > 0 ) {
				$msg[] = "<div class=\"error\"><span>You have already submitted the report for month of $reportMonth $reportYear. Please prepare for the $nextMonth $reportYear report first!</span></div>";
			}
			else {
				
				echo $sql0_1 = "SELECT a.report_month, MONTH(STR_TO_DATE(a.report_month,'%M')) AS currentMonthNum,
				MONTHNAME(STR_TO_DATE(MONTH(STR_TO_DATE(a.report_month,'%M'))+1, '%m')) AS next_month
				FROM pg_progress a
				...
				ORDER BY currentMonthNum, a.report_year desc
				LIMIT 1";
				
				$result_sql0_1 = $dba->query($sql0_1); 
				$dba->next_record();
				$row_cnt_sql0_1 = mysql_num_rows($result_sql0_1);
				$currentMonthNum = $dba->f('currentMonthNum');				
				$currentMonth = $dba->f('report_month');
				$nextMonth = $dba->f('next_month');
				$newMonth = date("n",strtotime($currentMonthNum));
				$currentMonthNum;
				$newMonth--;
				...
				if ($newMonth == $currentMonthNum) {
					...
					{	
						...
						(id, progress_id, pg_thesis_id, pg_proposal_id, student_matrix_no, pg_chapter_id, pg_subchapter_id, discussed_status, content_discussion, 
						insert_by, insert_date, modify_by, modify_date )
						VALUES ('$newDiscussionId', '$progressId', '$thesisId', '$proposalId', '$user_id', '$chapterId[$k]', '$subchapterId[$k]',   
					...
					$msg[] = "<div class=\"error\"><span>You last progress report was for month of $currentMonth $reportYear. Please prepare for the $nextMonth $reportYear report first!</span></div>";
				}
			}
		}
			...
			{	
				...
				if ($row_cnt_sql6 == 0) {
					...
					$sql3 = " INSERT INTO pg_discussion
					(id, progress_id, pg_thesis_id, pg_proposal_id, student_matrix_no, pg_chapter_id, pg_subchapter_id, discussed_status, 
			...
			{	
				...
				if ($row_cnt_sql6_0 > 0) {
					...
					(id, progress_id, pg_thesis_id, pg_proposal_id, student_matrix_no, pg_chapter_id, pg_subchapter_id, content_discussion, discussed_status, 
					insert_by, insert_date, modify_by, modify_date)
					VALUES ('$newDiscussionId', '$newProgressId', '$pg_thesis_id', '$pg_proposal_id', '$student_matrix_no', '$pg_chapter_id',
					...
					(id, progress_id, pg_thesis_id, pg_proposal_id, student_matrix_no, pg_chapter_id, pg_subchapter_id, content_discussion, discussed_status, 
					insert_by, insert_date, modify_by, modify_date)
					VALUES ('$newDiscussionId', '$newProgressId', '$thesisId', '$proposalId', '$user_id', '$chapterId[$k]',
...
if(isset($_POST['btnSubmit']) && ($_POST['btnSubmit'] <> ""))
{
	...
	if(empty($msg)) 
	{
		if ($progressStatus=="") {
			...
			if ($row_cnt_sql0 > 0 ) {
				$msg[] = "<div class=\"error\"><span>You have already submitted the report for month of $reportMonth $reportYear. Please submit for the $nextMonth $reportYear report first!</span></div>";
			}
			else {
				
				$sql0_1 = "SELECT a.report_month, MONTH(STR_TO_DATE(a.report_month,'%M')) AS currentMonthNum,
				MONTHNAME(STR_TO_DATE(MONTH(STR_TO_DATE(a.report_month,'%M'))+1, '%m')) AS next_month
				FROM pg_progress a
				LEFT JOIN pg_progress_detail b ON (b.pg_progress_id = a.id)
				WHERE a.student_matrix_no = '$user_id'
				...
				if ($newMonth == $currentMonthNum) {
					...
					$msg[] = "<div class=\"error\"><span>You last progress report was for month of $currentMonth $reportYear. Please submit for the $nextMonth $reportYear report first!</span></div>";
				}				
			}				
		}
		else if ($progressStatus=="SAV") {//SAV
	...
	<?if ($row_cnt1 > 0) {
		if ($row_cnt_supervisor > 0 ) {
			if ($row_cnt2_1 > 0) {
				if ($progressStatus == "" || $progressStatus == "SAV" || $progressStatus == "REQ") {?>
						...
						<tr>
						  ...
						  <td><select name="reportMonth">
							...
							<select name="reportYear">
								...
								<?if ($reportYear=="2010") {?><option value="2010" selected="selected">2010</option><?} else {?><option value="2010">2010</option><?}?>
								<?if ($reportYear=="2011") {?><option value="2011" selected="selected">2011</option><?} else {?><option value="2011">2011</option><?}?>
								<?if ($reportYear=="2012") {?><option value="2012" selected="selected">2012</option><?} else {?><option value="2012">2012</option><?}?>
								<?if ($reportYear=="2013") {?><option value="2013" selected="selected">2013</option><?} else {?><option value="2013">2013</option><?}?>
					...
					<table>
						<tr>
							<td><button type = "button" name="btnDiscussionDetail" onclick="return newDicussion('<?=$proposalId?>','<?=$thesisId?>','<?=$id?>')">
								...
								do {
									...
									<tr>
										...
										<?}
											...
											?>
											<td align="left"><span style="color:#FF0000"><?=$supervisorType;?></span></td>
											<?
										}?>
										...
										<td><a href="progress_view_feedback.php?tid=<? echo $thesisId;?>&pid=<? echo $proposalId;?>&eid=<? echo $employeeId;?>&id=<? echo $id;?>&mn=<?=$user_id?>" name="thesisId" value="<?=$thesisId?>" title="View feedback"><img src="../images/view.jpg" width="45" height="30" style="border:0px;" title="View feedback"></a></td>	
										<?}
										else {
											?>
											<td></td>
					...
					<br/>
					<fieldset>
					<legend><strong>Content of Discussion</strong> <span style="color:#FF0000"> *</span></legend>
					<table>
						<tr>
							<td><label>To be filled in by Supervisor <em>(Please tick).</em></label></td>
						</tr>
					</table>
					<table>
						<?
					...
					<?
					if ($id != "")	{
						$tmpProgressId = " AND (progress_id = '$id' OR progress_id = '' OR progress_id IS NULL)";
					}
					else {
						$tmpProgressId = "";
					}
					...
					<table>
						<tr>
							<td><button type="button" name="btnAttachment" value="Attachment" onclick="return issueAttachment('<?=$proposalId?>','<?=$id?>','I')" >
					...
					<table>
						<tr>
							<td><button type="button" name="btnAttachment" value="Attachment" onclick="return issueAttachment('<?=$proposalId?>','<?=$id?>','A')" >
							Attachment <FONT COLOR="#FF0000"><sup><?=$row_cnt_tmp?></sup></FONT></button></td>
				...
				{?>
					...
					<legend><strong><span style="color:#FF0000">Notification Message</span></strong></legend>
						...
						<table>
							<tr>
								<td><strong>Current Status of Monthly Progress Report</strong></td>
							</tr>
						</table>
						<table width="100%" border="1" cellpadding="3" cellspacing="1" style="border-collapse:collapse;" class="thetable">			
							...
							if ($row_cnt_supervisor>0) {
								do {
									...
									$employeeId = $db_klas2->f('pg_employee_empid');
									$progressDetailStatus = $db_klas2->f('progress_detail_status');
									...
									<tr>
										<td align="center"><?=$varRecCount;?>.</td>					
										<?if ($supervisorType !='XS') {?>
											<td align="left"><?=$supervisorDesc;?></td>
										<?}
										else {
											?>
											<td align="left"><span style="color:#FF0000"><?=$supervisorDesc;?></span></td>
											<?											
										}?>
							...
							<? 	} while($db_klas2->next_record());
								...
								$sql_count = "SELECT a.status AS progress_detail_status, 
								...
								$result_sql_count = $db->query($sql_count); 
								$db->next_record();
								$row_count = mysql_num_rows($result_sql_count);
								$statusArray = Array();
								
								$i=0;
								do {
									$statusArray[$i] = $db->f('progress_detail_status');
									$i++;
								} while ($db->next_record());
								
								$newReport = 'N';
								$appStatus = 'N'; $appCount = 0;
								$in1Status = 'N'; $in1Count = 0;
								$reqStatus = 'N'; $reqCount = 0;
								$sv1Status = 'N'; $sv1Count = 0;
								for ($j=0; $j<$row_count; $j++) {
									if ($statusArray[$j] == 'APP') {
										$newReport = 'Y';
										$appStatus = 'Y';
										$appCount++;
									} else if ($statusArray[$j] == 'IN1') {
										$newReport = 'Y';
										$in1Status = 'Y';
										$in1Count++;
									} else if ($statusArray[$j] == 'REQ') {
										$newReport = 'N';
										$reqStatus = 'Y';
										$reqCount++;
									} else if ($statusArray[$j] == 'SV1') {
										$newReport = 'N';
										$sv1Status = 'Y';
										$sv1Count++;
									}
								...
								<table>
								<tr>
									<td><br/><strong><label>Summary</strong></label></td>
									</tr>
								</table>
								<?
								if ($newReport == 'Y') 
								{
									if ($appStatus == 'Y') {
										...
										<table>
										<tr>
											<td>No. of Approved Report: <strong><?=$appCount?></strong></td>
											</tr>
										</table>
										<?
									}
									if ($in1Status == 'Y') {
										...
										<table>
										<tr>
											<td>No. of Pending Report: <strong><?=$in1Count?></strong></td>
											</tr>
										</table>
										<?
									}
									if ($appStatus == 'Y' && $respondStatus == 'Y') {
										?>
										<input type="hidden" name="thesisIdNewReport" id="thesisIdNewReport" value="<?=$thesisId;?>">
										<input type="hidden" name="proposalIdNewReport" id="proposalIdNewReport" value="<?=$proposalId;?>">
										<input type="hidden" name="progressIdNewReport" id="progressIdNewReport" value="<?=$id;?>">
										<table>
											<tr>
												<td><br/><input type="submit" name="btnNewReport" value="Click here" onclick="return newReport()"> to submit new monthly progress report.</td>
											</tr>											
										</table>
										<?
									}
									...
									if ($appStatus == 'Y') {
										...
										<table>
										<tr>
											<td><?=$no++?>. No. of Approved Report: <strong><?=$appCount?></strong></td>
											</tr>
										</table>
										<?
									}
									if ($in1Status == 'Y') {
										...
										<table>
										<tr>
											<td><?=$no++?>. No. of Pending Report: <strong><?=$in1Count?></strong></td>
											</tr>
										</table>
										<?
									}
									if ($reqStatus == 'Y') {
										...
										<table>
										<tr>
											<td><?=$no++?>. No. of Request Change Report: <strong><?=$reqCount?></strong></td>
											</tr>
										</table>
										<?
									}
									if ($sv1Status == 'Y') {
										...
										<table>
										<tr>
											<td><?=$no++?>. No. of Review in Progress Report: <strong><?=$sv1Count?></strong></td>
											</tr>
										</table>
										<?
									}
									...
									if ($reqStatus == 'Y') {
										...
										<table>
											<tr>
												<td><br/>Your Supervisor/Co-Supervisor has requested some changes to your report. Click here <a href="../monthlyreport/edit_progress.php?pid=<?=$proposalId;?>&tid=<?=$thesisId;?>&id=<?=$id;?>"><img src="../images/click_here.jpg" width="45" height="30" style="border:0px;" title="View Submitted Monthly Progress Report"></a> to edit your monthly progress report. </td>
											</tr>
										</table>
										<?
									}
				...
				<legend><strong><span style="color:#FF0000">Notification Message</span></strong></legend>
					<table>
						<tr>
							<td>Please define <a href="../monthlyreport/create_chapter.php">chapter</a> for your thesis first before submit monthly progress report.</td>
						</tr>
					</table>				
				</fieldset>
				<?
			...
			<legend><strong><span style="color:#FF0000">Notification Message</span></strong></legend>
				<table>
					<tr>
						<td>Your Supervisor/Co-Supervisor has yet to accept the invitation to supervise you as his/her student.
						You can submit the report once he/she has accepted it.</td>
					</tr>
				</table>				
			</fieldset>
			<?			
		...
		?>
			...
			<legend><strong><span style="color:#FF0000">Notification Message</span></strong></legend>
				<table>
					<tr>
						<td>You are not able to submit your <strong>monthly progress report</strong> due to your thesis proposal status.</br><br/>
						Please submit your thesis proposal if you have not done so. If you did, your thesis proposal is still pending for approval by the Senate.</br>
						Please check again later for the status.</td>
					</tr>
				</table>				
			</fieldset>
		<?
