	...
	if(empty($msg)) 
	{
		if ($progressStatus=="") {
			...
			{						
				...
				FROM base_constant WHERE const_term = 'MESSAGE_STU_TO_SUP'";
				$resultmsg = $dbd->query($sqlmsg);
				$dbd->next_record();
				$msg =$dbd->f('const_value');
				if($msg == 'Y')
				{
					include("../../../app/application/inbox/monthly/new_report_msg.php");
				}
			...
			$sql6_0 = "SELECT id, pg_progress_id, reference_no, pg_thesis_id, pg_proposal_id, student_matrix_no, 
			IFNULL(meeting_date,'0000-00-00 00:00:00') as meeting_date, IFNULL(meeting_stime,'00:00:00') as meeting_stime,
			IFNULL(meeting_etime,'00:00:00') as meeting_etime, add_status, insert_by, insert_date, modify_by, modify_date,
			archived_status, archived_date
			FROM pg_progress_meeting
			...
			if ($row_cnt >0){
				do {
					...
					$sql6_1 = "INSERT INTO pg_progress_meeting
					(id, pg_progress_id, reference_no, pg_thesis_id, pg_proposal_id, student_matrix_no, meeting_date, meeting_stime, meeting_etime,
					add_status, insert_by, insert_date, modify_by, modify_date)
					VALUES ('$newProgressMeetingId', '$newProgressId', '$referenceNo', '$pg_thesis_id', '$pg_proposal_id', '$student_matrix_no', 
					'$meeting_date', '$meeting_stime', '$meeting_etime', '$add_status', '$insert_by', '$insert_date', '$modify_by', '$modify_date')";
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	function newDicussion(pid, tid, pgid, ref) 
	{
		...
		if (ask) 
		{
			document.location.href = "../monthlyreport/progress_discussion.php?pid=" + pid + "&tid=" + tid + "&pgid=" + pgid + "&ref=" + ref;
			return true;
		}
		return false;
	}
	...
	if ($progressStatus == "" || $progressStatus == "SAV" || $progressStatus == "REQ") {?>
				...
				if ($row_cnt_supervisor>0) {
					do {
						...
						<tr>
							...
							WHERE a.student_matrix_no = '$user_id'
							AND a.reference_no = '$referecenNo'
		...
		<?
		if ($id != "")	{
			$tmpProgressId = " AND progress_id = '$id'";
		}
		else {
	...
	{?>
		...
		<legend><strong>Monthly Progress Report Status</strong></legend>
			...
			<table>
				<tr>
					<td><br/>Click here <a href="../monthlyreport/view_progress.php?pid=<?=$proposalId;?>&tid=<?=$thesisId;?>&ref=<?=$referenceNo?>"><img src="../images/view.jpg" width="45" height="30" style="border:0px;" title="View Submitted Monthly Progress Report"></a> to view your previously submitted report. </td>
				</tr>									
			</table>		
		</fieldset>
		<table>
