...
$xpic_desc = clean($xpic_desc, 50);
	...
	if (is_uploaded_file($xpic_file))
	{
		...
		if ($_GET['al'] == 'S') { //S - Student
			...
			VALUES ('$fu_cd','$FileName','$fileDesc','$xpic_file_type','$fileContents','$userid','$progressId',
		...
		if (($db->query($sql2)) && @ mysql_affected_rows() == 1) 
		{
			...
			echo "opener.location.href=\"submit_progress_attachment.php?pid=$proposalId&at=$attachmentType\";";
			echo "document.location.href=\"progress_proposal_receipt.php?data=$fileContent&status=T&userId=$userid&ppid=$pid&fucd=$fu_cd\";";
			echo "</script>";
			//header("Location: progress_proposal_receipt.php?data=$fileContent&status=T&userId=$userid&ppid=$pid&fucd=$fu_cd");
		}
		else 
		{
			header("Location: progress_proposal_receipt.php?status=F&userId=$userid&ppid=$pid&fucd=$fu_cd");
		}
	}
?>
...
<body>
<form name="frmEmpUpdPic" method="post" action="progress_monthly_upload.php?pid=<?=$pid?>&al=<?=$_GET['al']?>&at=<?=$_GET['at']?>&progid=<?=$_GET['progid']?>" enctype="multipart/form-data">
