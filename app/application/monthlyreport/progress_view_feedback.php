...
// Program Name: progress_view_feedback.php
//
// Created by: Zuraimi
...
DATE_FORMAT(b.responded_date, '%d-%b-%Y') as responded_date, a.student_matrix_no, a.pg_thesis_id, a.pg_proposal_id, 
...
c.description as progress_desc
FROM pg_progress a
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	function respConfirm () {
		...
		<table>
			<tr>
				<td><label><strong>Supervisor/Co-Supervisor</strong></label></td>
			</tr>
		</table>
		<table>			
				...
				$sql_supervisor = "SELECT a.id, b.pg_employee_empid, a.report_month, a.report_year, DATE_FORMAT(a.submit_date, '%d-%b-%Y') as submit_date, 
						...
						AND a.pg_proposal_id = '$proposalId'
						AND a.archived_status is null
						AND c.pg_student_matrix_no = '$user_id'
				...
				$varRecCount=0;	
					...
					<table>
						<tr>
							<td><input type="button" name="btnBack" value="Back" onClick="javascript:document.location.href='../monthlyreport/edit_progress.php?mn=<?=$studentMatrixNo?>&pid=<?=$proposalId;?>&tid=<?=$thesisId;?>&id=<?=$id;?>&ref=<?=$referenceNo;?>';" /></td>
						</tr>
					</table>
					<fieldset>
					...
					<table>
						...
						?>
							...
							<?
								$sql_discussion = " SELECT a.discussed_status, b.id as discussion_id
					...
					<table>
						<tr>
							<td><textarea name="advice" id="advice" class="ckeditor"/><?=$advice?></textarea>
							</br><strong>Attachment by Student:</strong></br>
