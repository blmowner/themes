...
$xpic_desc = clean($xpic_desc, 50);
	...
	if (is_uploaded_file($xpic_file))
	{
		...
		if ($_GET['al'] == 'S') { //S - Student
			...
			(fu_cd, fu_document_filename,fu_document_filedesc,fu_document_filetype,  fu_document_filedata, student_matrix_no, progress_id, 
			insert_date, insert_by, modify_date, modify_by,pg_proposal_id, attachment_level, attachment_type, status, upload_status)	
			VALUES ('$fu_cd','$FileName','$fileDesc','$xpic_file_type','$fileContents','$userid',null/*'".$_GET['prgid']."'*/,
			now(),'$userid',now(),'$userid','".$_GET['pid']."',
			'".$_GET['al']."', '".$_GET['at']."','A', 'TMP' )";
		}
		else {//F - Faculty
			$sql2="INSERT INTO file_upload_progress 
		...
		if (($db->query($sql2)) && @ mysql_affected_rows() == 1) 
		{
			...
			//echo "opener.location.href=\"review_monthly_attachment.php?tid=$thesisId&pid=$proposalId&prgid=$progressId&at=$attachmentType\";";
			echo "opener.location.href=\"review_monthly_attachment.php?tid=".$_GET['tid']."&pid=".$_GET['pid']."&mn=".$_GET['mn']."&prgid=".$_GET['prgid']."&al=".$_GET['al']."&at=".$_GET['at']."\";";
			echo "document.location.href=\"review_monthly_receipt.php?data=$fileContent&status=T&userId=$userid&ppid=$pid&fucd=$fu_cd\";";
			echo "</script>";
			//header("Location: progress_proposal_receipt.php?data=$fileContent&status=T&userId=$userid&ppid=$pid&fucd=$fu_cd");
		}
		else 
		{
			header("Location: review_monthly_receipt.php?status=F&userId=$userid&ppid=$pid&fucd=$fu_cd");
		}
	}
?>
