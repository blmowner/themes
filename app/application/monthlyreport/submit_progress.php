...
if(isset($_POST['btnSubmit']) && ($_POST['btnSubmit'] <> ""))
{
	...
	$chapterNo = $_POST['chapterNo'];
	$subchapterNo = $_POST['subchapterNo'];
	$supervisorIdEmail = $_POST['supervisorIdArray'];
	...
	if(empty($msg)) 
	{
		if ($progressStatus=="") {
			...
			$receiveemail = $dbn->f('email');
			$studname = $dbn->f('name');

			$sqlthesis = "SELECT thesis_title FROM pg_proposal WHERE pg_thesis_id = '$thesisId'";
			$resultthesis = $dbn->query($sqlthesis);
			$resultsqlthesis = $dbn->next_record(); 
			$title = $dbn->f('thesis_title');
			
			while (list ($key,$val) = @each ($supervisorIdEmail)) 
			{						
				...
				FROM base_constant WHERE const_term = 'EMAIL_STU_TO_SUP'";
				$resultvalidate = $dbd->query($sqlvalidate);
				$dbd->next_record();
				$valid =$dbd->f('const_value');
				if($valid == 'Y')
				{
					include("../../../app/application/email/monthlyemail/new_report.php");
				}
			}
