...
?>
  ...
  <form id="form1" name="form1" method="post" enctype="multipart/form-data" onsubmit="return saveRec();">
	 ...
	 <fieldset>
		...
		<table>
			<tr>
				<td><input type="submit" name="submit" value="Add Attachment" onclick="javascript:open_win('submit_progress_upload.php?tid=<?=$pgThesisId;?>&pid=<?=$pgProposalId;?>&prgid=<?=$progressId?>&al=S&at=<?=$attachmentType?>&ref=<?=$referenceNo?>',480,280,0,0,0,1,0,1,1,0,5,'winupload'); "/></td>
			</tr>
		</table>
		<table>
		<tr>							
	...
	<table>
		<tr>
			<td><input type="button" name="btnBack" value="Back" onclick="javascript:document.location.href='../monthlyreport/edit_progress.php?tid=<?=$pgThesisId?>&pid=<?=$pgProposalId?>&ref=<?=$referenceNo?>';" /></td>		
