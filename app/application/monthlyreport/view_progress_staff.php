...
// Program Name: view_progress_staff.php
//
// Created by: Zuraimi
...
WHERE a.reference_no = '$theReferenceNo'
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<table border="1" style="border-collapse:collapse;" cellpadding="2" cellspacing="1" width="80%" class="thetable">			
			...
			AND a.pg_thesis_id = '$thesisId'
			AND a.pg_proposal_id = '$proposalId'
			AND a.reference_no = '$theReferenceNo'
			AND a.archived_status is null
			AND b.archived_status is null
			...
			if ($row_cnt_supervisor>0) {
				do {
					...
					<tr>
						...
						<td><a href="progress_view_report_staff.php?eid=<?=$employeeId?>&mn=<?=$studentMatrixNo?>&pid=<?=$proposalId;?>&tid=<?=$thesisId;?>&id=<?=$progressId;?>&ref=<?=$referenceNo;?>&theRef=<?=$theReferenceNo;?>&pdid=<?=$progressDetailId?>" title=""><img src="../images/view.jpg" width="45" height="30" style="border:0px;" title="View feedback"></a>
								</td>	
	...
	<table>
		<tr>
			<td><input type="button" name="btnBack" value="Back" onClick="javascript:document.location.href='../monthlyreport/list_other_progress_report.php?mn=<?=$studentMatrixNo?>&pid=<?=$proposalId;?>&tid=<?=$thesisId;?>&ref=<?=$referenceNo;?>'" /></td>
		</tr>
	</table>			
	<?
	...
	<table>
		...
		if($archivedStatus == '')
		{
			...
			AND student_matrix_no = '$studentMatrixNo'
			AND reference_no = '$theReferenceNo'
