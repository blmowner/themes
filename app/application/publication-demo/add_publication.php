...
if($_POST['btnAdd'] <> "") 
{
	...
	$publicationDetailId = $_REQUEST['pub_id'];
	...
	if(empty($msg)) {
		if($status=="") {
			...
			$_POST['publicationId'] = $newPublicationId;
...
if($_POST['btnSave'] <> "") 
{
	...
	if(empty($msg)) {
		if($status=="") {
			$sql5 = "SELECT id
			FROM pg_publication 
			WHERE issn_no = '$newIssn'
			AND archived_status IS NULL";
			$result_sql5= $db->query($sql5);
			$db->next_record();
			$row_cnt_sql5 = mysql_num_rows($result_sql5);
			$thePublicationId = $db->f('id');
			
			if ($row_cnt_sql5 > 0) {
				$curdatetime = date("Y-m-d H:i:s");
				$sql="UPDATE pg_publication
				SET modify_by='$user_id', modify_date='$curdatetime'
				WHERE id='$thePublicationId'
				AND status = 'A'
				AND archived_status IS NULL";
				$dbe->query($sql);
				...
				(id, issue, volume_id, publication_id, author, title, publisher_id, abstract, type_pub_id, website,
				country_id, publication_uploader, published_date, insert_by, insert_date, status, keyword, modify_by, modify_date)
				VALUES ('$newPublicationDetailId', '$newIssue', '$newVolume', '$thePublicationId', '$author', '$title', '$publisher', '$abstract', 
				'$add_publication_type', '$website', '$add_country', '$user_id', STR_TO_DATE('$publishedDate','%d-%M-%Y'), 
				'$user_id', '$curdatetime', 'SAV', '$keyword', '$user_id', '$curdatetime')";
				$dbe->query($sql2);
				
				$_POST['publicationId'] = $thePublicationId;
			}
			else {
				$curdatetime = date("Y-m-d H:i:s");
				$newPublicationId = runnum('id','pg_publication');
				$sql="INSERT INTO pg_publication
				(id, issn_no, status, insert_by, insert_date, modify_by, modify_date)
				VALUES('$newPublicationId','$newIssn', 'A', '$user_id', '$curdatetime', '$user_id', '$curdatetime')";?><br><br><?
				$dbe->query($sql);
				
				$sql6 = "SELECT * FROM file_upload_publication
				WHERE upload_by = '$user_id'
				AND publication_id IN (NULL, '')";
				$dbfile = $db;
				$result_sql6= $dbfile->query($sql6);
				$dbfile->next_record();
				$row_cnt_sql6 = mysql_num_rows($result_sql6);
				
				if($row_cnt_sql6>0) {
				
					$sqlfile="UPDATE file_upload_publication
					SET publication_id='$newPublicationId', insert_by='$user_id', insert_date= '$curdatetime',
					modify_by = '$user_id', modify_date = '$curdatetime'
					WHERE upload_by = '$user_id'
					AND publication_id IN (NULL, '')";
					$dbfile = $dbe;
					$dbfile->query($sqlfile);
				}		
