...
if($_POST['btnupdate'] <> "") 
{
	...
	$authorId = "A".runnum3('id','pg_author');
	...
	if(empty($msg))
	{
		...
		else if(!empty($pub_id))
		{
			if(preg_match('/^[a-zA-Z ,.]+$/', $author))
			{
				...
				SET status= 'I', modify_by = '$user_id', modify_date = '$curdatetime3'
				WHERE id = '$pub_id'
				...
				$sqlUploadUpdate="UPDATE file_upload_publication SET publication_id = '$publicId'
				WHERE upload_by = '$user_id'
				AND publication_id = '$pub_id'";			
				
				$result = $dbe->query($sqlUploadUpdate); 
								
				/*$sqlupdateauthor = "UPDATE pg_author 
	...
	if(empty($msg))
	{
		...
		else if(!empty($pub_id))
		{
			if(preg_match('/^[a-zA-Z ,.]+$/', $author))
			{
				...
				author = '$author', modify_by = '$user_id', modify_date = '$curdatetime3'
				WHERE id = '$pub_id'
				AND insert_by is not null
				AND insert_date is not null";	 
				$resultPublicationDetail = $dbe->query($publicationDetail);
...
if($_POST['btnsave'] <> "") 
{
		...
		else if(!empty($pub_id))
		{
			if(preg_match('/^[a-zA-Z ,.]+$/', $author))
			{
				...
				/*if(!empty($authorId))
				{
					...
					{	
						$row_id ++;
						$sqlauthor = "SELECT * FROM pg_author
						WHERE publication_id = '$pub_id'
					...
					if ($row_id == $row_name)			
					{
						...
						{	
							echo $sqlupdateauthor = "UPDATE pg_author 
							SET name = '$NAME'
							WHERE publication_id= '$pub_id'";
							if(!empty($NAME))
							{
	...
	DATE_FORMAT(a.publish_date,'%d-%b-%Y') AS publishDate, a.type_pub_id, a.publication_status,
	...
	LEFT JOIN pg_author g ON (g.id = a.author_id)
	...
	$publication_status = $db->f('publication_status');
	//ref_publisher//
...
$(document).ready(function(){
	...
	<table class="idd">
		<tr>
			...
			<input type="text" id="pdate" readonly="readonly" name="pdate" value="<?= $publishDate?>"/>
		...
		<tr>
			<td colspan="6" style="background-color: rgba(0, 0, 0, 0.1); "><span style="color:#FF0000">Note</span>: Only numeric can be used
			</p></td>
		</tr>
		...
		<tr>
			...
			<td style="background-color: rgba(0, 0, 0, 0.1); " colspan="5"><input type = "text" id="keyword" name="keyword" size="50" value="<?=$keyword?>"/>			</td>
		</tr>
		<tr>
	...
	<table>
		<tr>
			<td align="right"><!--<input type="submit" id="btnsave" name="btnsave" value = "Save as Draft" />-->
				<? if($publication_status == 'S') {?>
					<input type="submit" id="btnsave" name="btnsave" value = "Save" />
					<input type="submit" id="btnadd" name="btnadd" value = "Add" />
				<? } else { ?>
					<input type="submit" id="btnupdate" name="btnupdate" value = "Update" />					
				<? } ?>
				<input type="button" name="btnBack" value="Back" onClick="javascript:document.location.href='../publication/list_of_publication.php';" />
			</td>
