...
$xpic_desc = clean($xpic_desc, 50);
	...
	if (is_uploaded_file($xpic_file))
	{
		...
		$upload_id = runnum('fu_cd','file_upload_publication');
		...
		|| $FileType == 'jpg' || $FileType == 'xlsx' || $FileType == 'pptx' || $FileType == 'ppt' || $FileType == 'oxps')
		{
			$sql2 = "INSERT INTO file_upload_publication (
			fu_cd,  
			...
			if (($db->query($sql2)) && @ mysql_affected_rows() == 1) 
			{
				...
				echo "opener.location.href=\"edit_publication_attachment.php?pid=$publicationId&pdid=$pubDetailId\";";
				echo "document.location.href=\"edit_public_receipt.php?data=$fileContent&status=T&userId=$user_id&fucd=$upload_id\";";
				echo "</script>";
				//header("Location: new_proposal_receipt.php?data=$fileContent&status=T&userId=$user_id&fucd=$upload_id");
			}
			else 
			{
				header("Location: edit_public_receipt.php?status=F&userId=$user_id&fucd=$upload_id");
			}
		}
		else
		{
...
<form name="frmEmpUpdPic" method="post" action="../publication/edit_public_upload.php?pid=<?=$publicationId?>&pdid=<?=$pubDetailId?>" enctype="multipart/form-data">
