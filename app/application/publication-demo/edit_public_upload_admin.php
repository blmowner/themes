	...
	if (is_uploaded_file($xpic_file))
	{
		...
		|| $FileType == 'jpg' || $FileType == 'xlsx' || $FileType == 'pptx' || $FileType == 'ppt')
		{
			...
			if (($db->query($sql2)) && @ mysql_affected_rows() == 1) 
			{
				...
				echo "opener.location.href=\"edit_publication_attachment_admin.php?pid=$pid\";";
				echo "document.location.href=\"edit_public_receipt_admin.php?data=$fileContent&status=T&userId=$user_id&fucd=$upload_id\";";
				echo "</script>";
				//header("Location: new_proposal_receipt.php?data=$fileContent&status=T&userId=$user_id&fucd=$upload_id");
			}
			else 
			{
				header("Location: edit_public_receipt_admin.php?status=F&userId=$user_id&fucd=$upload_id");
			}
		}
		else
		{
...
<form name="frmEmpUpdPic" method="post" action="../publication/edit_public_upload_admin.php?pid=<?=$pid?>&amp;al=S" enctype="multipart/form-data">
