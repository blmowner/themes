	...
	if(empty($msg))
	{
		if(empty($pub_id))
		{
			if(preg_match('/^[a-zA-Z ,.]+$/', $author))
			{
				...
				('$publicId', '$issue', '$volume', '$issn', '$title', '$publisher', '$abstract', '$newPublicDate', '$add_publication_type', '$website',
				 '$add_country', '$user_id', '$user_id', '$curdatetime', 'A', '$keyword', 'A', '$authorId')";
				...
				$sqlissn = "SELECT issn_no FROM pg_publication WHERE issn_no = '$issn'";
				...
				if(empty($issnNo))
				{
					...
					(issn_no, `status`, insert_by, insert_date)
					VALUES('$issn', 'A', '$user_id', '$curdatetime1')";
					$dbe->query($sq3);
				}
				...
				$sql_publish="SELECT publisher_name
				FROM ref_publisher
				WHERE status = 'A'
				AND id = '$publisher_id'";
		...
		else if(!empty($pub_id))
		{
			if(preg_match('/^[a-zA-Z ,.]+$/', $author))
			{
				...
				type_pub_id= '$add_publication_type', website = '$website', country_id = '$add_country', keyword = '$keyword', publication_status = 'A'
				WHERE id = '$pub_id'
...
if($_POST['btnsave'] <> "") 
{
	...
	$pub_id = $_REQUEST['pub_id'];
	$author_Id = $_REQUEST['authorID'];
	$pub_uploader = $_REQUEST['pub_uploader'];
	//$author = trim($author);
		...
		else if(!empty($pub_id))
		{
			if(preg_match('/^[a-zA-Z ,.]+$/', $author))
			{
				if($pdate == '')
				{
					...
					$publicationDetail = "UPDATE pg_publication_detail 
					SET issue = '$issue', volume_id = '$volume', issn_no= '$issn', title= '$title', publisher_id = '$publisher', abstract='$abstract',
					...
					modify_date = '$modify_date', author = '$author'
					WHERE id = '$pub_id'
					AND insert_by is not null
					AND insert_date is not null";	 
					...
					type_pub_id= '$add_publication_type', website = '$website', country_id = '$add_country', keyword = '$keyword', publication_status = 'S', 
					modify_by = '$user_id',
					modify_date = '$modify_date', author = '$author' 
					WHERE id = '$pub_id'
					AND insert_by is not null
					...
					$publicationDetail = "UPDATE pg_publication_detail 
					SET modify_by = '$user_id', modify_date = '$modify_date', status = 'I'
					WHERE id = '$pub_id'
					AND insert_by is not null
					AND insert_date is not null";	 
					$dbe->query($publicationDetail);
					...
					VALUES
					('$publicId', '$issue', '$volume', '$issn', '$title', '$publisher', '$abstract', '$newPublicDate', '$add_publication_type', '$website',
					 '$add_country', '$pub_uploader', '$user_id', '$curdatetime', 'A', '$keyword', 'S', '$authorId', '$author')";
	...
	$sql_thesis="SELECT a.id,a.issue, a.issn_no, a.volume_id, a.title, DATE_FORMAT(a.publish_date,'%d-%b-%Y') AS publishDate, a.type_pub_id, a.publication_uploader,
...
function newAttachment(pid) {
    ...
    if (ask) {
		document.location.href = "../publication/edit_publication_attachment_admin.php?pid=" + pid;
...
$(document).ready(function(){
	...
	<table class="idd">
		<tr>
			...
			<td colspan="5" style="background-color: rgba(0, 0, 0, 0.1); "><input type="text" id="pdate" name="pdate" value="<?= $publishDate?>"/>
			<input type="hidden" id="pub_id" name="pub_id" value="<?= $pid?>"/>
			<input type="hidden" id = "pub_uploader" name = "pub_uploader" value="<?=$publication_uploader?>" /></td>
				<?	$jscript .= "\n" . '$( "#pdate" ).datepicker({
												changeMonth: true,
												changeYear: true,
												yearRange: \'-100:+0\',
		...
		<tr>
			...
			<td style="background-color: rgba(0, 0, 0, 0.1); "><input type = "text" id="issn" name="issn" value="<?=$issn_no?>"/></td>
		...
		<tr>
		  ...
		  <td colspan="5" style="background-color: rgba(0, 0, 0, 0.1); ">
			...
			<br>
			<span style="color:#FF0000">Note</span>: Only alphabetical, commas and fullstop/period can be used
			<?
				/*for ($i=0; $i<$inc; $i++)
				{
		...
		<tr>
			...
			<td style="background-color: rgba(0, 0, 0, 0.1); " colspan="5"><input type = "text" id="keyword" name="keyword" value="<?=$keyword?>"/>			</td>
		</tr>
		<tr>
	...
	<table>
		<tr>
          ...
          <input type="button" name="btnBack" value="Back" onClick="javascript:document.location.href='../publication/list_of_publication_admin.php';" /></td>
