...
{					
	if (sizeof($_POST['attachment_checkbox'])>0) {
		...
		while (list ($key,$val) = @each ($_POST['attachment_checkbox'])) 
		{
			...
			$sql1 = "UPDATE file_upload_publication
...
{					
	if (sizeof($_POST['attachment_checkbox'])>0) {
		...
		while (list ($key,$val) = @each ($_POST['attachment_checkbox'])) 
		{
			...
			$sql1 = "DELETE FROM file_upload_publication
			WHERE fu_cd = '$attachment_id'
			AND upload_by = '$user_id'";
...
?>
  ...
  <form id="form1" name="form1" method="post" enctype="multipart/form-data" onsubmit="return saveRec();">
	 ...
	 <fieldset>
		...
		<table>
			<tr>
				<td><input type="submit" name="submit" value="Add Attachment" onclick="javascript:open_win('edit_public_upload.php?pid=<?=$publicationId;?>&pdid=<?=$pubDetailId;?>',480,280,0,0,0,1,0,1,1,0,5,'winupload'); "/></td>
			</tr>
		</table>
		<table>
		<tr>							
		...
		<? if($row_cnt > 10)
		{ ?>
			<div id = "tabledisplay" style="overflow:auto; height:330px;">
		<? }
		else
		{?>
		...
		<? }
		?>		
				<table border="1" cellpadding="3" cellspacing="3" width="75%" id="inputs9" class="thetable">
					<tr>
						<th width="5%" align="center"><label>Tick</label></td>
						<th width="5%" align="center"><label>No</label></td>
						<th width="20%" align="left"><span class="labeling">Document Description <span style="color:#FF0000">*</span></span></th>
						<th width="40%" align="left"><span class="labeling">Document Name</span></th>
						<th width="5%" align="left"><span class="labeling">Download</span></th>
					</tr>
				...
				if ($row_cnt > 0) {	
					...
					{ 
						...
						?><tr>
								<td align="center" width="30"><input type="checkbox" name="attachment_checkbox[]" id="publication_checkbox" value="<?=$tmp_no;?>" /></td>
								...
								<td align="center"><a href="../publication/download.php?fc=<?=$row["fu_cd"];?>" target="_blank" onMouseOver="toolTip('<?=$row["fu_document_filename"];?>', 300)" onMouseOut="toolTip()" align="center">
	...
	<table>
		<tr>
			<td><input type="button" name="btnBack" value="Back" onclick="javascript:document.location.href='../publication/edit_publication.php?pid=<?=$publicationId?>&pdid=<?=$pubDetailId?>';" /></td>		
