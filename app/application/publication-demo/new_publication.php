	...
	if(empty($msg))
	{
		if(empty($pub_id))
		{
			...
			{*/
				...
				`country_id`,`publication_uploader`,`insert_by`,`insert_date`,`status`,`keyword`,`publication_status`, author_id)
				VALUES
				('$publicId', '$issue', '$volume', '$issn', '$title', '$publisher', '$abstract', '$newPublicDate', '$add_publication_type', '$website',
				 '$add_country', '$user_id', '$user_id', '$curdatetime', 'A', '$keyword', 'P', '$authorId')";
		...
		else if(!empty($pub_id))
		{
			...
			{*/
				...
				SET issue = '$issue', volume_id = '$volume', issn_no= '$issn', title= '$title', publisher_id = '$publisher', abstract='$abstract', publish_date= '$newPublicDate',
				type_pub_id= '$add_publication_type', website = '$website', country_id = '$add_country', keyword = '$keyword', publication_status = 'P'
				WHERE id = '$pub_id'
	...
	if(empty($msg))
	{
		if(empty($pub_id))
		{
			...
			{*/
				...
				VALUES
				('$publicId', '$issue', '$volume', '$issn', '$title', '$publisher', '$abstract', '$newPublicDate', '$add_publication_type', '$website',
				 '$add_country', '$user_id', '$user_id', '$curdatetime', 'A', '$keyword', 'S', '$authorId')";
...
<?
	$sql_thesis="SELECT a.id,a.issue, a.issn_no, a.volume_id, a.title, DATE_FORMAT(a.publish_date,'%d-%b-%Y') AS publishDate, a.type_pub_id, 
...
$(document).ready(function(){
	...
	<table class="idd">
		<tr>
			...
			<td colspan="5" style="background-color: rgba(33, 26, 16, 0.3); "><input type="text" id="pdate" name="pdate" value="<?= $publishDate?>"/>
			<input type="hidden" id="pub_id" name="pub_id" value="<?= $pid?>"/>
			
			<input type="text" id="date2" name="date2" value=""/></td>
				<?	$jscript .= "\n" . '$( "#pdate" ).datepicker({
												changeMonth: true,
												changeYear: true,
												yearRange: \'-100:+0\',
		...
		<tr>
			...
			<td style="background-color: rgba(33, 26, 16, 0.3); "><input type = "text" id="issue" name="issue" value="<?=$issue?>"/></td>
			<td style="background-color: rgba(105, 162, 255, 0.7);"> <font color="#FF0000">*</font><b></b>Volume </td>
			<td style="background-color: rgba(33, 26, 16, 0.3); ">
			<select name="volume" id="volume">
				...
				?></select> </td>
			<td style="background-color: rgba(105, 162, 255, 0.7);"> <font color="#FF0000">*</font><b></b>ISSN </td>
			<td style="background-color: rgba(33, 26, 16, 0.3); "><input type = "text" id="issn" name="issn" value="<?=$issn_no?>"/></td>
		</tr>
		<tr>
			<td style="background-color: rgba(105, 162, 255, 0.7);"><font color="#FF0000">*</font><b></b>Title </td>
			<td colspan="5" style="background-color: rgba(33, 26, 16, 0.3); "><textarea name="title" cols="50" rows="2" id="title" ><?=$title?></textarea></td>
		</tr>
		<tr>
		<?
			...
			<td colspan="5" style="background-color: rgba(33, 26, 16, 0.3); ">
		...
		<tr>
			...
			<br>
			<span style="color:#FF0000">*</span>Only alphabetical, commas and fullstop/period can be used
			<?
				/*for ($i=0; $i<$inc; $i++)
				{
		...
		<tr>
			...
			<td colspan="5" style="background-color: rgba(33, 26, 16, 0.3); "><select name="add_publication_type" id="add_publication_type">
				<?
				do {
		...
		<td style="background-color: rgba(105, 162, 255, 0.7);"><font color="#FF0000">*</font><b></b>Country</td>
			<td colspan="5" style="background-color: rgba(33, 26, 16, 0.3); "><select name="add_country" id="add_country">
              <?
				do {
					$countryId = $dbb->f('id');
		...
		<tr>
			...
			<td colspan="5" style="background-color: rgba(33, 26, 16, 0.3); "><button type="button" name="btnAttachment" value="Attachment" onClick="return newAttachment('<?=$pid?>')" >
		...
		<tr>
			...
			<td style="background-color: rgba(33, 26, 16, 0.3); " colspan="5"><input type = "text" id="keyword" name="keyword" value="<?=$keyword?>"/>			</td>
		</tr>
		<tr>
	...
	<table>
		<tr>
		  <td align="right"><!--<input type="submit" id="btnsave" name="btnsave" value = "Save as Draft" />-->
			...
			<span style="color:#FF0000; font-family:Verdana, Arial, Helvetica, sans-serif;font-size:12px;">Note:</span> 
			<span style="font-family:Verdana, Arial, Helvetica, sans-serif;font-size:12px;">Field marks with (</span><span style="color:#FF0000">*</span><span style="font-family:Verdana, Arial, Helvetica, sans-serif;font-size:12px;">) is compulsory.</span></td>
		</tr>
	</table>
	</fieldset>
    </form>
