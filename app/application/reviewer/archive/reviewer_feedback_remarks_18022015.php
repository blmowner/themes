...
///////////////////////////////////////////////////////////////
		...
		DATE_FORMAT(f.extension_date,'%d-%b-%Y') AS extension_date
		FROM pg_proposal a 
		...
		LEFT JOIN new_employee d3 ON (d3.empid = '$userid') 
			...
			<table>
				...
				<tr>
					...
					<td>:</td>
						...
						if ($tmpRespondedByDate->format('d-M-Y') > $currentDate->format('d-M-Y')) {?>
							<td><textarea name="recipientRemarks" id="recipientRemarks" class="ckeditor" cols="50" rows="3"><?=$recipientRemarks;?></textarea></td>
						<?}
						else {?>
							<td><br/><label name="recipientRemarks" id="recipientRemarks" class="ckeditor" cols="50" rows="3"><?=$recipientRemarks;?></label>Note: You are not able to provide further feedback due to the due date has been exceeded.<br/>You may need to request for time extension to Faculty if needed.</td>
							<br/>
						<?}?>
				</tr>
			</table>
