...
// Program Name: assign_reviewer.php
//
// Created by: Zuraimi on 29-Jan-2015
// Modified by: Zuraimi on 29-Jan-2015l
//
//**************************************************************************************
...
if(isset($_POST['btnSearch']) && ($_POST['btnSearch'] <> "")) {
	...
	$sql2 = "SELECT a.pg_thesis_id, a.id, DATE_FORMAT(a.report_date,'%d-%b-%Y') AS theReportDate, a.thesis_title, a.thesis_type, 
			...
			WHERE a.verified_status IN ('INP','REQ')"
			.$tmpSearchThesisId." "
			.$tmpSearchStudent." "."
				...
				<legend><strong>List of Thesis / Proposal</strong></legend>
					<table>
						...
						<tr>
							...
							1. All students which the proposal has no reviewer been assigned and not yet approved by Faculty. </td>
						</tr>
					</table>
					<br/>
					<table>
					...
					<table border="1" style="border-collapse:collapse;" cellpadding="2" cellspacing="1" width="100%" class="thetable">
                      <tr>
						...
						<th width="102"><strong>Reviewer</strong></th>
                      </tr>
                      <?
						if ($row_cnt>0) {
						$no=0;
                      ...
                      <tr>
                        ...
                        <td><a href="../reviewer/assign_reviewer_outline.php?thesisId=<? echo $pgThesisIdArray[$i];?>&proposalId=<? echo $proposalIdArray[$i];?>" name="myPgThesisId[]" value="<?=$pgThesisIdArray[$i]?>" title="Outline of Proposed Case Study by the Student - Read more...">
						...
						if ($verifiedStatusArray[$i] !='APP') {
							if ($reviewerStatusArray[$i] =='A') {?>
								<a href="../reviewer/assign_reviewer_change.php?pid=<?php echo $myProposalId[$no]?>&tid=<? echo $myPgThesisId[$no]?>&mn=<? echo $myStudentMatrixNo[$no]?>">Change <img src="../images/person_reassigned.jpg" width="20" height="19" style="border:0px;" title="Reviewer has been assigned" /></a><br/>
								<br/>
							  <a href="../reviewer/assign_reviewer_view.php?mn=<? echo $myStudentMatrixNo[$no]?>&tid=<? echo $myPgThesisId[$no]?>">View</a>
							<?}
							else {?>
								<a href="../reviewer/assign_reviewer_change.php?pid=<?php echo $myProposalId[$no]?>&tid=<? echo $myPgThesisId[$no]?>&mn=<? echo $myStudentMatrixNo[$no]?>">Assign <img src="../images/person_assigned.jpg" width="20" height="19" style="border:0px;" title="Reviewer is not assigned yet" /></a>
							<?}
						}
						else {?><br/>
							  <a href="../reviewer/assign_reviewer_view.php?mn=<? echo $myStudentMatrixNo[$no]?>&tid=<? echo $myPgThesisId[$no]?>&pid=<? echo $myPgThesisId[$no]?>">View</a>
						<?}?>
