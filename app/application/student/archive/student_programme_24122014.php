...
$sql_personal="SELECT s.student_status,s.matrix_no,s.name,s.ic_passport,s.address_aa,s.address_ab,s.city_a,s.state_a,s.postcode_a,s.country_a,s.handphone,s.house,s.office,s.email,
					s.address_bb,s.address_ba,s.city_b,s.state_b,s.postcode_b,s.country_b,s.xgender,sp.intake_no,pa.approved_by,pa.approved_date,pa.status,pa.thesis_title,
					po.code
				FROM student s
				LEFT JOIN student_program sp ON (sp.matrix_no=s.matrix_no)
				LEFT JOIN pg_assessment pa ON (pa.pg_thesis_id=s.matrix_no)
				LEFT JOIN pg_thesis pt ON (pt.id=pa.pg_thesis_id)
		...
		<div id="tabs-1">
			<fieldset>
			  ...
			  <table border="0" width="100%">
				...
				<tr>
				  <td>Programme</td>
				  <td colspan="3"><input type="text" name="program_code" size="30" id="program" value="<?=$program?>" disabled="disabled"/></td>
				</tr>
				<tr>
				  <td>Intake</td>
			...
			<fieldset>
				...
				<table border="1" width="100%">
					<tr>
					 ...
					 <?php	
					  $sql="select * from asess_grade where matrix_no='$user_id'";			
					//$sql="SELECT * FROM pg_supervisor WHERE pg_thesis_id='$pg_thesis_id' ORDER BY type,supervisor_id  ";					
					$result = $db_klas2->query($sql); //echo $sql;
					$varRecCount=0;					
					while($row = mysql_fetch_array($result)) 
