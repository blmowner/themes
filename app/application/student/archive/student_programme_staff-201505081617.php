 ...
 <form id="form1" name="form1" method="post" action="student_programme_staff.php?act=<?=$act?>"; >
  <input type="hidden" name="matrix_no" id="matrix_no" value="<?=$matrix_no; ?>">
	...
	<legend><strong>LIST OF STUDENT</strong></legend>
			<table>
				<tr>
					<td><label>Please click button below to search your supervisee:-</label></td>
				</tr>
			</table>
			<br/>
			<table>	
				<tr>
					<td></td>
					<td>					  
				  	  <input type="submit" name="btnSearchSupervisee" id="btnSearchSupervisee" value="List My Supervisee"></td>
				</tr>
			</table>			
			<br />
			<table>
			...
			<? if($act=="search") 
				...
				$row_cnt2 = mysql_num_rows($result_sql);
					...
					{
						do  {
							$matrix_no=$dba->f("pg_student_matrix_no");
							...
							LEFT JOIN program p ON (p.programid = sp.program_code) 
							WHERE p.stage IN (4,5) 
							AND (s.matrix_no LIKE '%$matrix_no%' OR s.name LIKE '%$matrix_no%') 
							AND s.student_status = 'ACTIVE' 
							ORDER BY s.matrix_no
							LIMIT $startpoint,$perpage";
							...
							<tr>
								...
								<td align="center"><label name="stud_stat[]"><?=$stud_stat;?></label></td>
								<td align="center"><label name="entry[]"><?=$entry;?></label></td>
								<td align="center"><input type="button" value="View" onClick="javascript:document.location.href='student_programme_detail_staff.php?matrix_no=<?=$matrix_no?>&act=searchSupervisee&search=<?=$search?>';" name="B3"></td>
							</tr>
							<?
							$j++;
