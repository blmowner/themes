			...
			<table width="98%">	
				...
				<td width="88%" class="tbmain">
					  <input name="search" value="<?=$search?>" size="30">
				  	  <input type="submit" name="find" value="Find" onClick="return check(document.f1)"><span style="color:#FF0000"> Note:</span> If no entry is provided, it will search all.</td>
				</tr>
			</table>
			...
			<? if($act=="search") 
				{
					$i=$startpoint;
						...
						$sql="select distinct pt.id AS thesis_id, pt.student_matrix_no, pg.thesis_title 
								...
								LEFT JOIN pg_proposal pg ON (pg.pg_thesis_id=pt.id)
								where ((pt.id LIKE '%$search%')
								OR (pg.thesis_title LIKE '%$search%')) AND pg.archived_status IS NULL
								order by pt.id
					...
					//{
						...
						do  {
							...
							$sqlStudent="select distinct s.matrix_no, s.name, s.rel_mat_no, s.ic_passport,s.passport_no,
									s.student_status,lss.title,lss.id as stud_stat,color, s.entry, le.description, 
							...
							//$matrix_no=$dbc->f("student_matrix_no");
							//$intake_no=$row_student["intake_no"];
							//$name=$row_student["name"];
							//$stud_stat=$row_student["stud_stat"];
							//$entry=$row_student["entry"];
			...
			<?		
				...
				{	
					/*$count_total_result ="select count(*) as total
						...
						//display proposal: archived status is NULL
						$count_total_result = "select count(*) as total
										from pg_thesis pt 
								LEFT JOIN pg_proposal pg ON (pg.pg_thesis_id=pt.id)
								where ((pt.id LIKE '%$search%')
