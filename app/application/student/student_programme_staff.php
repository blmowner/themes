...
if(isset($_POST['btnSearchSupervisee']) && ($_POST['btnSearchSupervisee'] <> "")) {
...
<body>
 ...
 <form id="form1" name="form1" method="post" action="student_programme_staff.php?act=<?=$act?>"; >
  <input type="hidden" name="matrix_no" id="matrix_no" value="<?=$matrix_no; ?>">
	...
	<legend><strong>LIST OF STUDENT</strong></legend>
			...
			<table width="98%">	
				<tr>
					<td width="10%" class="tbmenu"><strong>Student Name</strong></td>
					<td width="80%" class="tbmain">
					  <input name="searchSupervisee" id="searchSupervisee" value="<?=$searchSupervisee?>" size="30">
				  	  <input type="submit" name="btnSearchSupervisee" id="btnSearchSupervisee" value="Search My Supervisee"></input></td>
				</tr>
			</table>
			<table>
				<tr>
				...
				<tr>
					<td>1. If no entry is provided, it will search all your supervisee.</td>
				</tr>
				<tr>
					<td>2. The student will be listed after you have accepted them as your supervisee.</td>
				</tr>
			</table>
			<br/>
			<table>
			...
			<? if($act=="search") 
					...
					if ($row_cnt1 > 0)  
					{
						do  {
							...
							<tr class="<?=$color?>">
								...
								<td align="center"><input type="button" value="View" onClick="javascript:document.location.href='student_programme_detail_staff.php?matrix_no=<?=$matrix_no?>&act=search&search=<?=$search?>';" name="B3"></td>
							</tr>
							<?
							$i++;
				...
				for ($i=0; $i<$no1; $i++){
					...
					/*AND s.student_status = 'ACTIVE' */
					ORDER BY s.matrix_no";
					if (substr($matrixNoArray[$i],0,2) != '07') { 
						$dbConnStudent= $dbc; 
					} 
					else { 
						$dbConnStudent=$dbc1; 
					...
					$result_sql2 = $dbConnStudent->query($sql2); 
					$dbConnStudent->next_record(); 
					$row_cnt_sql2 = mysql_num_rows($result_sql2);
					if ($row_cnt_sql2 > 0) {
						$studentNameArray[$no2] = $dbConnStudent->f("name");
						$matrixNoArray[$no2] = $matrixNoArray[$i];
						$icPasportArray[$no2] = $dbConnStudent->f("ic_passport");
						$passportNoArray[$no2] = $dbConnStudent->f("passport_no");
						$intakeNoArray[$no2] = $dbConnStudent->f("intake_no");
						$studStatArray[$no2] = $dbConnStudent->f("stud_stat");
						$entryArray[$no2] = $dbConnStudent->f("entry");
						$no2++;
					}
					...
					if ($row_cnt2 > 0)  
					{
						for ($j=0; $j<$no2; $j++){	
							...
							<tr>
								...
								<td align="center"><input type="button" value="View" onClick="javascript:document.location.href='student_programme_detail_staff.php?matrix_no=<?=$matrixNoArray[$j]?>&act=searchSupervisee&search=<?=$searchSupervisee?>';" name="B3"></td>
							</tr>
							<?
							//$j++;
						}	
