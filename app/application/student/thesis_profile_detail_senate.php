...
s.house,s.office,s.email,s.address_bb,s.address_ba,s.city_b,s.state_b,s.postcode_b,s.country_b,s.xgender,sp.intake_no,po.code,po.program_e,sp.program_code,s.skype_id, sp.manage_by_whom
FROM student s
...
$dbConnStudent->query($sql_personal);
$row_personal = $dbConnStudent->fetchArray(); //echo $sql_personal;
...
//untuk view student profile details
$sql_thesis="SELECT pa.verified_by,pa.verified_date,
pa.status,pa.thesis_title,pa.id,pt.id AS thesis_id, pt.ref_thesis_status_id_proposal, 
pt.ref_thesis_status_id_defense, pt.ref_thesis_status_id_work, pt.ref_thesis_status_id_evaluation, pt.ref_thesis_status_id_final, pt.ref_thesis_status_id_senate, DATE_FORMAT(ppa.endorsed_date,'%d-%b-%Y') AS endorsed_date,
rts1.description as proposal_desc, rts2.description as defense_desc, rts3.description as work_desc, rts4.description as evaluation_desc, rts5.description as final_desc, rts6.description as senate_desc 
		FROM pg_thesis pt  
		...
		WHERE pt.student_matrix_no = '$matrix_no'
		AND pt.archived_status is null
		ORDER BY pa.id DESC";
$db_klas2->query($sql_thesis);
$row_thesis=$db_klas2->fetchArray();
...
?>
  ...
  <input type="hidden" name="matrix_no" id="matrix_no" value="<?php echo $matrix_no; ?>">
 	...
 	<div id="tab1" style="width:950px">
		...
		<div id="tabs-1">
			<fieldset style="width:900px">
			  ...
			  <table border="0" width="100%">
				...
				<td width="20%" rowspan="20" align="center">&nbsp;<img src="getImage.php?userId=<?=$matrix_no?>" width="100" height="130" /><br> <?php?></td>	
				</tr>
		...
		<div id="tabs-2">
			...
			<legend><strong>LIST OF SUPERVISOR/CO-SUPERVISOR</strong></legend>	
				...
				WHERE a.pg_student_matrix_no='$matrix_no'
					...
					{ 	
						...
						?>
							<tr>
								...
								<td rowspan="2" align="lefcentert"><img src="getImageStaff.php?staffId=<?=$row["pg_employee_empid"];?>" width="40" height="40" /></td>
		...
		<div id="tabs-3">
			<fieldset>
				...
				<table border="1" style="border-collapse:collapse;" cellpadding="2" cellspacing="1" width="100%" class="thetable">	
					...
					$result = $dbConnStudent->query($sql); //echo $sql;
					$varRecCount=0;					
					if (mysql_num_rows($result)>0)  {
						while($row = mysql_fetch_array($result)) 
						{ 
		...
		<div id="tabs-4">			
				<iframe id="iframe" src="../thesis/proposal_history_staff_detail.php?matrix_no=<?=$matrix_no;?>" frameborder="0" style="height:500px;width:100%"></iframe>
				<?php //include("../thesis/proposal_history_staff_detail.php"); ?>
		</div>
	</div>
	<table>
			<tr>
				<td><input type="button" name="btnBack" value="Back to Student List" onclick="javascript:document.location.href='../student/thesis_profile_senate.php?act=<?=$act?>&search=<?=$search?>';" /></td>		
			</tr>
	</table>
 </form>
</body>
