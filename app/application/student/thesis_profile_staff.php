 ...
 <form id="form1" name="form1" method="post" action="thesis_profile_staff.php?act=search"; >
  <input type="hidden" name="thesis_id" id="thesis_id" value="<?=$thesis_id; ?>">
	...
	<legend><strong>LIST OF STUDENT</strong></legend>
			<table>
				<tr>
					<td><label>Please enter search criteria below to search your supervisee:-</label></td>
				</tr>
			</table>
			<br/>
			<table width="98%">	
				<tr>
					<td width="20%" class="tbmenu"><strong>[Thesis/Project ID]/[Title] </strong></td>
					<td width="80%" class="tbmain">
					  <input name="search" value="<?=$search?>" size="50"></input>
				  	  <input type="submit" name="find" value="Search My Supervisee" onClick="return check(document.f1)"></input></td>
				</tr>
			</table>
			<table>
				<tr>
			...
			<? if($act=="search") 
				{
					...
					$sql="select distinct pt.id AS thesis_id, pt.student_matrix_no, pg.thesis_title 
							...
							AND pgs.acceptance_status = 'ACC'
							AND pg.verified_status in ('APP','AWC')
							AND pg.status in ('APP','APC')
					...
					if (mysql_num_rows($result)>0)  {
						...
						for ($i=0; $i<$inc; $i++) 
						{
							...
							<tr class="<?=$color?>">
								...
								<td align="center"><input type="button" value="View" onClick="javascript:document.location.href='thesis_profile_detail_staff.php?matrix_no=<?=$matrix_no[$i]?>&search=<?=$search?>&act=search';" name="B3"></td>
							</tr>
							<?
						}
					}
