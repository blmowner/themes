...
{	
	...
	if (sizeof($_POST['myCheckbox'])>0) {
		...
		{		
			...
			$sqldate = "SELECT report_date FROM pg_proposal 
			WHERE pg_thesis_id = '$thesisid[$i]'
					...
					<?
						...
						?>
							<tr>
							    ...
							    <input type="hidden" name="hidthesisid[]" value = "<?=$thesisId;?>" /></td>	
								<?$myThesisId[$no]=$thesisIdArray[$i];?>
								
							  <td><label name="thesisTitle[]" cols="30" rows="3" id="thesisTitle" ></label><?=$thesisTitleArray[$i]?>
							    <input type="hidden" name="hidtitle[]" value = "<?=$thesisTitle?>" /></td>														
