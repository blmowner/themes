	...
	<table>
		...
		<table border="1" style="border-collapse:collapse;" cellpadding="2" cellspacing="1" width="100%" class="thetable">
				...
				<tr>
					...
					?>
					<td><label name="myEmployeeName1[]" size="50" id="employeeName1"></label><?=$employeeName1;?><br/>(<?=$employeeId1;?>)<br/>
					<em><?=$skypeId1?></em>
					...
					<?}
						...
						<a href="../supervisor/assign_supervisor_biodata.php?tid=<?=$thesisId;?>&sid=<?=$employeeId1;?>&pid=<?=$proposalId;?>&ename=<?=$employeeName1;?>&mn=<?=$matrixNo;?>&sname=<?=$sname;?>&id=<?=$supervisorId1;?>"><br/><img src="../images/red_edit.jpg" width="20" height="19" style="border:0px;" title="Supervisor is not assigned yet" />View Brief Biodata </a>
					<?}?>
					</td>	
					<td>
					<?
			...
			<table>
				<tr>
					<td><?if ($row_cnt_sv_chk==0) {							
							?><label><strong><span style="color:#FF0000">Note:</span></strong> Please assign Supervisor/Co-Supervisor to supervise this student.</label><br/><?
					}?></td>
				</tr>
			</table>
			<table>
		...
		if ($row_cnt_sv > 3)
		{?>
			<div id = "tabledisplay" style="overflow:auto; height:270px;">
		<? }
		else 
		{ ?>
			<div id = "tabledisplay">
		...
		<table border="1" style="border-collapse:collapse;" cellpadding="2" cellspacing="1" width="100%" class="thetable">
			...
			?>
			<tr>
				<td align="center"><input name="supervisorBox[]" type="checkbox" value="<?=$no;?>" /></td>
				...
				?>
				<td><label name="myEmployeeName[]" size="50" id="employeeName" ></label><?=$employeeName;?><br/>
		...
		<table>
			<tr>
				<td><label><strong> Notes:<br/></strong> 1. Please select the Supervisor from the list before click ASSIGN button.<br/>2. <span style="color:#FF0000">Role Status</span> - If more than 1 Supervisor has been assigned, ONLY one can be a primary supervisor.</label></td>
			</tr>
			<tr>
