...
$xpic_desc = clean($xpic_desc, 50);
	...
	if (is_uploaded_file($xpic_file))
	{
		...
		$upload_id = runnum('fu_cd','file_upload_biodata');
		$fileContents = addSlashes(fread(fopen($xpic_file, "r"), filesize($xpic_file)));
	
		//var_dump($_GET);
		
		/*$sql2="INSERT INTO file_upload_proposal 
			(fu_cd, fu_document_filename,fu_document_filetype,  fu_document_filedata, insert_date, insert_by, modify_date,
			modify_by,pg_proposal_id, attachment_level)	
			VALUES ('$fu_cd','$mimeName','$xpic_file_type','$fileContents',now(),'$userid',now(),'$userid','".$_GET['pid']."','".$_GET['al']."')";*/

              $FileName=$_FILES['xpic_file']['name'];
		
		/*$sql2="INSERT INTO file_upload_proposal 
			(fu_cd, fu_document_filename,fu_document_filedesc,fu_document_filetype,  fu_document_filedata, insert_date, insert_by, modify_date,
			modify_by,pg_proposal_id, attachment_level)	
			VALUES ('$fu_cd','$FileName','$fileDesc','$xpic_file_type','$fileContents',now(),'$userid',now(),'$userid','".$_GET['pid']."','".$_GET['al']."')";
			*/
			$sql2 = "INSERT INTO file_upload_biodata (
										fu_cd,
										employee_id,
		...
		{
			header("Location: supervisor_biodata_receipt.php?data=$fileContent&status=T&userId=$user_id&uid=$upload_id");
		}
		else 
		{
			header("Location: supervisor_biodata_receipt.php?status=F&userId=$user_id&uid=$upload_id");
		}
	}
?>
...
<body>
<form name="frmEmpUpdPic" method="post" action="supervisor_biodata_upload.php" enctype="multipart/form-data">
