		...
		<form id="form1" name="form1" method="post" enctype="multipart/form-data">				
				...
				<br/>
					...
					<table border="1" style="border-collapse:collapse;" cellpadding="2" cellspacing="1" width="100%" class="thetable">				
						...
						$verifiedDesc=array();
						$verifiedRemarks=array();	
						$markedStatus=array();
						$verifiedStatus=array();
						$cancelApprovedRemarks=array();			
						$cancelRequestedDate=array();
						$cancelApprovedDate=array();
						...
						while($db->next_record()) {						
							...
							$proposalId[$i]=$db->f('id');
							$reportDate[$i]=$db->f('theReportDate');
							$thesisTitle[$i]=$db->f('thesis_title');
							$supervisorStatus[$i]=$db->f('supervisor_status');	
							$verifiedDesc[$i]=$db->f('verified_desc');	
							$verifiedRemarks[$i]=$db->f('verified_remarks');	
							$markedStatus[$i]=$db->f('marked_status');
							$verifiedStatus[$i]=$db->f('verified_status');
							$cancelApprovedRemarks[$i]=$db->f('cancel_approved_remarks');			
							$cancelRequestedDate[$i]=$db->f('cancel_requested_date');
							$cancelApprovedDate[$i]=$db->f('cancel_approved_date');
							$inc++;
							$i++;	
							$no++;
						}
					for ($i=0; $i<$inc; $i++) 
					{
						...
						?>
							...
							<tr>
								...
								<? if ($verifiedStatus[$i]=='CAN'){
									?><td align="center"><label><input name="myApprovalBox[]" type="checkbox" value="<?=$$i;?>" disabled="disabled"/></label></td><?
								}
								else {
									?><td align="center"><label><input name="myApprovalBox[]" type="checkbox" value="<?=$i;?>"/></label></td><?
								}
								?>
								
								
								<input type="hidden" name="myProposalId[]" size="12" id="proposalId" value="<?=$proposalId[$i];?>"/>
								<? $myProposalId[$i]=$proposalId[$i]; ?>
								
								<td align="center"><?=$myNo++;?>.</td>	

							  <td><label name="cancelRequestedDate" size="15" id="cancelRequestedDate" ></label><?=$cancelRequestedDate[$i];?></td>
							  <td><label name="cancelApprovedDate" size="15" id="cancelApprovedDate" ></label><?=$cancelApprovedDate[$i];?></td>
							  
							  <td width="103"><label name="myVerifiedDesc[]" cols="45" id="verifiedDesc"><?=$verifiedDesc[$i]?></label></td>
								
								
								<td><a href="cancel_proposal_outline.php?thesisId=<? echo $pgThesisId[$i];?>&proposalId=<? echo $proposalId[$i];?>" name="myPgThesisId[]" value="<?=$pgThesisId[$i]?>" title="Outline of Proposed Case Study by the Student - Read more..."><?=$pgThesisId[$i];?></a><br/>
								...
								<?
									...
									?>
								<td><label name="myStudentName[]" size="30" id="studentName" ></label><?=$studentName;?><br/>(<?=$studentMatrixNo[$i];?>)</td>
								<?$myStudentMatrixNo[$no]=$studentMatrixNo;?>
								
								<?php
									$sqlUpload="SELECT * FROM file_upload_proposal 
									WHERE pg_proposal_id='$proposalId[$i]' 
