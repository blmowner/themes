...
<legend><strong>View Approved Thesis- Outline of Research/Case Study</strong></legend>	
	...
	<table border="1" cellpadding="2" cellspacing="1" width="99%" id="inputs10" class="thetable">
	 <tr>
		 ...
		 <th width="29%">External Lecturer Name</th>
		 <th width="41%">Notes</th>				 
	 </tr>
	 <?php
		$sqlMeeting="SELECT pmd.id,pmd.lecturer_id, pmd.external_lecturer, DATE_FORMAT(pmd.meeting_sdate,'%d/%m/%Y') as date,
		...
		if ($row_cnt > 0 ) {
			do {			
				...
				?><tr>
						<td align="center" width="116"><label name="date[]" ></label><?=$db_klas2->f('date');?></td>
						<td align="center" width="124"><label name="meeting_time[]"></label><?=$db_klas2->f('time');?></td>
						<td align="left" width="185"><label name="lecturer_name[]" ></label><?=$lecturerName;?></td>
						<td align="left" width="185"><label name="lecturer_name[]" ></label><?=$db_klas2->f('external_lecturer');?></td>
						<td align="left" width="340"><label name="remarks[]" size="50" ></label><?=$db_klas2->f('remark');?></td>		
				</tr>
