	...
	$proposalApprovalId = "A".runnum('id','pg_proposal_approval');	
		...
		if (sizeof($_POST['myApprovalBox'])>0) {
			...
			while (list ($key,$val) = @each ($myApprovalBox)) {
				...
				$sql6_1 = "SELECT pp.id, IFNULL(pp.report_date,'0000-00-00 00:00:00') as report_date, pp.thesis_title, 
				pp.thesis_type, pp.introduction, pp.objective, pp.description, pp.discussion_status,
				...
				if(is_array($myApprovalBox))
				{
					...
					foreach($superid as $superid)
					{
							...
							$studname = "SELECT name FROM student WHERE matrix_no='$insertBy'";
							...
							elseif($verified == 'APC')
							{
								...
								include("../../../app/application/email/cubaemail.php");
							}
							elseif($verified == 'DIS')
							{
								$vstatus = "Disapproved";
...
else 
{
	$sql2 = "SELECT a.pg_thesis_id , a.id, DATE_FORMAT(a.report_date,'%d-%b-%Y') AS theReportDate, a.thesis_title, a.thesis_type, 
		...
		if(isset($_POST['btntest']) && ($_POST['btntest'] <> "")) 
{
	...
	$myProposalId=$_SESSION['myProposalId'];
				...
				foreach($check as $s)
				{
					...
					/*SELECT SUPERVISOR
					$sqlsuper = "SELECT * FROM pg_supervisor
					WHERE pg_student_matrix_no = '$insertBy'";
					$resultsuper = $db->query($sqlsuper);		
					$db->next_record();
					$superid = $dbg->f('pg_employee_empid');
					
					$sqlname = "SELECT name FROM new_employee 
					WHERE empid = '$superid'";
					$resultname = $db->query($sqlname);		
					$db->next_record();
					$supername = $dbg->f('name');
	...
	<body>  
		...
		?>	
						...
						<?	$sqlSupervisor="SELECT ps.id, ps.ref_supervisor_type_id, ps.pg_employee_empid, rst.description  
							...
							if ($row_cnt>0) {
								...
								{ 
									...
									?>
									<?=$no1?>) <?=$row_personal["name"];?> (<?=$employeeId;?>)
									<input type="hidden" name="hid" value="<?=$employeeId;?>" />
									<br/>								
									<? $no1++;
								} ?>
			...
			<table>
				...
				<td><input type="submit" name="btntest" value="Test" /></td>
				</tr>
			</table>
			<br/>
		<?}
