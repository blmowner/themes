...
<script language="JavaScript" src="../../../lib/js/windowopen.js" type="text/javascript"></script>
		...
		while (list ($key,$val) = @each ($myApprovalBox)) 
		{
			...
			if ($row_cnt_supervisor > 0) {
				...
				$verified = $_REQUEST['myVerifiedStatus'];
					...
					$selectsenname = "SELECT name,email from new_employee 
					WHERE `empid` = '$selectidsenate'";
					$resultselsenname = $dbc->query($selectsenname);
					$dbc->next_record();
					$selectname =$dbc->f('name');
					$senateemail =$dbc->f('email');
					
					$studname = "SELECT name,email FROM student WHERE matrix_no='$insertBy'";
					...
					if($verified == 'APP')
					{
						...
						FROM file_upload_proposal WHERE student_matrix_no = '$insertBy' 
...
<table>
  <tr>
    <td>The confirmation of thesis proposal for student (
        <?=$myStudentMatrixNo[$val]?>
      ) is put on hold due to no supervisor is assigned .</td>
  </tr>
</table>
</fieldset>
<?
...
<form id="form1" name="form1" method="post" enctype="multipart/form-data">
  ...
  <table>
    ...
    <tr>
      ...
      <td><input type="submit" name="btnSearchByName" value="Search by Name Only" />
  ...
  <table border="1" style="border-collapse:collapse;" cellpadding="2" cellspacing="1" width="100%">
    ...
    <tr>
      ...
      <td align="center"><label>
        <input name="myApprovalBox[]" type="checkbox" value="<?=$no;?>" disabled="disabled"/>
      </label></td>
      <?
								}
								else {
									?>
      <td align="center"><label>
        <input name="myApprovalBox[]" type="checkbox" value="<?=$no;?>"/>
      </label></td>
      <?
								}
								?>
      ...
      <td align="center"><?=$myNo++;?>
        ...
        <?
          ...
          <?}?></td>
      <td width="22"><label name="myVerifiedDesc[]" cols="45" id="verifiedDesc">
        <?=$verifiedDescArray[$i]?>
      </label></td>
      <td><label name="reportDate[]" cols="45" id="reportDate">
        <?=$reportDateArray[$i]?>
      </label></td>
      <td><a href="proposal_outline.php?thesisId=<? echo $pgThesisIdArray[$i];?>&amp;proposalId=<? echo $proposalIdArray[$i];?>" name="myPgThesisId[]" title="Outline of Proposed Case Study by the Student - Read more..." id="myPgThesisId[]" value="<?=$pgThesisIdArray[$i]?>">
        <?=$pgThesisIdArray[$i];?>
        <br/>
        ...
        <img src="../images/red_edit.jpg" width="20" height="19" style="border:0px;" title="Faculty Remark is not yet provided">Enter remarks</a>
          ...
          <br/>
        <img src="../images/mark.jpg" width="20" height="19" style="border:0px;" title="Thesis is marked for review">Review in progress</a>
          <?}?></td>
      ...
      <td align="left"><?
										...
										{ 
											?>
          <a href="download.php?fc=<?=$row["fu_cd"];?>&amp;al=S" title="File Description: <?=$row["fu_document_filedesc"];?>">Attachment
            <?=$attachmentNo1++;?>
            : <br/>
      ...
      <td align="left"><?
										...
										{ 
											?>
          <a href="download.php?fc=<?=$row["fu_cd"];?>&amp;al=F" title="File Description: <?=$row["fu_document_filedesc"];?>">Attachment
            <?=$attachmentNo2++;?>
            : <br/>
            <img src="../images/download.png" width="20" height="19" style="border:0px;" title="Download <?=$row["fu_document_filename"];?>" /></a> <a href="delete_attachment_confirm.php?fc=<?=$row["fu_cd"];?>&amp;al=F" ><img src="../images/delete_on.gif" width="20" height="19" style="border:0px;" title="Delete <?=$row["fu_document_filename"];?>" /></a> <a href="edit_attachment_confirm.php?fc=<?=$row["fu_cd"];?>&amp;al=F" ><img src="../images/red_edit.jpg" width="20" height="19" style="border:0px;" title="Edit file description" /></a><br/>
        <br/>
          <?}
										?>
        <br/>
      ...
      <?
								...
								?>
      <td width="84"><?$_SESSION['studentMatrixNo']=$studentMatrixNo;
          ...
          <br/>
        <a href="../supervisor/view_supervisor.php?mn=<? echo $studentMatrixNoArray[$i]?>&amp;tid=<? echo $pgThesisIdArray[$i]?>">View</a>
          <?}
								else {?>
      ...
      <td width="84"><?
          ...
          <br/>
        <a href="../reviewer/view_reviewer.php?mn=<? echo $studentMatrixNoArray[$i]?>&amp;tid=<? echo $pgThesisIdArray[$i]?>">View</a>
          <?}
								else {?>
  ...
  <table>
    <tr>
      ...
      <td>:</td>
      <td><input name="myVerifiedStatus" type="radio" value="APP" checked="checked"/>
        Approved
        <input name="myVerifiedStatus" type="radio" value="AWC"/>
        Approved with Changes
        <input type="radio" name="myVerifiedStatus" value="REQ"/>
        Request Changes
        <input type="radio" name="myVerifiedStatus" value="DIS"/>
        Disapproved </td>
    </tr>
  </table>
  </fieldset>
  ...
  <table>
    <tr>
      ...
      <td><input type="submit" name="btnSubmit" value="Submit" onclick="return respConfirm()" />
          <span style="color:#FF0000"> Note:</span> Ensure the proposal above has been selected before click Submit.</td>
    </tr>
  </table>
  <?
				}
  ...
  <table>
    <tr>
      <td><p>There is no proposal to show for verification and supervisor invitation.</p></td>
    </tr>
  </table>
  </fieldset>
  <table>
