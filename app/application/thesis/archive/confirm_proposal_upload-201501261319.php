...
$xpic_desc = clean($xpic_desc, 50);
	...
	if (is_uploaded_file($xpic_file))
	{
		...
		$sql2="INSERT INTO file_upload_proposal 
			(fu_cd, fu_document_filename,fu_document_filetype,  fu_document_filedata, insert_date, insert_by, modify_date,
			modify_by,pg_proposal_id, attachment_level)	
			VALUES ('$fu_cd','$FileName','$xpic_file_type','$fileContents',now(),'$userid',now(),'$userid','".$_GET['pid']."','".$_GET['al']."')";
			
		if (($db->query($sql2)) && @ mysql_affected_rows() == 1) 
		{
			header("Location: confirm_proposal_receipt.php?data=$fileContent&status=T&userId=$userid&ppid=$pid&al=F");
		}
		else 
		{
			header("Location: confirm_proposal_receipt.php?status=F&userId=$userid&ppid=$pid&al=F");
		}
	}
?>
...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
	...
	<td>
		<table border="1" cellpadding="0" cellspacing="0" bordercolorlight="#cccccc" bordercolordark="#eeeeee" width="100%">
			<tr>
				<td width="100%" colspan="8" valign="top" height="17">&nbsp;Upload Attachment</td>
			</tr>
			<tr>
				<td width="25%" ><p align="left"><font size="1">&nbsp;Attachment File:</font></td>
				<td width="75%" ><p align="left">&nbsp;<input type="file" name="xpic_file" size="25"></td>
			</tr>
			<tr>
				<td width="100%" colspan="8" align="middle">
				<input type="button" value="Insert Attachment" onClick="validate();" name="B2">
