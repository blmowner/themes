...
if(isset($_POST['btnSearch']) && ($_POST['btnSearch'] <> "")) {
	...
	{
		$searchApprovedStatus="APP','APC','DIS";
	}
	...
	echo $sql2 = "SELECT a.pg_thesis_id , a.id, DATE_FORMAT(a.report_date,'%d-%b-%Y') AS theReportDate, a.thesis_title, a.thesis_type, 
		...
		a.verified_date, a.verified_remarks, a.verified_status,a.status as endorsedStatus, a.discussion_status, a.proposal_remarks, 
		c.description AS endorsedDesc, d.student_matrix_no, e.name,c.description AS facultyStatus,
		a.endorsed_remarks
		FROM pg_proposal a
		...
		AND (DATE_FORMAT(a.endorsed_date,'%d-%b-%Y') = '$searchRequestDate'
		OR a.endorsed_date is null)		
...
} else 
{
	echo $sql2 = "SELECT a.pg_thesis_id , a.id, DATE_FORMAT(a.report_date,'%d-%b-%Y') AS theReportDate, a.thesis_title, a.thesis_type, 
		...
		LEFT JOIN pg_thesis d ON (d.id = a.pg_thesis_id) 
		LEFT JOIN student e ON (e.matrix_no = d.student_matrix_no) 
		WHERE a.verified_status in ('APP','APC','DIS') 
		AND (DATE_FORMAT(a.endorsed_date,'%Y-%m-%d') = CURDATE()
		OR a.endorsed_date is null)
...
if(isset($_POST['btnSubmit']) && ($_POST['btnSubmit'] <> "")) {
 ...
 $sql2 = "SELECT a.id, a.description 
 			FROM ref_proposal_status a 
			WHERE a.usage_id = 'PAP' AND a.status = 'A' ";		
	...
	<body>  
		...
		<?  
			...
			<tr>
				...
				<td>Cancellation Approved Date</td>
				<td>:</td>
			...
			<table border="1" style="border-collapse:collapse;" cellpadding="2" cellspacing="1">
				...
				?>
					<tr>
						...
						<td><label name="myFacultyDesc[]" id="facultyStatus" ></label><?=$facultyStatus; ?></td>
