...
{	
	...
	$area = "INSERT INTO pg_proposal_area(id,job_id_area,job_id2_area,job_id3_area,job_id4_area,job_id5_area,job_id6_area,insert_date,insert_by,modified_date,modified_by)
			VALUES('".$proposal_id."','".$_POST['jobs_area']."','".$_POST['jobs2_area']."','".$_POST['jobs3_area']."','".$_POST['jobs4_area']."','".$_POST['jobs5_area']."',
			'".$_POST['jobs6_area']."',now(),'".$_SESSION['user_id']."',now(),'".$_SESSION['user_id']."')";
	...
	<table>
     <tr>
      <td><p>Have you discussed about your research/case to any lecturer of MSU?  </p>       </td>
	   ...
	   <br />
	   <fieldset style="width:800px">
	   <legend><strong>Areas of Priority</strong></legend>	
	  
	  	<table  align="center">
		<?php /*?><tr>
			<td width="70" ></td>
			<th width="135"><label class="labeling">Area of Priority</label></td>
			<td width="25">&nbsp;</td>
			<th >&nbsp;</td>
		  	<th width="137"><label class="labeling">Area of Priority</label></td>
   		</tr><?php */?>
		<tr>
			<td width="70" nowrap><font color="#FF0000">*</font><b>Area 1</b></td>
			<td width="134"><span id="span_area" style="display:">
			  <select name="jobs_area" id="jobs_area"><option value="">--Please Select--</option>
        <?php
			$sqlGetJobArea="SELECT JobArea,area,substr(JobArea,3,3) header FROM job_list_category WHERE selection is not NULL order by selection asc  ";
			$rsGetJobArea = $db->query($sqlGetJobArea);
			
			while($rowGetJobArea = mysql_fetch_array($rsGetJobArea)) 
			{
				if($rowGetJobArea["header"]=="000")
				{
				...
				echo "<option style=\"$style\" value=\"".$rowGetJobArea["JobArea"]."\" ".($jobs_area==$rowGetJobArea["JobArea"]?"selected":"").">".$indent.$rowGetJobArea["area"]."</option>";
			}
		?></select></span></td>
        ...
        <?php
			...
			while($rowGetJobArea = mysql_fetch_array($rsGetJobArea)) 
			{
				...
				echo "<option style=\"$style\" value=\"".$rowGetJobArea["JobArea"]."\" ".($jobs4_area==$rowGetJobArea["JobArea"]?"selected":"").">".$indent.$rowGetJobArea["area"]."</option>";
			}
		?></select></span></td>
        ...
        <?php
			...
			while($rowGetJobArea = mysql_fetch_array($rsGetJobArea)) 
			{
				...
				echo "<option style=\"$style\" value=\"".$rowGetJobArea["JobArea"]."\" ".($jobs2_area==$rowGetJobArea["JobArea"]?"selected":"").">".$indent.$rowGetJobArea["area"]."</option>";
			}
		?></select></span></td>
        ...
        <?php
			...
			while($rowGetJobArea = mysql_fetch_array($rsGetJobArea)) 
			{
				...
				echo "<option style=\"$style\" value=\"".$rowGetJobArea["JobArea"]."\" ".($jobs5_area==$rowGetJobArea["JobArea"]?"selected":"").">".$indent.$rowGetJobArea["area"]."</option>";
			}
		?></select></span></td>
        ...
        <?php
			...
			while($rowGetJobArea = mysql_fetch_array($rsGetJobArea)) 
			{
				...
				echo "<option style=\"$style\" value=\"".$rowGetJobArea["JobArea"]."\" ".($jobs3_area==$rowGetJobArea["JobArea"]?"selected":"").">".$indent.$rowGetJobArea["area"]."</option>";
			}
		?></select></span></td>
        ...
        <?php
			...
			while($rowGetJobArea = mysql_fetch_array($rsGetJobArea)) 
			{
				...
				echo "<option style=\"$style\" value=\"".$rowGetJobArea["JobArea"]."\" ".($jobs6_area==$rowGetJobArea["JobArea"]?"selected":"").">".$indent.$rowGetJobArea["area"]."</option>";
			}
		?></select></span></td>
    	</tr>
		
		<script language="javascript">
        <?php /*?>displayAreaPriority1(document.getElementById('jobs'),'span_area','jobs_area');
        displayAreaPriority1(document.getElementById('jobs2'),'span_area2','jobs2_area');
        displayAreaPriority1(document.getElementById('jobs3'),'span_area3','jobs3_area');
        displayAreaPriority1(document.getElementById('jobs4'),'span_area4','jobs4_area');
        displayAreaPriority1(document.getElementById('jobs5'),'span_area5','jobs5_area');
        displayAreaPriority1(document.getElementById('jobs6'),'span_area6','jobs6_area');<?php */?>
    	</script>
		<?php
			/*$varAreaCategory=array();
			$sqlGetCat="SELECT JobArea,area FROM job_list_category ";
			$rsGetCat = $db->query($sqlGetCat);
			
			$varGetCatCount=0;
			while($rowGetCat = mysql_fetch_array($rsGetCat)) 
			{
				$varAreaCategory[$varGetCatCount]["JobArea"]=$rowGetCat["JobArea"];
				$varAreaCategory[$varGetCatCount]["area"]=$rowGetCat["area"];
				$varGetCatCount++;
			}
			
			echo "<br><table border=\"0\" align=\"center\" id=\"tab_area\" style=\"display:none\">
						<tr><td colspan=\"$varAreaTableColspan\"><b>Area of Priority</b></td></tr>
						<tr>";
			foreach($varAreaCategory as $key=>$value)
			{
				echo "<td><input type=\"checkbox\" name=\"cbAreaPriority[]\" id=\"".$value["JobArea"]."\" value=\"".$value["JobArea"]."\" ".(isset($varAreaPriorityDisplay[$value["JobArea"]])?"checked":"").">&nbsp;</td><td width=\"170:px\" height=\"20:px\"><label for=\"".$value["JobArea"]."\">".$value["area"]."</label>&nbsp;&nbsp;</td>";
				echo ((($key+1) % $varAreaTableColspan) == 0?"</tr><tr>":"");
			}
			echo "</tr><table><br>";
			
		if(count($varAreaPriorityDisplay)>0)
        {
            $varAreaCategory=array();
            $sqlGetCat="SELECT id,area FROM job_list_category WHERE id IN (".implode(",",$varAreaPriorityDisplay).") ";
            $rsGetCat = $db->query($sqlGetCat);
            
            $varGetCatCount=0;
            while($rowGetCat = mysql_fetch_array($rsGetCat)) 
            {
                $varAreaCategory[$varGetCatCount]["id"]=$rowGetCat["id"];
                $varAreaCategory[$varGetCatCount]["area"]=$rowGetCat["area"];
                $varGetCatCount++;
            }
            ...
            foreach($varAreaCategory as $key=>$value)
            {
                echo "<td>$varRecDisplay.&nbsp;</td><td width=\"170:px\" height=\"20:px\">".$value["area"]."&nbsp;&nbsp;</td>";
                echo ((($key+1) % $varAreaTableColspan) == 0?"</tr><tr>":"");
                $varRecDisplay++;
            }
            echo "</tr><table><br>";
