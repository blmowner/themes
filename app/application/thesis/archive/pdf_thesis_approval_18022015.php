...
$varHeader=array("No","Thesis/Project ID","Thesis/Project Title","Student Information","Supervisor List","Verified by Faculty","Status by Senate");
$varColWidth=array(7,22,35,35,35,25,25);	  				
...
$sqlSupervisor="SELECT ps.id, ps.ref_supervisor_type_id,ne.skype_id,ne.name AS name,
		ne.empid, ne.mobile,rst.description  
		FROM  pg_supervisor ps 
		LEFT JOIN ref_supervisor_type rst ON (rst.id = ps.ref_supervisor_type_id) 
		LEFT JOIN new_employee ne ON (ne.empid = ps.pg_employee_empid) 
		LEFT JOIN student c ON (c.matrix_no = ps.pg_student_matrix_no) 
		WHERE ps.pg_student_matrix_no='$studentMatrixNo'
		AND ps.ref_supervisor_type_id IN ('SV','CS')
...
//------------
		...
		array_push($setOfIngres, array($no,$pgThesisId,$thesisTitle,$studentName." - ".$studentMatrixNo,$super,$facultyStatus,$endorsedDesc));	
		$no++;
	}
