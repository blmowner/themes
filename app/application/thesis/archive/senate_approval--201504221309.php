...
if(isset($_POST['btnSubmit']) && ($_POST['btnSubmit'] <> "")) {
	...
	if (sizeof($_POST['myApprovalBox'])>0) {
		...
		while (list ($key,$val) = @each ($myApprovalBox)) {
			...
			$sql6_1 = "SELECT pp.id, IFNULL(pp.report_date,'0000-00-00 00:00:00') as report_date, pp.thesis_title, pp.thesis_type, pp.introduction, pp.objective, pp.description, pp.discussion_status,
		...
		//LOOP FOR EVERY SUPERVISOR/CO-SUPERVISOR ASSIGNED FOR A SINGLE STUDENT
		...
		while($db->next_record())
		{
			...
			elseif($verified == 'APC')
			{
				...
				//SENATE TO SUPERVISOR ---> APP WITH CHANGES					 
				$selectconstant= "SELECT const_value
				FROM base_constant WHERE const_term = 'EMAIL_SEN_TO_SUP'"; 
				$resultconstant = $dbj->query($selectconstant);
				$dbj->next_record();
			...
			elseif($verified == 'DIS')
			{
				...
				//SENATE TO STUDENT
				$selectconstant1= "SELECT const_value
				FROM base_constant WHERE const_term = 'EMAIL_SEN_TO_STU'"; 
				$resultconstant1 = $dbu->query($selectconstant1);
				$dbu->next_record();
				$constvalue1 =$dbu->f('const_value');
				if($constvalue1 = 'Y')
				{
		...
		//verified for approved SENATE TO FACULTY
		$selectconstant= "SELECT const_value
		FROM base_constant WHERE const_term = 'EMAIL_SEN_TO_FAC'"; 
		$resultconstant = $dbj->query($selectconstant);
		$dbj->next_record();
		...
		if($constvalue = 'Y')
		{
			...
			if($verified == 'APP')
			{
				...
				//echo " Approved for faculty READ ";	
			}
			elseif($verified == 'APC')
			{	
				$vstatus = "Approved with Changes";
		...
		//verified for approved with changes SENATE TO STUDENT
		$selectconstant= "SELECT const_value
		FROM base_constant WHERE const_term = 'EMAIL_SEN_TO_STU'"; 
		...
		$msg[] = "<div class=\"success\"><span>The selected thesis proposal status has been confirmed accordingly.</span></div>";
