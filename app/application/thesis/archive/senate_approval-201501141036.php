...
$sql2 = " SELECT a.pg_thesis_id , a.id, DATE_FORMAT(a.report_date,'%d-%b-%Y') AS theReportDate, a.thesis_title, a.thesis_type, 
		...
		a.verified_date, a.verified_remarks, a.status, a.discussion_status, a.proposal_remarks, 
...
/*		$approvalStatus=$_POST['approvalStatus'];
		...
		$sql5 = "UPDATE pg_supervisor SET
				respondedby_date = '$respondedByDate',endorsed_by = '$userid', endorsed_date = '$curdatetime', endorsed_remarks='$remark',
				modify_date = '$curdatetime', modify_by = '$userid'		
				WHERE pg_student_matrix_no ='$myStudentMatrixNo[$i]' ";
		$dbg->query($sql5); //echo $sql5;
	}
...
tinyMCE.init({
	...
	<body>  
			...
			<?  
				...
				?>
					<tr>
						...
						<td><input type="hidden" name ="myPgThesisId[]" size="12" id="pgThesisId" value="<?=$pgThesisId;?>" disabled="disabled"/><?=$pgThesisId;?></td>	
						<? $myPgThesisId[$no]=$pgThesisId;?>
						<? //echo "myPgThesisId[$no] ".$myPgThesisId[$no];?>
						
						<td><input type="hidden" name="myStudentName[]" size="30" id="studentName" value="<?=$studentName;?>" disabled="disabled"/><?=$studentName;?>
						<input type="hidden" name="myStudentMatrixNo[]" size="12" id="studentMatrixNo" value="<?=$studentMatrixNo;?>" disabled="disabled"/> - <?=$studentMatrixNo;?></td>
						<?$myStudentMatrixNo[$no]=$studentMatrixNo;?>
						
						<td><input type="hidden" name="myReportDate[]" size="15" id="reportDate" value="<?=$reportDate;?>" disabled="disabled"/><?=$reportDate;?></td>						
						<td><input type="hidden" name="myThesisTitle[]" id="thesisTitle" value="<?=$thesisTitle;?>" disabled="disabled"/><?=$thesisTitle; ?></td>
						<td>
		...
		<table>
			<tr>
				<td><INPUT name="BUTTON" TYPE="BUTTON" onClick="open_win('print_form.php?empid=<?=$user_id?>',900,700,0,0,0,0,1,1,0,0,5,'print_form');return false;" VALUE="Print Form">
