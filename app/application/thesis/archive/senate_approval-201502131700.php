...
if(isset($_POST['btnSubmit']) && ($_POST['btnSubmit'] <> "")) {
	...
	if (sizeof($_POST['myApprovalBox'])>0) {
		...
		while (list ($key,$val) = @each ($myApprovalBox)) {
			...
			$sql6_1 = "SELECT pp.id, pp.report_date, pp.thesis_title, pp.thesis_type, pp.introduction, pp.objective, pp.description, pp.discussion_status,
			...
			FROM pg_proposal pp 
			LEFT JOIN pg_proposal_area ppa ON (ppa.id=pp.id)
			...
			$job_area1 = $dbg->f('job_id1_area');
			$job_area2 = $dbg->f('job_id2_area');
			$job_area3 = $dbg->f('job_id3_area');
			$job_area4 = $dbg->f('job_id4_area');
			$job_area5 = $dbg->f('job_id5_area');
			$job_area6 = $dbg->f('job_id6_area');
			

			$proposal_id = "P".runnum('id','pg_proposal');	
			$new_proposal_id = runnum('id','pg_proposal_area');	
			...
			$sqlArea = "INSERT INTO pg_proposal_area
			(id, pg_proposal_id, job_id1_area,job_id2_area,job_id3_area,job_id4_area,job_id5_area,job_id5_area,insert_by, insert_date, modify_by, modify_date)
			VALUES
			('$new_proposal_id','$proposal_id','$job_area1','$job_area2','$job_area3','$job_area4','$job_area5','$job_area6','$userid', '$curdatetime','$userid', '$curdatetime')";
			$resultArea = 	$dbg->query($sqlArea);			
	...
	<body>  
			...
			<table>
				...
				?>
					<tr>
						...
						if (($tmpNewVerifiedDate->format('d-M-Y') >= $currentDate->format('d-M-Y')) && ($endorsedStatus=='')) {
						?>
						...
						<?	$sqlSupervisor="SELECT ps.id, ps.ref_supervisor_type_id,ne.skype_id,ne.name AS 
							...
							$no1=1;
								if ($row_cnt>0) {
									...
									{ ?>
										<a href="../supervisor/edit_supervisor_senate.php?mn=<?=$studentMatrixNo;?>&tid=<?=$pgThesisId;?>&sname=<?php echo $studentName?>" name="mySupervisor[]">Change <img src="../images/person_reassigned.jpg" width="20" height="19" style="border:0px;" title="Supervisor details" ></a>
								...
								{
									?>
									<a href="../supervisor/edit_supervisor_senate.php?mn=<?=$studentMatrixNo;?>&tid=<?=$pgThesisId;?>&sname=<?php echo $studentName?>" name="mySupervisor[]">Assign <img src="../images/person_assigned.jpg" width="20" height="19" style="border:0px;" title="Supervisor details" ></a>
									<?
								}
						?>
		...
		<table>
			<tr>	
				...
				<td><input type="submit" name="btnSubmit" id="btnSubmit" value="Submit" /><span style="color:#FF0000"> Note:</span> Ensure the proposal above has been selected before click Submit.</td>
			</tr>
		</table>
		<br/>
		<?}
