		...
		if (sizeof($_POST['myApprovalBox'])>0) {
			...
			while (list ($key,$val) = @each ($myApprovalBox)) {
				...
				$result6_2 = $dbg->query($sql6_2); 	
					...
					$studname = "SELECT name FROM student WHERE matrix_no='$studentid'";
		...
		if(isset($_POST['btntest']) && ($_POST['btntest'] <> "")) 
{
	...
	$myProposalId=$_SESSION['myProposalId'];
					...
					$resultsuper = $db->query($sqlsuper);		
					$db->next_record();
					echo $superid = $dbg->f('pg_employee_empid');
					
					$sqlname = "SELECT name FROM new_employee 
					WHERE empid = '$superid'";
					$resultname = $db->query($sqlname);		
					$db->next_record();
					echo $supername = $dbg->f('name');
		...
		?>	
			...
			<table border="1" style="border-collapse:collapse;" cellpadding="2" cellspacing="1">						
				...
				?>
					<tr>
						...
						(<?=$studentMatrixNoArray[$i];?>)<input type="hidden" name="hidstudid" value="<?=$studentMatrixNoArray[$i];?>" /></td>
			...
			<table>
				<tr>	
					...
					<td><input type="submit" name="btntest" id="btntest" value="Test" /></td>
