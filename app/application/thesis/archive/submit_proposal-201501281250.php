...
{			
	...
	$sqlUpdate2 = "UPDATE pg_proposal
				SET thesis_title = '$thesis_title', thesis_type = '$thesis_type', introduction = '$introduction', objective = '$objective', 
				description = '$description', modify_by = '$user_id', modify_date = '$curdatetime', discussion_status = '$discussion_status',
				report_date = '$curdatetime', proposal_remarks = '$proposalRemarks', verified_status = 'INP'
				WHERE id = '$proposal_id'
				AND verified_status in ('SAV')";
...
$sql_thesis="SELECT pt.id AS thesis_id, pt.student_matrix_no,pt.status as thesis_status,
				...
				pp.endorsed_remarks, pp.status as endorsed_status, pp.proposal_remarks,
				rps.description AS proposal_description, rps2.description AS endorsed_desc, ne.name AS verified_name
				FROM pg_thesis pt 
				LEFT JOIN pg_proposal pp ON (pp.pg_thesis_id=pt.id) 
				LEFT JOIN ref_proposal_status rps ON (rps.id = pp.verified_status)
				LEFT JOIN ref_proposal_status rps2 ON (rps.id = pp.status)  				
				LEFT JOIN new_employee ne ON (ne.empid = pp.verified_by) 
				WHERE pt.student_matrix_no = '$user_id'
				AND pp.verified_status in ('SAV','INP','APP','REQ','DIS','REV','WIT')				
				AND pp.archived_status is null
				AND pt.ref_thesis_status_id_proposal in ('INP','APP','DIS')
				ORDER BY pt.id";
	...
	{//echo "has record";
		if ($proposal_status=='APP') 
		{
			...
			?>
				...
				<legend><strong>Endorsement by Senate</strong></legend>
					...
					<table>
						<tr>
							<?if ($proposal_status=='APP' && $endorsed_status=='APP')
							{?>
							...
							<?}
							elseif ($proposal_status=='APP' && $endorsed_status=='DIS'){
							?>
		...
		<?	}
				...
				<table>
						<tr>
							...
							You can request for withdraw or cancel it before the Faculty review and provide their feedback<br/>
		...
		{ 
		 ...
		 <legend><strong>Verification by Faculty</strong></legend>
			...
			<table>
				<tr>
					<td><strong>Note: </strong>Your thesis proposal has been reviewed and require changes. Please <strong><a href="edit_proposal.php?id=<?=$thesis_id;?>&pid=<?=$proposal_id;?>"><img src="../images/right_arrow.jpg" width="20" height="19" style="border:0px;" title="Please click here"> click here </a></strong>to update it and re-submit.</td>
				</tr>
			</table>					
		...
		<fieldset>
		 <legend><strong>Verification by Faculty</strong></legend>
			...
			<table>
				<tr>
					<td><strong>Note: </strong>Your thesis proposal has been disapproved by the Senate. You may need to <strong><a href="../thesis/new_proposal.php?pid=<?php echo $userid;?>"><img src="../images/right_arrow.jpg" width="20" height="19" style="border:0px;" title="Please click here"> click here to create another one </a></strong> and re-submit.</td>
				</tr>
			</table>
		...
		<fieldset>
			...
			<table>
				<tr>			
					<td>You don't have a thesis proposal to view. Please <strong><a href="../thesis/new_proposal.php?pid=<?=$userid;?>"><img src="../images/right_arrow.jpg" width="20" height="19" style="border:0px;" title="Plase click here"> click here </a></strong> to create a new one and submit. </td>							
