...
if(isset($_POST['btnSave']) && ($_POST['btnSave'] <> ""))
	{	
		...
		$thesis_id = runnum('id','pg_thesis');
		 ...
		 $ptid = run_num('id','pg_thesis'); // generate id (running number)
		 
		 $sqlsubmit2="INSERT INTO pg_thesis(id,student_matrix_no,proposal_date,modify_by,modify_date,ref_thesis_status_id_proposal)
		 			VALUES('$ptid','$user_id','$curdatetime','$user_id','$curdatetime','INP') ";
		 $db_klas2->query($sqlsubmit2); 
		 
		  $pid = run_num('id','pg_proposal'); // generate id (running number)
		
		$sqlsubmit="INSERT INTO pg_proposal(id,thesis_title,thesis_type,introduction,objective,description,insert_by,insert_date,modify_by,modify_date,discussion_status,pg_thesis_id)
		 			VALUES('$pid','$thesis_title','$thesis_type','$introduction','$objective','$description','$user_id','$curdatetime','$user_id','$curdatetime','$discussion_status','$ptid') ";
...
<?php /*?>   <table>
   ...
   <p>
     ...
     <input type="submit" name="btnSave" id="btnSubmit" align="center"  value="<?php echo $varBtnNm; ?>" />
   </p>
  </fieldset>
  </form>
</body>
