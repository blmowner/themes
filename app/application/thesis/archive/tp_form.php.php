...
{	
	...
	for ($i=0; $i<sizeof($_POST['date']); $i++) {
		//$sqlsubmit3="INSERT INTO pg_meeting_detail(id,lecturer_name,meeting_sdate,meeting_time,remark,insert_by,insert_dt,modify_by,modify_dt)
		//		VALUES('$thesis_id','$lecturer_name','$meeting_sdate','$meeting_time','$remark','$user_id','$curdatetime','$user_id','$curdatetime') ";
		//$db_klas2->query($sqlsubmit3); //echo $sqlsubmit3;
		...
		$sql = "INSERT INTO pg_meeting_detail(lecturer_name,meeting_sdate,meeting_time,remark,pg_proposal_id,pg_thesis_progress_id,insert_by,insert_date,modify_by,modify_date)
				...
				'$thesis_id','','$user_id','$curdatetime','$user_id','$curdatetime')";
		$db_klas2->query($sql); 
	}
	
	for ($i=0; $i<sizeof($_POST['file_name']); $i++) {
		// insert into file bla bla
	}
}
  ...
  <strong>Discussion Details </strong><br />
   ...
   <fieldset>
		   ...
		   <p>
					...
					<?php
							/* temp hide $sql="SELECT * FROM pg_thesis pt 
