	...
	if (sizeof($_POST['myApprovalBox'])>0) {
		...
		while (list ($key,$val) = @each ($myApprovalBox)) 
		{
			...
			if ($confirmProposal == 'Y'){				
				...
				$emailfaculty = explode(" , ", $fromfaculty);
				...
				$selectfacultydept = "SELECT a.name,a.email,a.unit_id,b.title 
				FROM new_employee a
				LEFT JOIN lookup_gelaran b ON(b.id = a.title)
				WHERE a.empid = '$user_id'";
				$resultselsenname = $dbc->query($selectfacultydept);
				$dbc->next_record();
				$facultyname =$dbc->f('name');
				$facultyemail =$dbc->f('email');
				$facultytitle=$dbc->f('title');
				$facultyDept=$dbc->f('unit_id');//>>>>>>>>>>>>>>>>>>>>>> UNIT ID for Faculty
				
				/////>>>>>>>>>>STUDENT INFO for email<<<<<<<<<<<<<<<<<<<<<<<<<<<
				...
				WHERE matrix_no = '$myStudentId[$val]'";		
				$resultstuddept = $dbk->query($studdept);
				$dbk->next_record();
				$studDept = $dbk->f('dept_unit');

				$studinfo = "SELECT name, email FROM student
							WHERE matrix_no = '$myStudentId[$val]'";/////>>>>>>>>>>STUDENT INFO for email		
				$resultstudinfo = $dbn->query($studinfo);
				$dbn->next_record();
				$studidname =$dbn->f('name');
				$studemail	=$dbn->f('email');
				//>>>>>>>>>>>>>>>END Student INFO<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
				...
				if($verified == 'APP')
				{
					...
					while($dba->next_record())// Loop for every reviewer that has been assigned.
					{
						...
						$sqlrev = "SELECT a.name,a.email,a.unit_id,b.title 
						FROM new_employee a
						LEFT JOIN lookup_gelaran b ON(b.id = a.title)
						...
						$revieweremail =$dbl->f('email');//>>>>>>>>>>>>>>>>>>>>>>>>>reviewer email
						$reviewerDept = $dbl->f('unit_id');/////////>>>>>>>>>>>>>>UNIT ID for Reviewer
						$title =$dbl->f('title');			
						...
						if (($facrev == 'Y') && (!empty($revieweremail))) 
						{
					...
					FROM base_constant WHERE const_term = 'MESSAGE_FAC_TO_SEN'";
					$resultvalidate = $db->query($sqlval);
					$db->next_record();
					$valid =$db->f('const_value');
					if($valid == 'Y')
					{
						include("../../../app/application/inbox/submission/faculty_to_senate_inbox.php"); //>>>>>>>>>>>>>>>>>>>>>>message notification
				...
				elseif($verified == 'REQ')
				{
					...
					//>>>>>>>>>>>>>>>>>>>>>>message notification<<<<<<<<<<<<<<<<<<<<<<<<<
					$sqlval = "SELECT const_value
					FROM base_constant WHERE const_term = 'MESSAGE_FAC_TO_STU'";
					$resultvalidate = $db->query($sqlval);
					$db->next_record();
					$valid =$db->f('const_value');
					if($valid == 'Y')
					{
						include("../../../app/application/inbox/submission/faculty_to_student_inbox.php"); //>>>>>>>>>>>>>>>>>>>>>>message notification
...
$(function() {
	...
	?>
		...
		?>
			...
			<?
				...
				?>
					...
					<tr>
						...
						<td><label name="myThesisTitle[]" cols="45" id="thesisTitle"><?=$thesisTitleCut[$i]?></label><?=$more[$i]?>
						</td>
			...
			<table>
				<tr>
					<td><span style="color:#FF0000"> Notes:</span> <br/>Ensure the following has been checked before SUBMIT:-</td>
				</tr>
				<tr>
					<td>1. The proposal has been ticked / selected.</td>
				</tr>
				<tr>
					<td>2. The Supervisor / Co-Supervisor has been assigned before the approval.</td>
				</tr>
			</table>
			<table>					
				<tr>				
