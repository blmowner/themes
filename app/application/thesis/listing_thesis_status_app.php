	...
	<body>  
		...
		?>				
			...
			<?
				...
				$verifiedDesc=array();
				$endorsedRemarks=array();
				$endorsedStatus=array();
				$endorsedDesc=array();
				$endorsedDate=array();
				$verifiedDate=array();								
				...
				while($db->next_record()) {						
					...
					$lecturer_name[$i]=$db->f('lecturer_name');
					$lecturer_id[$i]=$db->f('lecturer_id');
					$description[$i]=$db->f('description');
					$verified_status[$i]=$db->f('verified_status');
					$verifiedDesc[$i]=$db->f('verifiedDesc');
					$endorsedRemarks[$i]=$db->f('endorsed_remarks');
					$endorsedStatus[$i]=$db->f('endorsedStatus');
					$endorsedDesc[$i]=$db->f('endorsedDesc');
					$endorsedDate[$i]=$db->f('endorsed_date');
					$verifiedDate[$i]=$db->f('verified_date');
				...
				?>
					<tr>
						...
						<td><a href="../thesis/thesis_approval_outline.php?thesisId=<?=$pgThesisId[$i];?>&proposalId=<?=$proposalId[$i];?>&endorsedDate=<?=$searchRequestDate;?>&lot=A" name="myPgThesisId[]" value="<?=$pgThesisId[$i]?>" title="Outline of Proposed Case Study by the Student - Read more..."><?=$pgThesisId[$i];?></a><br/>
						
						<? $myPgThesisId[$no]=$pgThesisId[$i];?>
						
					  <td><label name="myThesisTitle[]" id="thesisTitle" ><?=$thesisTitleCut[$i]?></label><?=$more[$i]?></td>
						<?
						$sql3 = "SELECT name
								FROM student
								WHERE matrix_no = '$studentMatrixNo[$i]'";		
						if (substr($studentMatrixNo[$i],0,2) != '07') { 
							$dbConnStudent= $dbc; 
						} 
						else { 
							$dbConnStudent=$dbc1; 
						...
						(<?=$studentMatrixNo[$i];?>)</td>
						<? $myStudentMatrixNo[$no]=$studentMatrixNo[$i];?>
						...
						<?	$sqlSupervisor="SELECT ps.id, ps.ref_supervisor_type_id,
									...
									WHERE ps.pg_student_matrix_no='$studentMatrixNo[$i]' 
									AND ps.ref_supervisor_type_id IN ('SV','CS','XS') 
									AND ps.pg_thesis_id = '$pgThesisId[$i]' 
									AND ps.status = 'A' 
									ORDER BY rst.seq";
							...
							if ($row_cnt>0) {
								...
								{ 
									...
									?>
									<?=$no1?>. <?=$staffName;?> - <?=$row["description"];?> (<?=$row["pg_employee_empid"];?>)<br/>								
								<? $no1++;}?>
						...
						<td><label name="myVerifiedDesc[]" id="verifiedDesc" ></label><?=$verifiedDesc[$i]; ?><br/>(<?=$verifiedDate[$i];?>)</td>
						<td><label name="myEndorsedDesc[]" id="endorsedDesc" ></label><?=$endorsedDesc[$i]; ?><br/>(<?=$endorsedDate[$i];?>)</td>
		...
		<br />		
		<td><input type="button" name="btnPrintProposal" value="Print Listing" onclick="javascript:document.location.href='../thesis/pdf_thesis_approval_app.php?smd=<?=$searchRequestDate?>';" /> 
		</td><br/>
		<br />	
