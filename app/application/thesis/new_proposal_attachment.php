...
{					
	if (sizeof($_POST['attachment_checkbox'])>0) {
		...
		while (list ($key,$val) = @each ($_POST['attachment_checkbox'])) 
		{
			...
			$sql1 = "UPDATE file_upload_proposal
...
{					
	if (sizeof($_POST['attachment_checkbox'])>0) {
		...
		while (list ($key,$val) = @each ($_POST['attachment_checkbox'])) 
		{
			...
			$sql1 = "DELETE FROM file_upload_proposal
			WHERE fu_cd = '$attachment_id'
			AND student_matrix_no = '$user_id'";
...
?>
  ...
  <form id="form1" name="form1" method="post" enctype="multipart/form-data" onsubmit="return saveRec();">
	 ...
	 <fieldset>
		...
		<table>
			<tr>
				<td><input type="submit" name="submit" value="Add Attachment" onclick="javascript:open_win('new_proposal_upload.php?pid=<?=$proposal_id;?>&al=S',480,280,0,0,0,1,0,1,1,0,5,'winupload'); "/></td>
			</tr>
		</table>
		<table>
		<tr>							
	...
	<table>
		<tr>
			<td><input type="button" name="btnBack" value="Back" onclick="javascript:document.location.href='../thesis/new_proposal.php';" /></td>		
