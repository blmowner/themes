...
if(isset($_POST['btnAdd']) && ($_POST['btnAdd'] <> ""))
{
	...
	if(empty($msg)) 
	{
		...
		$sqlMeeting = "INSERT INTO pg_meeting_detail(id, student_matrix_no,
						lecturer_id,external_lecturer,meeting_sdate,meeting_mode,remark,pg_proposal_id,
						pg_thesis_progress_id,insert_by,insert_date,modify_by,modify_date)
						VALUES (
...
{					
	if (sizeof($_POST['meeting_detail_checkbox'])>0) {
		...
		while (list ($key,$val) = @each ($_POST['meeting_detail_checkbox'])) 
		{
			...
			external_lecturer = '$external_lecturer', meeting_mode = '$meetID', remark = '$remark', 
	...
	<script>
		...
		$(document).ready(function(){
              ...
              $.fn.getParameterValue = function(data,data2) {
               ...
               $(".select_user").colorbox({width:"80%", height:"90%", iframe:true, onClosed:function(){} }); 
          });
	</script>
...
?>
  ...
  <form id="form1" name="form1" method="post" enctype="multipart/form-data" onsubmit="return saveRec();">
	 ...
	 <fieldset>
		...
		<table>
			<tr>
				...
				<td><input type="text" name="add_meeting_date" id="add_meeting_date" maxlength="50" readonly=""/></td>
			...
			<tr>
			  ...
			  <td><input type="text "name="add_external_lecturer" cols="50" id="add_external_lecturer"> <em><span style="color:#FF0000">Note:</span> If non-MSU staff.</em></input>
			  </td>
			<tr>
		...
		<table border="1" cellpadding="3" cellspacing="3" width="100%" id="inputs10" class="thetable">
			  <tr align="left">
				<th width="4%" align="center"><label>Tick</label></td>
				<th width="4%" align="center"><label>No</label></td>
				<th width="11%" align="center"><label>Date <span style="color:#FF0000">*</span></label></th>
				<th width="11%" align="center"><label>Time <span style="color:#FF0000">*</span></label></th>
				<th width="28%" >Lecturer Name <span style="color:#FF0000">*</span></th>
				<th width="19%" >External Lecturer <span style="color:#FF0000">*</span></th>
			...
			if ($j > 0) {				
				...
				?>
							...
							<td align="left"><input type="hidden" name="staffIdArray<?=$k;?>" id="staffIdArray<?=$k;?>" value="<?=$staffIdArray[$k];?>" />
	...
	<table>
		<tr>
			<td><input type="button" name="btnBack" value="Back" onclick="javascript:document.location.href='../thesis/new_proposal.php';" /></input></td>		
