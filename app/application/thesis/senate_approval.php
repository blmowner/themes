...
// Program Name: senate_approval.php
//
...
if(isset($_POST['btnSubmit']) && ($_POST['btnSubmit'] <> "")) 
{
	...
	if (sizeof($_POST['myApprovalBox'])>0) {
		...
		(id, endorsed_status, endorsed_by, endorsed_date, endorsed_remarks, insert_by, insert_date, modify_by, modify_date)
		VALUES ('$proposalApprovalId', '$endorsedStatus', '$userid', STR_TO_DATE('$senateMtgDate','%d-%b-%Y'), '$endorsedRemarks', 
		'$userid', '$curdatetime', '$userid', '$curdatetime')";
		
		$result_sql7=$dbg->query($sql7);
		$dbg->next_record();
		
		$sql7_1 = "UPDATE file_upload_senate
		SET approval_id = '$proposalApprovalId', upload_status = 'CFM', modify_by = '$userid', modify_date = '$curdatetime'
		WHERE employee_id = '$userid'
		AND upload_status = 'TMP'";
		
		$result_sql7_1=$dbg->query($sql7_1);
		$dbg->next_record();
		...
		while (list ($key,$val) = @each ($myApprovalBox)) {
			...
			$sql6_1 = "SELECT pp.id, IFNULL(pp.report_date,'0000-00-00 00:00:00') as report_date, pp.thesis_title, pp.thesis_type, pp.introduction, pp.objective, pp.description, pp.discussion_status, DATE_FORMAT(pp.report_date,'%d-%b-%Y') as report_date_email,
			...
			pp.pg_thesis_id, pp.pg_proposal_approval_id, ppa.*
			FROM pg_proposal pp 
			...
			('$proposal_id', '$reportDate', '".mysql_real_escape_string($thesisTitle)."', '$thesisType', '".mysql_real_escape_string($introduction)."',
						'".mysql_real_escape_string($objective)."', '".mysql_real_escape_string($description)."', '$discussionStatus', 
			...
			while($db->next_record())
			{
				...
				$sqlname = "SELECT a.name,a.email,b.title 
				FROM new_employee a
				LEFT JOIN lookup_gelaran b ON(b.id = a.title)
				WHERE a.empid = '$superid'";
				...
				if($verified == 'APP')
				{
					...
					FROM pg_employee 
					WHERE staff_id = '$superid'";
					$resultstat = $dbf->query($sqlstat);
					$dbf->next_record();
					$emailStatus =$dbf->f('email_status');
					
					$selectconstant= "SELECT const_value
					FROM base_constant WHERE const_term = 'EMAIL_SEN_TO_SUP'"; 
					$resultconstant = $dbj->query($selectconstant);
					$dbj->next_record();
					$constvalue =$dbj->f('const_value');
					if ($constvalue == 'Y' && $emailStatus == 'Y' && !empty($superemail))
					{
						//echo "Email dalam APP supervisor";
						include("../../../app/application/email/email_assign_supervisor.php");
					}
					//>>>>>>>>>>>>>>>>>>>>>>message notification<<<<<<<<<<<<<<<<<<<<<<<<<
					$sqlval = "SELECT const_value
					FROM base_constant WHERE const_term = 'MESSAGE_SEN_TO_SUP'";
					$resultvalidate = $dbf->query($sqlval);
					$dbf->next_record();
					$valid =$dbf->f('const_value');
					if($valid == 'Y')
					{
						include("../../../app/application/inbox/submission/senate_to_supervisor_inbox.php"); //>>>>>>>>>>>>>>>>>>>>>>message notification
				...
				elseif($verified == 'APC')
				{
					...
					if($constvalue == 'Y' && $emailStatus =='Y' && !empty($superemail))
					{
						//echo " Email Dalam APC supervisor ";
						//echo " ".$facultyemail." ";
						include("../../../app/application/email/email_assign_supervisor.php");
					...
					$sqlval = "SELECT const_value
					FROM base_constant WHERE const_term = 'MESSAGE_SEN_TO_SUP'";
					$resultvalidate = $db->query($sqlval);
					$db->next_record();
			...
			if($verified == 'DIS')
			{
				...
				$FileType = Array();
				$selectconstant= "SELECT const_value
				FROM base_constant WHERE const_term = 'EMAIL_SEN_TO_STU'"; 
				$resultconstant = $dbj->query($selectconstant);
				$dbj->next_record();
				$constvalue =$dbj->f('const_value');
				if($constvalue == 'Y')
				...
				//>>>>>>>>>>>>>>>>>>>>>>message notification <<<<<<<<<<<<<<<<<<<<<<<<<<<<<
				$sqlval = "SELECT const_value
				FROM base_constant WHERE const_term = 'MESSAGE_SEN_TO_STU'";
				$resultvalidate = $db->query($sqlval);
				$db->next_record();
				$valid =$db->f('const_value');
				if ($valid == 'Y' && !empty($studemail))
				{
					include("../../../app/application/inbox/submission/senate_to_student_inbox_dis.php"); //>>>>>>>>>>>>>>>>>>>>>>message notification
		
				}
				//>>>>>>>>>>>>>>>>>>>>>>message notification end <<<<<<<<<<<<<<<<<<<<<<<<<<<<<
				//>>>>>>>>>>>>>>>>>>>>>>message notification <<<<<<<<<<<<<<<<<<<<<<<<<<<<<
				$sqlval = "SELECT const_value
				FROM base_constant WHERE const_term = 'MESSAGE_SEN_TO_FAC'";
				$resultvalidate = $db->query($sqlval);
				$db->next_record();
				$valid =$db->f('const_value');
				if($valid == 'Y')
				{
					include("../../../app/application/inbox/submission/senate_to_faculty_inbox_dis.php"); //>>>>>>>>>>>>>>>>>>>>>>message notification
			...
			if($constvalue == 'Y')
			{
				...
				if($verified == 'APP')
				{
					...
					//echo " Approved with changes for faculty READ ";
...
/*************************************************Message Notification**************************/
			...
			if($verified == 'APP')
			{
				...
				$dbj->next_record();
				$valid =$dbj->f('const_value');
				if($valid == 'Y')
				{
					include("../../../app/application/inbox/submission/senate_to_student_inbox.php"); //>>>>>>>>>>>>>>>>>>>>>>message notification
				...
				if($valid == 'Y')
				{
					include("../../../app/application/inbox/submission/senate_to_faculty_inbox.php"); //>>>>>>>>>>>>>>>>>>>>>>message notification
		
				}
				//>>>>>>>>>>>>>>>>>>>>>>message notification end <<<<<<<<<<<<<<<<<<<<<<<<<<<<<
			}
			else if($verified == 'APC')
			{
				//>>>>>>>>>>>>>>>>>>>>>>message notification <<<<<<<<<<<<<<<<<<<<<<<<<<<<<
				$sqlval = "SELECT const_value
				FROM base_constant WHERE const_term = 'MESSAGE_SEN_TO_STU'";
				...
				if($valid == 'Y')
				{
					include("../../../app/application/inbox/submission/senate_to_student_inbox_awc.php"); //>>>>>>>>>>>>>>>>>>>>>>message notification
				...
				if($valid == 'Y')
				{
					include("../../../app/application/inbox/submission/senate_to_faculty_inbox_awc.php"); //>>>>>>>>>>>>>>>>>>>>>>message notification
...
/*************************************************Message Notification End**************************/
			...
			//verified for approved with changes
			$selectconstant= "SELECT const_value
			FROM base_constant WHERE const_term = 'EMAIL_SEN_TO_STU'"; 
			$resultconstant = $dbu->query($selectconstant);
			$dbu->next_record();
			$constvalue =$dbu->f('const_value');
			if($constvalue == 'Y' && !empty($studemail))
			{
				$verified = $_REQUEST['endorsedStatus'];
				if($verified == 'APP')
				{
					...
					//echo " Approved for student READ ";
				}
				elseif($verified == 'APC')
				{	
					$vstatus = "Approved with Changes";
					include("../../../app/application/email/senate_app_changes_student.php");
					//echo " Approved with changes for student READ ";
				}
...
if(isset($_POST['btnSearch']) && ($_POST['btnSearch'] <> "")) {
	...
	$senateStatusDropDown = $_POST['senateStatusDropDown'];
	...
	if ($senateStatusDropDown!="") 
	{
		$tmpSenateStatus = " AND a.status = '$senateStatusDropDown'";
	}
	else 
	{
		$tmpSenateStatus="";
	 ...
	 $sql2 = "SELECT a.pg_thesis_id , a.id, DATE_FORMAT(a.report_date,'%d-%b-%Y') AS theReportDate, a.thesis_title, a.thesis_type, 
		...
		/*$totalresult = mysql_num_rows($result12);
		echo $totalresult;*/
...
if(isset($_POST['btnSearchByName']) && ($_POST['btnSearchByName'] <> "")) {
	...
	$sql10 = "SELECT a.pg_thesis_id , a.id, DATE_FORMAT(a.report_date,'%d-%b-%Y') AS theReportDate, a.thesis_title, a.thesis_type, 
		...
		do {
			...
			$verifiedDateArray[$no1] = $db->f('verified_date');
			$endorsedDateArray[$no1] = $db->f('endorsed_date');
			$approvalIdArray[$no1] = $db->f('endorsed_date');
			$no1++;
...
function newAttachment() 
{
    ...
    if (ask) 
	{
		document.location.href = "../thesis/senate_proposal_attachment.php";
		return true;
    }
	return false;
}
		...
		?>	
			...
			<table>				
				...
				<tr>
					<td>Senate Status </td>
					<td>:</td>
					<td><select name = "senateStatusDropDown" >
					
					<?if ($senateStatusDropDown == "") $senateStatusDropDown = $_POST['senateStatusDropDown'];?>
					<?if ($senateStatusDropDown == "") {?><option value="" selected="selected"></option><?} else {?><option value=""></option><?}?>					
					...
					do {
						$statusId=$dbf->f('id');	
						if ($statusId == 'OPN') $statusDesc = 'Pending';
						else $statusDesc=$dbf->f('description');	
						
						?>
						<?if ($senateStatusDropDown == $statusId) {?><option value="<?=$statusId?>" selected="selected"><?=$statusDesc?></option><?} else {?><option value="<?=$statusId?>"><?=$statusDesc?></option><?}?>		
						<?
					}while ($dbf->next_record());
					?>
				</select></td>
				...
				<tr>
						<td>Student Name</td>
						<td>:</td>
						<td><input type="text" name="searchStudentName" size="30" id="searchStudentName" value="<?=$searchStudentName;?>"/><input type="submit" name="btnSearch" value="Search"/> <span style="color:#FF0000"> Note:</span> If no parameters are provided, it will search all.</td>
				</tr>
			</table>
			<br/>
			...
			<table>
					<tr>							
						<td>Searching Results:- <?=$no2?> record(s) found</td>
					</tr>
			</table>
		<? if($no2 == 1)
		{?>
		...
		?>
			...
			<table border="1" style="border-collapse:collapse;" cellpadding="2" cellspacing="1" width="100%" class="thetable">						
				<tr>						
					...
					<th width="100"><strong>Senate Status</strong></th>
					...
					<th width="151"><strong>Supervisor/Co-Supervisor </strong></th>
				...
				?>
					<tr>
						<? if ($endorsedStatusArray[$i]=='APP' || $endorsedStatusArray[$i]=='APC' || $endorsedStatusArray[$i]=='DIS'){
							?><td width="30" align="center"><input name="myApprovalBox[]" type="checkbox" value="<?=$no;?>" disabled="disabled" /></td><?							
						}
							else {
								?><td width="30" align="center"><input name="myApprovalBox[]" type="checkbox" value="<?=$no;?>" /></td><?
						}
						?>		
						...
						?>
							<img src="../images/new.jpg" width="40" height="40" style="border:0px;" title="Proposal is considered new if it is submitted within <?=$parameterValue?> day(s)">
						<?}?></td>
						
						<?if ($endorsedDescArray[$i]!="") {?>
							<td width="100"><label name="myEndorsedDesc[]" id="endorsedDesc" ></label><?=$endorsedDescArray[$i];?><br/><?=$endorsedDateArray[$i];?><br/><br/>
							<a href="senate_approval_detail.php?aid=<?=$approvalIdArray[$i];?>" name="myPgThesisId[]" value="<?=$pgThesisIdArray[$i]?>" title="Approval detail - Read more...">Status ID: <?=$approvalIdArray[$i]?></a></td>
						<?}
						else {
							?>
							<td width="100"><label name="myEndorsedDesc[]" id="endorsedDesc" ></label><?=$endorsedDescArray[$i];?><br/><?=$endorsedDateArray[$i];?></td>
							<?
						}?>
						
						<td width="100"><label name="reportDate[]" cols="45" id="reportDate"><?=$reportDateArray[$i]?></label></td>
						<td width="131"><a href="senate_approval_outline.php?thesisId=<? echo $pgThesisIdArray[$i];?>&proposalId=<? echo $proposalIdArray[$i];?>" name="myPgThesisId[]" value="<?=$pgThesisIdArray[$i]?>" title="Outline of Proposed Case Study by the Student - Read more..."><?=$pgThesisIdArray[$i];?><br/>
						...
						<td width="208"><label name="myThesisTitle[]" id="thesisTitle" ><?=$thesisTitleCut[$i]; ?></label><?=$more[$i]?></td>
						
						<td width="102"><label name="myStudentName[]" size="30" id="studentName" ></label><?=$studentNameArray[$i];?>
			...
			<table>				 
				<tr>
					...
					<td>Senate Meeting Date</td>
					<td>:</td>
					<td><input type="text" name="senateMtgDate" size="15" id="senateMtgDate" value="<?=$senateCurrentDate;?>" readonly="true"/></td>
				...
				<? 
					   $sqlUpload="SELECT COUNT(*) as total  FROM file_upload_senate 
						WHERE employee_id = '$userid'
						AND upload_status = 'TMP'";			
				...
				<tr>
					<td>Attachment</td>
					<td>:</td>
					<td><button type="button" name="btnAttachment" value="Attachment" onclick="javascript:document.location.href='../thesis/senate_proposal_attachment.php'" >
			...
			<table>
				<tr>	
					<td><input type="button" name="btnPrintProposal" id="btnPrintProposal" value="Print Proposal List" onclick="javascript:document.location.href='pdf_thesis_approval_app2.php';"/></td>
					<td><input type="submit" name="btnSubmit" id="btnSubmit" value="Submit" onClick="return respConfirm()" /><span style="color:#FF0000"> Note:</span> Ensure the required proposal above has been selected before click Submit.</td>
				</tr>
			</table>
			<br/>
		<?}
