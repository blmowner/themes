...
$xpic_desc = clean($xpic_desc, 50);
	...
	if (is_uploaded_file($xpic_file))
	{
		...
		$fu_cd = runnum('fu_cd','file_upload_senate');
		$approvalId = runnum('id','pg_proposal_approval');
		...
		{	
			...
			$sql2="INSERT INTO file_upload_senate 
				(fu_cd, approval_id, fu_document_filename,fu_document_filedesc,fu_document_filetype,  fu_document_filedata, employee_id, 
				upload_status, insert_date, insert_by, modify_date, modify_by)	
				VALUES ('$fu_cd',null, '$FileName','$fileDesc','$xpic_file_type','$fileContents', '$userid', 'TMP', now(),'$userid',now(),'$userid')";
				
			if (($db->query($sql2)) && @ mysql_affected_rows() == 1) 
			{
				echo "<script language=\"javascript\">";
				echo "opener.location.href=\"senate_proposal_attachment.php\";";
				echo "document.location.href=\"senate_proposal_receipt.php?data=$fileContent&status=T&fucd=$fu_cd\";";
				echo "</script>";
				//header("Location: senate_proposal_receipt.php?data=$fileContent&status=T&fucd=$fu_cd&taid=$theApprovalId");
			}
			else 
			{
				header("Location: senate_proposal_receipt.php?data=$fileContent&status=F&fucd=$fu_cd&taid=$theApprovalId");
			}
		}
		else
		{
...
<body>
<form name="frmEmpUpdPic" method="post" action="senate_proposal_upload.php" enctype="multipart/form-data">
