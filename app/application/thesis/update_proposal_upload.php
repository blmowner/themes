	...
	if (is_uploaded_file($xpic_file))
	{
		...
		{	
			...
			if (($db->query($sql2)) && @ mysql_affected_rows() == 1) 
			{
				...
				echo "opener.location.href=\"update_proposal_attachment.php?pid=$pgProposalId\";";
				echo "document.location.href=\"update_proposal_receipt.php?data=$fileContent&status=T&userId=$user_id&fucd=$upload_id\";";
				echo "</script>";
				//header("Location: new_proposal_receipt.php?data=$fileContent&status=T&userId=$user_id&fucd=$upload_id");
			}
			else 
			{
				header("Location: update_proposal_receipt.php?status=F&userId=$user_id&fucd=$upload_id");
			}
		}
		else
		{
...
<body>
<form name="frmEmpUpdPic" method="post" action="update_proposal_upload.php?pid=<?=$pgProposalId?>&al=S" enctype="multipart/form-data">
