...
{					
	if (sizeof($_POST['attachment_checkbox'])>0) {
		...
		while (list ($key,$val) = @each ($_POST['attachment_checkbox'])) 
		{
			...
			$sql1 = "UPDATE file_upload_attachment
...
?>
  ...
  <form id="form1" name="form1" method="post" enctype="multipart/form-data" onsubmit="return saveRec();">
	 ...
	 <fieldset>
		...
		<table>
			<tr>
				...
				<td><input type="submit" name="submit" value="Add Attachment" onclick="javascript:open_win('edit_amendment_upload.php?pid=<?=$amendmentId;?>&tid=<?=$pgThesisId;?>&ppid=<?=$proposalId;?>&ref=<?=$referenceNo;?>&mid=<?=$mid?>',480,280,0,0,0,1,0,1,1,0,5,'winupload'); "/></td>
			</tr>
		</table>
		<table>
		<tr>							
	...
	<table>
		<tr>
			<td><input type="button" name="btnBack" value="Back" onclick="javascript:document.location.href='../viva/edit_amendment.php?tid=<?=$pgThesisId;?>&pid=<?=$proposalId;?>&ref=<?=$referenceNo;?>&mid=<?=$mid?>';" /></td>
