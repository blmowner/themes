...
if(isset($_POST['btnDis']) && ($_POST['btnDis'] <> ""))
{
	...
	$lock_tables="LOCK TABLES pg_viva_appeal WRITE, pg_viva WRITE"; //lock the table
	...
	WHERE `empid` = '$spMain'";
	$dbsp = $dbk;
	$resultreceive = $dbsp->query($sqlemail);
	$resultsqlreceive = $dbsp->next_record(); 
	$superEmail = $dbsp->f('email');
	$superName = $dbsp->f('name');
	
	$sqlemailfaculty = "SELECT name,email FROM new_employee
	WHERE `empid` = '$user_id'";
	$dbuser = $dbk;
	$resultreceive = $dbuser->query($sqlemailfaculty);
	$resultsqlreceive = $dbuser->next_record(); 
	$userEmail = $dbuser->f('email');
	$userName = $dbuser->f('name');
		
	///// message notification//// STUDENT
	$sqlmsg = "SELECT const_value FROM base_constant
	WHERE const_term = 'MESSAGE_FAC_TO_STU'";
	...
	if ($msgConstValue == 'Y')
	{
		include("../../../app/application/inbox/viva/appeal_review_dis.php");
	}
	
	////email notification/////
	$sqlemail = "SELECT const_value FROM base_constant
	WHERE const_term = 'EMAIL_FAC_TO_STU'";
	$dbg1 = $dbg;
	...
	{
		include("../../../app/application/email/viva/email_appeal_review_dis.php");
	}
	
	///// message notification//// SUPERVISOR
	$sqlmsg = "SELECT const_value FROM base_constant
	WHERE const_term = 'MESSAGE_FAC_TO_SUP'";
	...
	if ($msgConstValue == 'Y')
	{
		include("../../../app/application/inbox/viva/appeal_review_dis_sp.php");
	}
	...
	if ($emailConstValue == 'Y')
	{
		include("../../../app/application/email/viva/email_appeal_review_dis_sp.php");
	}
	...
	if($msgConstValue == 'Y')
	{
		include("../../../app/application/inbox/viva/school_senate_2.php");
	}
	...
	{
		//echo "sadsa";
		//include("../../../app/application/email/viva/email_school_senate_2.php");
	}
	
	$save = "2";
	echo "<script>window.location = 'appeal_review_view.php?tid=".$pgThesisId."&vid=".$pgVivaId."&aid=".$pgVivaId."&mn=".$matrixNo."&save=2';</script>";
}
...
{	
	...
	if ($msgConstValue == 'Y')
	{
		include("../../../app/application/inbox/viva/appeal_review_app.php");
	}
	...
	if ($msgConstValue == 'Y')
	{
		include("../../../app/application/inbox/viva/appeal_review_app_sp.php");
	}
...
$sql_supervisor_main = " SELECT a.pg_employee_empid, a.ref_supervisor_type_id, d.description as supervisor_type, 
...
$dbSpMain = $db_klas2;
$result_sql_supervisor_main = $dbSpMain->query($sql_supervisor_main); //echo $sql;
$dbSpMain->next_record();
$row_cnt_supervisor_main = mysql_num_rows($result_sql_supervisor_main);
$pg_employee_empid = $dbSpMain->f('pg_employee_empid');
...
function newAttachment(tid,pid,mn,role,vid, vd) {
	...
	<form id="form1" name="form1" method="post" enctype="multipart/form-data">	
			...
			<table width="52%">
				...
				<tr>
			      ...
			      <input type="hidden" name= "scheId" id= "scheId" value="<?=$calendarIdViva?>" />
				  <input type="hidden" name= "spMain" id= "spMain" value="<?=$pg_employee_empid?>" /></td>				
				</tr>   			
			</table>
			<br />
			...
			<table border="1" style="border-collapse:collapse;" cellpadding="2" cellspacing="1" width="90%" class="thetable">
              ...
              <tr>
				...
				<? if($report_status == 'SUB') {?>
                    <a href="view_edit_viva_report_faculty.php?tid=<?=$thesisId?>&type=<?=$ref_supervisor_type_id?>&mn=<?=$matrixNo?>&role=<?=$role_status?>&pd=<?=$otherDetailId?>&empid=<?=$employeeId?>&other=<?=$evaluationId?>&othertype=<?=$roleType?>&vid=<?=$pgVivaId?>&aid=<?=$appealId?>" name="thesisId" value="<?=$thesisId?>" title="View feedback"><img src="../images/view.jpg" width="45" height="30" style="border:0px;" title="View feedback" /></a>
	...
	<table width="90%">
		<tr>
			<td width="6%"><strong>Content</strong>:</td>
	...
	<table>
		<tr>
			<td colspan="2"><label><span style="color:#FF0000">Notes:</span><br/>
					1. Save as Draft button is for save the appeal in the system..<br />
			2. Submit button is to submit appeal to faculty </label></td>
		</tr>
		<tr>
		  <td colspan="2">
		  ...
		  <input type="submit" name="btnDis" id="btnDis" value="Disapprove" onclick = "return submitReportDis()"/>
		  <input type="submit" name="btnApp" id="btnApp" value="Approve" onclick = "return submitReport()"/>
</td>
