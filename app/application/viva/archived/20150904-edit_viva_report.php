...
if(isset($_POST['btnSave']) && ($_POST['btnSave'] <> ""))
{
	...
	$sql7 = "UPDATE pg_evaluation_viva_detail
			...
			recommendation_id = '$recCheck', modify_by = '$user_id', modify_date = '$curdatetime'
			WHERE id = '$pd'
			AND report_status = 'SAV'
			AND status = 'A'
...
?>
	...
	<input type="hidden" id="evaVivaId" name="evaVivaId" value="<?=$evaVivaId?>" />
		...
		<tr>
			...
			<td width="3%"><?=$iq+1?><input type="hidden" name="overallStyle[]" id="overallStyle" value="<?=$idArray[$iq]?>" />
		...
		<tr>
			<td colspan="4"><textarea cols="40" name="commentA" id="commentA"><?=$commentSecA?></textarea></td>
		</tr>
	</table>
...
?>
	...
	<table class="thetable" border="1">
		...
		<tr>
			...
			<input type="radio" name="recCheck" id="recCheck" checked="checked" value="<?=$idRecArray[$no]?>" />
			<? } else { ?>
			<input type="radio" name="recCheck" id="recCheck" value="<?=$idRecArray[$no]?>" />
			<? } ?>
			</td>
