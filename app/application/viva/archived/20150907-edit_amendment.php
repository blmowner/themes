...
if(isset($_POST['btnSubmit']) && ($_POST['btnSubmit'] <> ""))
{
	...
	if(empty($msg))
	{	
		if($amendmentStatus == '' || $amendmentStatus == 'SAV' || $amendmentStatus == 'SUB')
		{
...
DATE_FORMAT(a.acceptance_date,'%d-%b-%Y') as acceptance_date, h.description as role_status_desc, a.role_status
					...
					if ($row_cnt_supervisor>0) {
						do {
							...
							<tr>
								...
								WHERE pg_supervisor_empid = '$employeeId'
								AND pg_amendment_id = '$mid'";
...
<? 
	...
	AND confirm_by IS NULL
	AND student_matrix_no = '$user_id'
	AND amendment_status IN ('REC','SAV', 'SUB')
