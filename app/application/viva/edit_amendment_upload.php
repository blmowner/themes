	...
	if (is_uploaded_file($xpic_file))
	{
		...
		|| $FileType == 'jpg' || $FileType == 'xlsx' || $FileType == 'pptx' || $FileType == 'ppt')
		{
			...
			if (($db->query($sql2)) && @ mysql_affected_rows() == 1) 
			{
				...
				echo "opener.location.href=\"amendment_edit_attachment.php?pid=$amendmentId&tid=$pgThesisId&ppid=$proposalId&ref=$referenceNo&mid=$mid\";";
...
<form name="frmEmpUpdPic" method="post" action="../viva/edit_amendment_upload.php?pid=<?=$amendmentId?>&mid=<?=$mid?>&tid=<?=$pgThesisId?>&ppid=<?=$proposalId?>&ref=<?=$referenceNo?>" enctype="multipart/form-data">
