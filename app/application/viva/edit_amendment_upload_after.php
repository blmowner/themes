	...
	if (is_uploaded_file($xpic_file))
	{
		...
		|| $FileType == 'jpg' || $FileType == 'xlsx' || $FileType == 'pptx' || $FileType == 'ppt')
		{
			...
			if (($db->query($sql2)) && @ mysql_affected_rows() == 1) 
			{
				...
				echo "opener.location.href=\"amendment_edit_attachment_after.php?pid=$amendmentId&tid=$pgThesisId&ppid=$proposalId&ref=$referenceNo&mid=$mid\";";
				echo "document.location.href=\"edit_amendment_receipt.php?data=$fileContent&status=T&userId=$user_id&fucd=$upload_id\";";
				echo "</script>";
				//header("Location: new_proposal_receipt.php?data=$fileContent&status=T&userId=$user_id&fucd=$upload_id");
			}
			else 
			{
				header("Location: edit_amendment_receipt.php?status=F&userId=$user_id&fucd=$upload_id");
			}
		}
		else
		{
...
<form name="frmEmpUpdPic" method="post" action="../viva/edit_amendment_upload_after.php?pid=<?=$amendmentId?>&mid=<?=$mid?>&tid=<?=$pgThesisId?>&ppid=<?=$proposalId?>&ref=<?=$referenceNo?>" enctype="multipart/form-data">
