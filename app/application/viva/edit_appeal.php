...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	{
		var ask = window.confirm("Are you sure to submit appeal? \nClick OK to proceed or CANCEL to stay on the same page.");
		if (ask) 
		{
...
if(isset($_POST['btnsave']) && ($_POST['btnsave'] <> ""))
{
	...
	$appealId = $_REQUEST['appealId'];
	$newAppealId = "A".runnum2('id','pg_viva_appeal');
	
	$lock_tables="LOCK TABLES pg_viva_appeal WRITE"; //lock the table
	$db->query($lock_tables);
	
	if(empty($appealId)) {
		$sql7 = "INSERT INTO pg_viva_appeal
		(id, appeal_desc, date_request, pg_viva_id, insert_by, insert_date, STATUS, student_matrix_no)
		VALUES
		('$newAppealId','$content','$curdatetime','$pgVivaId','$user_id','$curdatetime','SAV', '$user_id')";
					
		$db->query($sql7);
	} else {		
	
		$sql7 = "UPDATE pg_viva_appeal
		SET modify_by = '$user_id', modify_date = '$curdatetime', date_request = '$curdatetime', appeal_desc = '$content'
		WHERE id = '$appealId'
...
if(isset($_POST['btnSubmit']) && ($_POST['btnSubmit'] <> ""))
{
	...
	$msg = array();
	if (empty($_REQUEST['content'])) $msg[] = "<div class=\"error\"><span>Please insert the content.</span></div>";
	...
	if (empty($msg)) {
		...
		if(empty($appealId)) {
			...
			(id, appeal_desc, date_request, pg_viva_id, insert_by, insert_date, STATUS, student_matrix_no)
			VALUES
			('$newAppealId','$content','$curdatetime','$pgVivaId','$user_id','$curdatetime','SUB', '$user_id')";
						
			$db->query($sql7);
		} else {		
		
			$sql7 = "UPDATE pg_viva_appeal
			SET modify_by = '$user_id', modify_date = '$curdatetime', date_request = '$curdatetime', appeal_desc = '$content', status = 'SUB'
			WHERE id = '$appealId'
			AND pg_viva_id = '$pgVivaId'
			AND student_matrix_no = '$user_id'";
		...
		$msg = array();
		$msg[] = "<div class=\"success\"><span>Your Appeal has has been submit successfully.</span></div>";		$msg = array();
		...
		if ($msgConstValue == 'Y')
		{
			include("../../../app/application/inbox/viva/review_appeal_faculty.php");
		}
		...
		{
			include("../../../app/application/email/viva/email_review_appeal_faculty.php");
		}
		$save = "2";
		echo "<script>window.location = 'view_appeal.php?tid=".$pgThesisId."&pid=".$proposalId."&vid=".$pgVivaId."&save=1';</script>";	
...
function newAttachment(tid,pid,mn,role,vid, vd) {
	...
	<table>
		<tr>
			<td>Content:</td>
			<td><textarea class="ckeditor" name="content" id="content"><?=$appeal_desc?></textarea></td>
		</tr>
	</table>
<?
