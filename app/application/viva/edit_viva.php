...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<link rel="stylesheet" type="text/css" href="../../../../20151011-backpostgrad/app/theme/css/colorbox.css" media="screen" />
		<link rel="stylesheet" href="../../../../20151011-backpostgrad/app/lib/js/datePicker/jquery-ui-1.8.11.custom.css" />    
		<script src="../../../../20151011-backpostgrad/app/lib/js/jquery.min2.js"></script>
		<script src="../../../../20151011-backpostgrad/app/lib/js/jquery.colorbox.js"></script>
		<script src="../../../../20151011-backpostgrad/app/lib/js/jquery.mask_input-1.3.js"></script>
		<script type="text/javascript" src="../../../../20151011-backpostgrad/app/lib/js/datePicker/jquery-ui-1.8.11.custom.min.js"></script>
		<script src="../../../../20151011-backpostgrad/app/lib/js/datePicker/jquery.ui.core.js"></script>
		<script src="../../../../20151011-backpostgrad/app/lib/js/datePicker/jquery.ui.widget.js"></script>
		<script src="../../../../20151011-backpostgrad/app/lib/js/datePicker/jquery.ui.datepicker.js"></script>
		<script type="text/javascript" src="../../../../20151011-backpostgrad/lib/js/ckeditor/ckeditor.js"></script>
		<script language="JavaScript" src="../../../../20151011-backpostgrad/lib/js/windowopen.js"></script>
...
if($save == '1')
{
	$msg[] = "<div class=\"success\"><span>Your thesis has been save successfully.</span></div>";
}
...
{	
	...
	if($totalA > 0)
	{
		...
		$sql7 = "UPDATE pg_viva
				...
				, submit_date = '$curdatetime', pg_calendar_id = '$recSche'
				WHERE id = '$vivaId'
		...
		foreach($supervisorIdArray as $empid)
		{
			...
			if($respond_status != 'CON')
			{
				...
				SET modify_by = '$user_id', modify_date = '$curdatetime', pg_viva_detail_id = '$vivadetailIdNew'
				WHERE pg_viva_id = '$vivaId'
				AND insert_by= '$empid'
				...
				VALUES
				('$vivadetailIdNew','$vivaId','$empid','$user_id','$curdatetime','A', 'SUB')";
		...
		$save = "1";
		echo "<script>window.location = 'view_viva.php?tid=".$thesisId."&pid=".$proposalId."&save=".$save."&vid=".$vivaId."';</script>";	
	}
	else {
		
		$msg[] = "<div class=\"error\"><span>Please upload thesis by attachment below.</span></div>";
...
if(isset($_POST['btnSave']) && ($_POST['btnSave'] <> ""))
{
	...
	$sql7 = "UPDATE pg_viva
			SET insert_by = '$user_id', insert_date = '$curdatetime', modify_by = '$user_id', modify_date = '$curdatetime',
			pg_calendar_id = '$recSche'
			WHERE id = '$vivaId'
			AND pg_thesis_id = '$pgThesisId'
			AND status = 'A'
			AND student_matrix_no = '$user_id'
...
AND (DATE_FORMAT(defense_date,'%Y-%m-%d') > DATE_FORMAT(NOW(),'%Y-%m-%d'))
AND id NOT IN (
...
$sql_viva = "SELECT id AS pgVivaId, viva_status, submit_status, submit_date, session_no ,pg_calendar_id
FROM pg_viva
WHERE pg_thesis_id = '$pgThesisId'
AND pg_proposal_id = '$proposalId'
AND student_matrix_no = '$user_id'
AND viva_status IS NULL
AND submit_status IN ('SAV', 'SUB', 'REQ') OR submit_status IS NULL
AND STATUS = 'A'
AND id = '$vivaId'
AND viva_status is null";
...
$pg_calendar_id= $dba->f('pg_calendar_id');
?>
...
function newAttachment(vid,tid,pid) {
	...
	<? if(!empty($pgVivaId)) { ?>
			...
			<table width="75%">
				...
				<tr>
					<td width="15%">Thesis / Project ID</td>
					<td width="1%">:</td>
				  <td width="84%"><label><?=$thesisId;?></label><input type="hidden" name="thesisTitle" id="thesisTitle" value="<?=$thesisTitle; ?>"></td>
				...
				<tr>
					...
					<? for ($ir=0; $ir<$inr; $ir++){ 
						...
						$recommStatus = $dba->f('recomm_status');
						?>
						<? if($pg_calendar_id == $recommendedId) { ?>
						<option selected="selected" value="<?=$recommendedId?>"><?=$vivaDate?>, <?=$vivaSTime?> to <?=$vivaETime?>, <?=$venue?></option>
						<? } else { ?>
			...
			<table border="1" style="border-collapse:collapse;" cellpadding="2" cellspacing="1" width="80%" class="thetable">			
					<tr>
						...
						<th width="6%" align="left">Feedback</th>
						...
						do {
							...
							<tr>
								...
								*/
								...
								<td><a href="../viva/view_viva_feedback.php?tid=<?=$thesisId;?>&pid=<?=$proposalId;?>&empid=<?=$employeeId;?>&mn=<?=$user_id?>&vd=<?=$pgVivaDetailId?>&vid=<?=$pgVivaId?>" name="thesisId" value="<?=$thesisId?>" title="View feedback"><img src="../images/view.jpg" width="45" height="30" style="border:0px;" title="View feedback"></a></td>
							...
							<? } else {?>
								...
								if ($respond_status == '' || empty($respond_status)) {
								?>
								...
								<? } else {
									?>								
									<!--<td align="left"><label><?=$defenseDetailDesc;?></label></td>--><td>Pending</td>
									<?
								}?>
...
?>
	<table>
		<tr>
			<td><button type="button" name="btnAttachment" value="Attachment" onClick="return newAttachment('<?=$pgVivaId?>', '<?=$thesisId?>', '<?=$proposalId?>')" >
