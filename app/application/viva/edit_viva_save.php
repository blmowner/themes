...
{	
	...
	if($totalA > 0)
	{
		...
		{	
			...
			if($respond_status != 'CON')
			{
				...
				AND (respond_status <> 'CON' OR respond_status IS NULL)";
	...
	$save = "1";
		...
		$msg[] = "<div class=\"error\"><span>Plese upload thesis.</span></div>";
...
AND id NOT IN (
SELECT b.pg_calendar_id FROM pg_viva b 
WHERE b.pg_thesis_id = 'T20150618002' 
AND b.student_matrix_no = '012014030387' AND b.submit_status IN ('SUB', 'CON'))
ORDER BY defense_date ASC";
...
function newAttachment(vid,tid,pid) {
   // var ask = window.confirm("Ensure your viva has been saved before proceed or otherwise the last change will be discarded.\nClick OK to proceed or CANCEL to stay on the same page.");
    //if (ask) {
		document.location.href = "../viva/viva_edit_attachment_save.php?vid=" + vid + "&tid=" +tid+"&pid=" +pid+"&save=1";
