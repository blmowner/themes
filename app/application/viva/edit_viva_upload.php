...
if (isset($_GET['vid'])) 
{
$vivaId = (get_magic_quotes_gpc()) ? $_GET['vid'] : addslashes($_GET['vid']);
}	
...
$xpic_desc = clean($xpic_desc, 50);
	...
	if (is_uploaded_file($xpic_file))
	{
		...
		{
			$lock_tables="LOCK TABLES file_upload_viva WRITE"; //lock the table
			...
			if (($db->query($sql2)) && @ mysql_affected_rows() == 1) 
			{
				...
				echo "opener.location.href=\"viva_edit_attachment.php?vid=$vivaId&tid=$pgThesisId&pid=$proposalId\";";
				echo "document.location.href=\"edit_viva_receipt.php?data=$fileContent&status=T&userId=$user_id&fucd=$upload_id\";";
				echo "</script>";
			...
			{
				header("Location: new_viva_receipt.php?status=F&userId=$user_id&fucd=$upload_id");
			}
		}
		else
		{
...
<form name="frmEmpUpdPic" method="post" action="../viva/edit_viva_upload.php?vid=<?=$vivaId?>&tid=<?=$pgThesisId?>&pid=<?=$proposalId?>&referenceNo=<?=$referenceNo?>" enctype="multipart/form-data">
