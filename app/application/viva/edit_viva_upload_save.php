	...
	if (is_uploaded_file($xpic_file))
	{
		...
		|| $FileType == 'jpg' || $FileType == 'xlsx' || $FileType == 'pptx' || $FileType == 'ppt')
		{
			...
			if (($db->query($sql2)) && @ mysql_affected_rows() == 1) 
			{
				...
				echo "opener.location.href=\"viva_edit_attachment_save.php?vid=$vivaId&tid=$pgThesisId&pid=$proposalId\";";
				echo "document.location.href=\"edit_viva_receipt_save.php?data=$fileContent&status=T&userId=$user_id&fucd=$upload_id\";";
				echo "</script>";
			...
			{
				header("Location: new_viva_receipt_save.php?status=F&userId=$user_id&fucd=$upload_id");
			}
		}
		else
		{
...
<form name="frmEmpUpdPic" method="post" action="../viva/edit_viva_upload_save.php?vid=<?=$vivaId?>&tid=<?=$pgThesisId?>&pid=<?=$proposalId?>&referenceNo=<?=$referenceNo?>" enctype="multipart/form-data">
