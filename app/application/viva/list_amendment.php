...
/*$sql_failed = "SELECT a.id, a.amendment_status, a.confirm_status as confirmMain, 
a.confirm_date, a.reference_no, a.pg_viva_id
FROM pg_amendment a
WHERE a.pg_thesis_id = '$thesisId'
AND a.pg_proposal_id = '$proposalId'
AND a.amendment_status IS NOT NULL
...
$sql_failed = "SELECT a.id, a.viva_status, d.description AS vivaStatDesc, a.pg_calendar_id, a.student_matrix_no, 
...
$dbFail->next_record();
$viva_status_fail=$dbFail->f('viva_status');

$row_cnt_failed = mysql_num_rows($result_sql_failed);
if($viva_status_fail == 'FAI') {
