...
ORDER BY defense_date ASC";
$dbfDate = $dbf;
$result_sqldate = $dbfDate->query($sqldate); 
$dbfDate->next_record();
$row_cnt_sqlDate = mysql_num_rows($result_sqldate);

$sql_viva = "SELECT a.id AS pgVivaId, a.viva_status, a.submit_status, DATE_FORMAT(a.submit_date,'%d-%b-%Y %h:%i%p') as submit_date , 
a.session_no , a.pg_calendar_id, a.viva_status, DATE_FORMAT(b.end_appeal_date,'%d-%b-%Y') as endAppealDate, 
DATE_FORMAT(b.end_appeal_date,'%Y-%m-%d') AS end_appeal_date 
FROM pg_viva a
...
AND a.insert_by IS NOT NULL
AND a.STATUS in ('ARC','A', 'ARC1')
AND a.viva_status = 'FAI'
/*AND b.end_appeal_date > NOW()*/";// 'ARC1'
...
?>	
	<? if ($row_cnt_supervisor > 0 ) {
				?>
				<fieldset>
					<legend><strong>Appeal Session </strong></legend>
					<table width="90%">
						<tr>
				...
				<div id = "tabledisplay" style="overflow:auto; height:100px;">
					<table border="1" style="border-collapse:collapse;" cellpadding="2" cellspacing="1" width="100%" class="thetable">			
							<tr>
								...
								<th width="11%" align="left">Thesis Submit Date</th>
								<th width="17%" align="left">Evaluation Date </th>
								<th width="12%" align="left">Appeal Result</th>
								<th width="12%" align="left">End of Appeal Date</th>
							...
							if ($row_cnt_viva > 0) {
								...
								$viva_status = $dba->f('viva_status');
								$endAppealDate = $dba->f('endAppealDate');
								$end_appeal_date = $dba->f('end_appeal_date');
								...
								$sql_appeal="SELECT appeal_desc, date_request, result_appeal, respond_date, respond_by, student_matrix_no, status
								FROM pg_viva_appeal
								WHERE pg_viva_id = '$pgVivaId'
								AND student_matrix_no = '$user_id'
								AND STATUS IN ('SAV','SUB')";
								
								$dbr = $dbu;
								$result_sql_appeal = $dbr->query($sql_appeal);
								$dbr->next_record();
								
								$rowappeal = mysql_num_rows($result_sql_appeal);
								
								$date_request = $dbr->f('date_request');
								$result_appeal = $dbr->f('result_appeal');
								$respond_date = $dbr->f('respond_date');
								$respond_by = $dbr->f('respond_by');
								$statusAppeal = $dbr->f('status');
								...
								<tr>
									...
									<? 
									if($rowappeal > 0) {
										if($statusAppeal == 'SAV' && $end_appeal_date > $curdatetime)
										{
											$statDesc = "Save in Draft";
										}
										else if($statusAppeal == 'SUB') {
											$statDesc = "Submitted";
										}
										else if ($statusAppeal == 'SAV' && $end_appeal_date < $curdatetime) {
											$statDesc = "Appeal session ended";
										}
										...
										} else if(empty($result_appeal)){
											$resDesc = "";
										}
										
									} else {
										
										if(empty($statusAppeal) && $end_appeal_date > $curdatetime){
											$statDesc = "Not Submitted";
										}
										else if(($end_appeal_date < $curdatetime) && empty($statusAppeal)) {
											$statDesc = "Due date for appeal has ended";
											$statDesc = "Appeal session ended";
										}
										if(empty($result_appeal)){
											$resDesc = "";
										}
									}
									...
									?>
									<td><?=$resDesc?></td>
									<td align="left"><?=$endAppealDate?></td>
									<td><?=$statDesc?></td>
									...
									if ($statusAppeal =='SAV' && $end_appeal_date > $curdatetime) { ?>
										...
										onclick="javascript:document.location.href='edit_appeal.php?pid=<?=$proposalId;?>&tid=<?=$thesisId;?>&vid=<?=$pgVivaId;?>'"/>
								  <!--<a href="edit_amendment.php?pid=<?=$proposalId;?>&tid=<?=$thesisId;?>&ref=<?=$referenceNo;?>&mid=<?=$amendmentId?>">Edit</a>--></td>
									<? } else if (empty($statusAppeal) && $end_appeal_date > $curdatetime) { ?>
										<td width="5%" align="center">
										<input type="button" name="btnNew" id="btnNew" value="Add" 
										onclick="javascript:document.location.href='new_appeal.php?pid=<?=$proposalId;?>&tid=<?=$thesisId;?>&vid=<?=$pgVivaId;?>'" />
									...
									<? } else if ($statusAppeal =='SUB'){ ?>
										...
										onclick="javascript:document.location.href='view_appeal.php?id=<?=$id;?>&pid=<?=$proposalId;?>&tid=<?=$thesisId;?>&vid=<?=$pgVivaId;?>'" />
										<!--<a href="view_amendment.php?id=<?=$id;?>&pid=<?=$proposalId;?>&tid=<?=$thesisId;?>&ref=<?=$referenceNo;?>">View</a>-->
								  </td>
									<? } else if($end_appeal_date < $curdatetime && empty($statusAppeal)) {?>
										<td></td>
									<? } else if($end_appeal_date < $curdatetime && $statusAppeal == 'SAV') {?>
										<td align="center"><input type="button" name="btnView" id="btnView" value="View" 
										onclick="javascript:document.location.href='view_appeal.php?id=<?=$id;?>&pid=<?=$proposalId;?>&tid=<?=$thesisId;?>&vid=<?=$pgVivaId;?>'" /></td>
									<? }?>
								</tr>
					...
					<table>
						<tr>
							...
							1. Appeal can only be apply when school board has submit thesis final result.<br />
							2. If appeal result is approved by faculty, you need to submit amendment on thesis and 
							<br />wait for the faculty to finalize the amendment.
							Then you can submit new thesis for viva</label></td>
						</tr>
					</table>	
				</fieldset>
				<?
