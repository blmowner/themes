...
$sql_viva = "SELECT id AS pgVivaId, viva_status, submit_status, DATE_FORMAT(submit_date,'%d-%b-%Y %h:%i%p') as submit_date , 
session_no , pg_calendar_id, viva_status
FROM pg_viva
...
?>	
	<? if ($row_cnt_supervisor > 0 ) {
				?>
				<fieldset>
					<legend><strong>Thesis Submission  </strong></legend>
					<table width="90%">
						<tr>
				...
				<div id = "tabledisplay" style="overflow:auto; height:100px;">
					<table border="1" style="border-collapse:collapse;" cellpadding="2" cellspacing="1" width="95%" class="thetable">			
							<tr>
								...
								<th width="11%" align="left">Submit Date</th>
								<th width="17%" align="left">Evaluation Date </th>
								<th width="15%" align="left">Result</th>
							...
							if ($row_cnt_viva > 0) {
								...
								<tr>
									...
									$sql_result="SELECT  description from ref_recommendation where id = '$viva_status'";
									$dbgResult = $dbg;
									$result_sql_result = $dbgResult->query($sql_result);
									$dbgResult->next_record();
									
									$resDesc = $dbgResult->f('description');
									...
									if ($submit_status =='SAV') { ?>
										...
										onclick="javascript:document.location.href='edit_viva_save.php?pid=<?=$proposalId;?>&tid=<?=$thesisId;?>&vid=<?=$pgVivaId;?>'"/>
								  <!--<a href="edit_amendment.php?pid=<?=$proposalId;?>&tid=<?=$thesisId;?>&ref=<?=$referenceNo;?>&mid=<?=$amendmentId?>">Edit</a>--></td>
									<? } else if ($submit_status == 'REQ' || ($submit_status == 'SUB' && $rowReq > 0)) { ?>
										<td width="4%" align="center">
										<input type="button" name="btnUpdate" id="btnUpdate" value="Edit" 
										onclick="javascript:document.location.href='edit_viva.php?pid=<?=$proposalId;?>&tid=<?=$thesisId;?>&vid=<?=$pgVivaId;?>'" />
										<!--<a href="edit_amendment.php?pid=<?=$proposalId;?>&tid=<?=$thesisId;?>&ref=<?=$referenceNo;?>&mid=<?=$amendmentId?>">Edit</a>-->
								  </td>
									<? } else if ($submit_status == 'CON' || ($submit_status == 'SUB' && $rowReq ==0)){ ?>
										<td width="4%" align="center">
										<input type="button" name="btnView" id="btnView" value="View" 
										onclick="javascript:document.location.href='view_viva.php?id=<?=$id;?>&pid=<?=$proposalId;?>&tid=<?=$thesisId;?>&vid=<?=$pgVivaId;?>'" />
...
$sql_viva2 = "SELECT id AS pgVivaId, viva_status, submit_status, DATE_FORMAT(submit_date,'%d-%b-%Y %h:%i%p') as submit_date , 
session_no , pg_calendar_id, appeal_result
FROM pg_viva
WHERE pg_thesis_id = '$thesisId'
AND pg_proposal_id = '$proposalId'
AND student_matrix_no = '$user_id'
...
?>					
					<table>
						<tr>
							...
							if($row_cnt5>0) {
							//do {
								$pgVivaId1 = $dba4->f('pgVivaId');
								$viva_status1 = $dba4->f('viva_status');
								$submit_status1 = $dba4->f('submit_status');
								$submit_date1 = $dba4->f('submit_date');
								$session_no1 = $dba4->f('session_no');
								$calendarIdViva1 = $dba4->f('pg_calendar_id');
								$appeal_result = $dba4->f('appeal_result');
								...
								if($viva_status1 == 'PMR'){ 
										if($confirmMain == 'CON2') { ?>
								<input type="button" name="btnNewReport" value="New Thesis Submission" onclick="return newReport('<?=$thesisId?>', '<?=$proposalId?>')">
								<?		}
								} else if($row_cnt_viva == 0) { ?>
								<input type="button" name="btnNewReport" value="New Thesis Submission" onclick="return newReport('<?=$thesisId?>', '<?=$proposalId?>')">
							<?	} else if ($appeal_result == 'APP') { 
										if($confirmMain == 'CON2') {?>
								<input type="button" name="btnNewReport" value="New Thesis Submission" onclick="return newReport('<?=$thesisId?>', '<?=$proposalId?>')">
							<?			}
								}
					...
					<table width="1085">
						<tr>							
							<td width="545"><span style="color:#FF0000"> Notes:</span><br/>
							Possibility New Thesis Submission button is not appear: <br />
						  1. Your supervisor not recommend the schedule date yet.<br />
						  2. If your appeal has been approved, the previous amendment is not confirmed by faculty yet or not been completed yet.</td>
						</tr>
				</table>		
				</fieldset>
				<?
...
<? 
	...
	echo "<span style=\"color:red;\">Notes</span>:-<br />1. Please verified with your main supervisor regarding date of viva evaluation.";	
} 
