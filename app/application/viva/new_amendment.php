...
if(isset($_POST['btnAdd']) && ($_POST['btnAdd'] <> ""))
{
	...
	if(empty($msg))
	{
		if(empty($amendmentId))
		{
			...
			$lock_tables="LOCK TABLES pg_amendment WRITE, pg_amendment_detail WRITE, file_upload_amendment WRITE"; //lock the table
			...
			VALUES
			('$amendmentIdNew', '$thesisId', '$reference_no', '$proposalId', 'A', 'SAV', '$user_id', '$user_id', '$curdatetime', '$vivaId')";
			
			$dba->query($sqlamend);

			$sqlamendetail = "INSERT INTO pg_amendment_detail 
			(id, pg_amendment_id, feedback_by_examiner, status, insert_by, insert_date, pg_thesis_id, student_matrix_no, amendment_detail_status)
			VALUES
			('$amendmentDetailId', '$amendmentIdNew' , '$feedbackEE', 'A', '$user_id', '$curdatetime', '$thesisId', '$user_id', 'SAV')";
...
$sql4 = "SELECT a.id, b.id as vivaId
FROM pg_calendar a
	...
	<form id="form1" name="form1" method="post" enctype="multipart/form-data">	
			...
			<table width="52%">
				...
				<tr>
					...
					<input type="hidden" name="vivaId" id="vivaId" value="<?=$vivaId; ?>">
					...
					if ($row_cnt_supervisor>0) {
						do {
							...
							<tr>
								...
								<td align="left"><?=$employeeId;?>
								<input type="hidden" name="empid[]" id="empid" value="<?=$employeeId?>" /></td>
...
<? // Call list of amendment
	...
	AND pg_thesis_id = '$thesisId'
	AND student_matrix_no = '$user_id'
	AND amendment_detail_status IN ('SAV', 'CON') 
	AND confirm_by IS NULL
	...
	<table width="64%" class="thetable" border="1">
		<tr>
		...
		<? if($row_cnt5>0) {?>
			...
			<tr>
				...
				<td align="left"><textarea cols="40" name="feedbackAdd[]" id="feedbackAdd"><?=$feedbackByExaminerArray[$i]?></textarea></td>
