	...
	if (is_uploaded_file($xpic_file))
	{
		...
		|| $FileType == 'jpg' || $FileType == 'xlsx' || $FileType == 'pptx' || $FileType == 'ppt')
		{
			...
			if (($db->query($sql2)) && @ mysql_affected_rows() == 1) 
			{
				...
				echo "opener.location.href=\"amendment_attachment.php?pid=$amendmentId&tid=$pgThesisId&pidd=$proposalId&referenceNo=$referenceNo\";";
				echo "document.location.href=\"new_amendment_receipt.php?data=$fileContent&status=T&userId=$user_id&fucd=$upload_id\";";
				echo "</script>";
			...
			{
				header("Location: new_amendment_receipt.php?status=F&userId=$user_id&fucd=$upload_id");
			}
		}
		else
		{
...
<form name="frmEmpUpdPic" method="post" action="../viva/new_amendment_upload.php?pid=<?=$amendmentId?>&pid=<?=$amendmentId?>&tid=<?=$pgThesisId?>&pidd=<?=$proposalId?>&referenceNo=<?=$referenceNo?>" enctype="multipart/form-data">
