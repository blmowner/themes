	...
	if (is_uploaded_file($xpic_file))
	{
		...
		|| $FileType == 'jpg' || $FileType == 'xlsx' || $FileType == 'pptx' || $FileType == 'ppt')
		{
			...
			if (($db->query($sql2)) && @ mysql_affected_rows() == 1) 
			{
				...
				echo "opener.location.href=\"viva_attachment.php?vid=$vivaId&tid=$pgThesisId&pid=$proposalId\";";
				echo "document.location.href=\"new_viva_receipt.php?data=$fileContent&status=T&userId=$user_id&fucd=$upload_id\";";
				echo "</script>";
...
<form name="frmEmpUpdPic" method="post" action="../viva/new_viva_upload.php?vid=<?=$vivaId?>&tid=<?=$pgThesisId?>&pid=<?=$proposalId?>&referenceNo=<?=$referenceNo?>" enctype="multipart/form-data">
