...
require("../../../lib/fpdf/viva_report_summary_table.php");
...
$pdf->Cell(0,9,'Summary for VIVA Exam Report',1);
$pdf->Ln(10);
...
//////////////////////////SECTION B///////////////////////////////
	...
	$pdf->SetX (20);
	$pdf->SetFont("Arial","B","9");
	$pdf->Cell(70,5,"RECOMMENDATION",0,0);
	$pdf->ln();

	$pdf->SetFont('Arial','B',9);
	//Table with 3 rows and 3 columns
	$pdf->SetWidths(array(7,125,48));
	$pdf->Row(array("No","Components","Recomendations"));
	$pdf->SetFont('Arial','',9);		
...
$header11 = $dbg5->f('description');
	...
	$pdf->SetX (20);
	$pdf->SetFont("Arial","B","9");
	$pdf->Cell(45,5,"Prepared By: ",1,0);
	$pdf->SetFont("Arial","","9");
	$pdf->Cell(0,5,$empName." (".$empid.")",1,1);
	$pdf->SetX (20);
	$pdf->SetFont("Arial","B","9");
	$pdf->Cell(45,5,"Role:",1,0);
	$pdf->SetFont("Arial","","9");
	$pdf->Cell(0,5,$header11 ,1,1);
	$pdf->SetX (20);
	$pdf->SetFont("Arial","B","9");
...
///////////////////////////////////////////////////////////
	...
	$pdf->SetX (20);
	$pdf->SetFont("Arial","B","9");
	$pdf->Cell(70,5,"NOTE",0,0);
	$pdf->Ln();
	$pdf->SetX (20);
	$pdf->SetFont("Arial","","9");
	$pdf->Cell(70,5,"Based on the evaluation from VIVA committee,",0,0);
	$pdf->Ln();
	$pdf->SetX (30);
	$pdf->SetFont("Arial","","9");
	$pdf->Cell(70,5,"- Submission time is 3 months for Pass with minor correction and ",0,0);
	$pdf->Ln();
	$pdf->SetX (30);
	$pdf->SetFont("Arial","","9");
	$pdf->Cell(70,5,"- Submission time is 6 months for Pass with major correction and ",0,0);
	$pdf->Ln();
	$pdf->Ln();
	$pdf->SetX (20);
	$pdf->SetFont("Arial","","9");
	$pdf->MultiCell(0,5,"Student is subjected to the Approval from Supervisor on your Amendments of thesis and put the condition in your Amendments based on the comments from the External Examiners and VIVA Committee (Please specify tha page number)",0);
	$pdf->Ln();	
	$pdf->SetX (20);
	$pdf->SetFont("Arial","","9");
	$pdf->Cell(70,5,"Student need to read carefully and complete all changes given by External Examiner and VIVA Committee.",0,0);
	$pdf->Ln();
	$pdf->Ln();
	$pdf->SetX (20);
	$pdf->SetFont("Arial","","9");
	$pdf->MultiCell(0,5,"If the student have any objections or disagree with the Examiner's report and VIVA Committee, please write the Official Letter with the justification and sent to the MSU.",0);
	$pdf->Ln();
	$pdf->SetX (20);
	$pdf->SetFont("Arial","","9");
	$pdf->MultiCell(0,5,"MSU Senate will appoint to another group of the Examiner for re-exam of the thesis. This process will take the minimun of 6 month if any of objection from student.",0);
	$pdf->Ln();
	$pdf->SetX (20);
	$pdf->SetFont("Arial","","9");
	$pdf->MultiCell(0,5,"Therefore, please complete all Amendments given by your External Examiners and VIVA Committee accordingly and if should you require further clarification, please do not hesitate to contact Graduate School of Management (GSM), MSU.",0);

$pdf->Output('viva_summarry_'.$empid.'('.$matrixNo.').pdf','I');
close();
?>
