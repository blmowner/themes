...
$sqlamend = "SELECT a.id,a.amendment_by_examiner AS amendment, a.feedback_by_examiner, a.amendment_detail_status, 
 ...
 $sqlamend2 = "SELECT a.id,a.feedback_by_examiner,a.amendment_confirm_status As amendmentConfirmStatus
	FROM pg_amendment_detail a
	WHERE a.pg_amendment_id = '$amendmentId'
	AND a.pg_thesis_id = '$thesisId'
	AND a.student_matrix_no = '$matrixNo'
	...
	$id1 = $dbb->f('id');
	$feedbackByExaminerArray1 = array();
	$amendmentConfirmStatusArray1 =array();
	$commentArray1 = array();
	$result_sqlamend2 = $dbb->query($sqlamend2); 
	$dbb->next_record();
	$row_cnt5 = mysql_num_rows($result_sqlamend2);
	$i= 0;
	$inc= 0;

	do{
		
		$amendmentConfirmStatusArray1[$i]=$dbb->f('amendmentConfirmStatus');
		$feedbackByExaminer1 = $dbb->f('feedback_by_examiner');
		$id1 = $dbb->f('id');
		$idArray1[$i] = $id1;
		$amendmentArray1[$i] =$amendment;
		$feedbackByExaminerArray1[$i] =$feedbackByExaminer1;
		$amendment_detail_statusArray1[$i] =$amendment_detail_status;
		
		$inc++;
		$i++;
//echo $feedbackByExaminer1;
	}while($dbb->next_record());
	...
	echo $sql_employee="SELECT  b.name, c.id, c.description
		FROM new_employee b 
...
$sql13 = "SELECT a.confirm_status, a.comment, a.id AS commentId
	...
	$sql133 = "SELECT a.comment, a.id AS commentId
	FROM pg_amendment_review a
	WHERE pg_amend_detail_id = '$idArray1[$i]'
	AND a.empid = '$employeeId'
	AND a.comment_status IN ('SUB', 'SAV')
	AND a.status = 'A' OR a.status IS NULL";
	$db2 = $db;
	$result_sql133 = $db2->query($sql133); 
	$db2->next_record();
	$comment3=$db2->f('comment');
	$commentId3=$db2->f('commentId');
	...
	$sql_name_sp = "SELECT name from new_employee where empid = '$employeeId'";
			...
			if($amendmentConfirmStatusArray[$i] == "CON1") 
				{ 
					if ($fal_id == Null){
						$pdf->Row2(array(($i+1).".",$feedbackByExaminerArray[$i],$amendmentArray[$i],$employeeName."\n(".$employeeId.")\nRemark: ".$comment3,"Remark: Not Verify"));
						}
						else
						{
						$pdf->Row2(array(($i+1).".",$feedbackByExaminerArray[$i],$amendmentArray[$i],$employeeName."\n(".$employeeId.")\nRemark: ".$comment3,$fal_name."\n".$fal_id."\nRemark: ".$facultyRemark[$i]));
						}
						}
			else if ($amendmentConfirmStatusArray1[$i] == "CON")
			{
				$pdf->Row2(array(($i+1).".",$feedbackByExaminerArray1[$i],$amendmentArray[$i],$employeeName."\n(".$employeeId.")\nRemark:".$comment3."\nStatus: Proceed to Amendment", "Remark: Not Verify"));
			}
			else{
			$pdf->Row2(array(($i+1).".",$feedbackByExaminerArray1[$i],$amendmentArray[$i],"Remark: Not Confirmed", "Remark: Not Verified"));
			}
