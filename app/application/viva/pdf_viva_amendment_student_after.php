...
$sqlamend2 = "SELECT a.id,a.feedback_by_examiner,a.amendment_confirm_status As amendmentConfirmStatus
	...
	$dbb->next_record();
	$row_cnt51 = mysql_num_rows($result_sqlamend2);
	$i= 0;
...
$sql13 = "SELECT a.confirm_status, a.comment, a.id AS commentId
	...
	$sql_name_sp = "SELECT name from new_employee where empid = '$employeeId'";
			...
			if($amendmentConfirmStatusArray[$i] == "CON1") 
				{ 
					if ($fal_id == Null){
						$pdf->Row2(array(($i+1).".",$feedbackByExaminerArray[$i],$amendmentArray[$i],$employeeName."\n(".$employeeId.")\nRemark: ".$comment."\nStatus: Verified","Remark: Not Verified"));
						}
						else
						{
						$pdf->Row2(array(($i+1).".",$feedbackByExaminerArray[$i],$amendmentArray[$i],$employeeName."\n(".$employeeId.")\nRemark: ".$comment."\nStatus: Verified",$fal_name."\n".$fal_id."\nRemark: ".$facultyRemark[$i]."\nStatus: Verified"));
						}
						}
		else if ($amendmentConfirmStatusArray[$i] == "CON")
			{
				$pdf->Row2(array(($i+1).".",$feedbackByExaminerArray[$i],$amendmentArray[$i],$employeeName."\n(".$employeeId.")\nRemark: ".$comment."\nStatus: Not Verified","Remark: Not Verified"));
			}
			else{
