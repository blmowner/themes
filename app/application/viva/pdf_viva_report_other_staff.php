...
$pdf->Ln(10);
	...
	$pdf->Row(array("Student's Name",$studentName));
	
	$pdf->SetFont('Arial','B',9);
	$pdf->SetWidths(array(35,145));
	$pdf->Row(array("Matrix Number",$matrixNo));
	
	$pdf->SetFont('Arial','B',9);
	$pdf->SetWidths(array(35,145));
	$pdf->Row(array("Programme",$program_code." - ".$program_e));
	
	$pdf->SetFont('Arial','B',9);
	$pdf->SetWidths(array(35,145));
	$pdf->Row(array("Thesis Topic",$thesisId." - ".$thesis_title));
...
//////////////////////////SECTION A///////////////////////////////	
	...
	$pdf->Row(array("No","Item Description","Comments", "Rating Description","Rating"));
...
$sqlrate = "SELECT * FROM ref_overall_viva_rating where ques_seq = '$seqArray[$i]' ORDER BY seq ASC ";
	...
	//for ($i=0;$i<17;$i++){
	$pdf->Row(array(($j+1).".",$descArray[$j],$comments[$j],"1 = ".$var1."\n"."5 = ".$var2,$rateA[$j]));	
	//}
...
//////////////////////////SECTION B///////////////////////////////
	...
	for ($no=0; $no<$noU; $no++){
		if($recommendation_id == $idRecArray[$no])
			{
			$pdf->Row(array(($no+1).".",$descRecArray[$no],"Recommended"));
			}
			else
			{
			$pdf->Row(array(($no+1).".",$descRecArray[$no],""));
			}
	...
	$pdf->SetFont("Arial","B","9");
	$pdf->Cell(120,5,"Components",1,0);
	$pdf->Cell(0,5,"Rating",1,1);
	
	for ($no1=0; $no1<$noC; $no1++){//ref_overall_comments
		$pdf->SetX (20);
		$pdf->SetFont("Arial","","9");							
		$pdf->Cell(10,5,($no1+1). ".",1,0,"C");
		$pdf->Cell(120,5,$descComArray[$no1],1,0);
		
		
			for ($no2=0; $no2<$noOR; $no2++){//ref_overall_rating
				if($ref_overall_rating_id[$no1] == $idOrArray[$no2]) {
						$pdf->Cell(0,5,$rateOrArray[$no2]. " - ".$descOrArray[$no2],1,1);
					}
					else {
					}
				}
...
/////////////////////////
	...
	for ($i=0;$i<$row_cnt17;$i++){ //ref_overall_comments
		...
		$pdf->Cell(120,5,$overallCommentsArray[$i],1,0);
			...
			for ($l=0;$l<$row_cnt2B;$l++) {//ref_overall_rating
				if ($overallCommentsIdArray[$i] == $eoRefOverallCommentsIdArray[$l]) {
					for ($k=0;$k<$row_cnt18;$k++) {//ref_overall_rating
						...
						$pdf->Cell(0,5,$overallRatingRateArray[$k]. " - ".$overallRatingDescArray[$k],1,1);
				...
				for ($j=0;$j<$row_cnt18;$j++) {
					...
					if ($overallRatingIdArray[$j] == $_REQUEST['addOverallRating'][$i])
						{
						$pdf->Cell(0,5,$overallRatingRateArray[$j]. " - ".$overallRatingDescArray[$j],1,1);
