...
?>
	...
	<input type="hidden" id="evaVivaId" name="evaVivaId" value="<?=$evaVivaId?>" />
		...
		<? for ($iq=0; $iq<$inq; $iq++){ ?>
		<table class="" width="100%" frame="box">
			<tr>
				<td rowspan="2"><strong><?=$descArray[$iq]?></strong></td>
				<? 
		...
		?>
		<? for ($ir=0; $ir<$inr; $ir++){ ?>
			<td width="120"><?=$descRateArray[$ir]?></td>
		...
		<? } ?>
		  ...
		  <tr>
			  <? for ($ir=0; $ir<$inr; $ir++){ ?>
					<?
					if($rateA[$iq] == $rateArray[$ir]) {
						?><td align="center"><?=$rateArray[$ir]?><input type="radio" disabled="disabled" checked="checked"/></td><?
					}
					else {
						?><td align="center"><?=$rateArray[$ir]?><input type="radio" disabled="disabled"/></td><?
					}
				 } ?>
			</tr>
			<tr>
			...
			<tr>
				<td colspan="6" height="100"><?=$comments[$iq]?></td>
			</tr>
			</table>
		<? }?>
...
?>
	...
	<table border="1" style="border-collapse: collapse;border-spacing: 0px;" width="100%">
		...
		<tr>
			...
			<td><?=$rateE[$no1]?>-<?=$ratingDesc[$no1]?></td>
