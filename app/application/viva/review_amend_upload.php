...
if (isset($_GET['pid'])) 
{
$amendmentId = (get_magic_quotes_gpc()) ? $_GET['pid'] : addslashes($_GET['pid']);
}	
...
$xpic_desc = clean($xpic_desc, 50);
	...
	if (is_uploaded_file($xpic_file))
	{
		...
		$upload_id = runnum('fu_cd','file_upload_amendment');
		...
		{
			$sql2 = "INSERT INTO file_upload_amendment (
			fu_cd,  
			...
			if (($db->query($sql2)) && @ mysql_affected_rows() == 1) 
			{
				...
				echo "opener.location.href=\"review_amendment_attachment.php?pid=$amendmentId&tid=$pgThesisId&ppid=$proposalId&ref=$referenceNo&mid=$mid&role=$rolestatus&mn=$matrixNo\";";
				echo "document.location.href=\"review_amendment_receipt.php?data=$fileContent&status=T&userId=$user_id&fucd=$upload_id\";";
				echo "</script>";
				//header("Location: new_proposal_receipt.php?data=$fileContent&status=T&userId=$user_id&fucd=$upload_id");
			}
			else 
			{
				header("Location: review_amendment_receipt.php?status=F&userId=$user_id&fucd=$upload_id");
			}
		}
		else
		{
...
<form name="frmEmpUpdPic" method="post" action="../viva/review_amend_upload.php?pid=<?=$amendmentId?>&mid=<?=$mid?>&tid=<?=$pgThesisId?>&ppid=<?=$proposalId?>&ref=<?=$referenceNo?>&role=<?=$rolestatus?>&mn=<?=$matrixNo?>" enctype="multipart/form-data">
