...
$proposalId=$_GET['ppid'];
$amendmentId=$_GET['pid'];
$referenceNo=$_REQUEST['ref'];
$mid=$_GET['mid'];
...
{					
	if (sizeof($_POST['attachment_checkbox'])>0) {
		...
		while (list ($key,$val) = @each ($_POST['attachment_checkbox'])) 
		{
			...
			$sql1 = "UPDATE file_upload_amendment
...
{					
	if (sizeof($_POST['attachment_checkbox'])>0) {
		...
		while (list ($key,$val) = @each ($_POST['attachment_checkbox'])) 
		{
			...
			$sql1 = "DELETE FROM file_upload_amendment
			WHERE fu_cd = '$attachment_id'
			AND employee_id = '$user_id'";
...
?>
  ...
  <form id="form1" name="form1" method="post" enctype="multipart/form-data" onsubmit="return saveRec();">
	 ...
	 <fieldset>
		...
		<table>
			<tr>
				...
				<td><input type="submit" name="submit" value="Add Attachment" onclick="javascript:open_win('review_amend_upload.php?pid=<?=$amendmentId;?>&tid=<?=$pgThesisId;?>&ppid=<?=$proposalId;?>&ref=<?=$referenceNo;?>&mid=<?=$mid?>&role=<?=$rolestatus?>&mn=<?=$matrixNo?>',480,280,0,0,0,1,0,1,1,0,5,'winupload'); "/></td>
			</tr>
		</table>
		<table>
		<tr>							
				...
				if ($row_cnt > 0) {	
					...
					{ 
						...
						?><tr>
								...
								<td><input type="text" name="file_desc[]" id="file_desc" size="50" value="<?=$row["fu_document_filedesc"];?>" /></td>
								<td><label name="file_name[]" size="40" ></label><?=$row["fu_document_filename"];?></td>
								<td><a href="../viva/downloadamend.php?id=<?=$row["fu_cd"];?>&amp;al=S" target="_blank" onMouseOver="toolTip('<?=$row["fu_document_filename"];?>', 300)" onMouseOut="toolTip()" align="center">
	...
	<table>
		<tr>
			<td>
			<input type="button" name="btnBack" value="Back" onclick="javascript:document.location.href='../viva/review_amendment_staff_after.php?tid=<?=$pgThesisId?>&pid=<?=$proposalId?>&rid=<?=$referenceNo?>&mn=<?=$matrixNo?>&role=<?=$rolestatus?>&mid=<?=$mid?>';" />
			</td>
