...
if(isset($_POST['btnConfirmAll']) && ($_POST['btnConfirmAll'] <> ""))
{
	...
	if($rolestatus == 'PRI')
	{
		$lock_tables="LOCK TABLES pg_amendment WRITE,pg_amendment_review WRITE, pg_amendment_detail WRITE, pg_amendment_confirmation WRITE"; //lock the table
		$db->query($lock_tables);
			
		$sql1 = "UPDATE pg_amendment SET
			confirm_status = 'CON1', confirm_by = '$user_id', confirm_date = '$curdatetime', 
			...
			amendment_confirm_status = 'CON1', confirm_by = '$user_id', confirm_date = '$curdatetime',
			modify_by = '$user_id', modify_date = '$curdatetime1'
			...
			$sql1 = "INSERT INTO pg_amendment_confirmation
					...
					VALUES
					('$conId', '$amendmentId', 'CON1',  '$user_id', '$user_id', '$curdatetime', 'A', '$user_id', '$curdatetime')";
		...
		$db->query($lock_tables);
			...
			if ($msgConstValue == 'Y')
			{
				include("../../../app/application/inbox/viva/review_amendment_confirm_2.php");
			}
			
			///// message notification////
			$sqlmsgfac = "SELECT const_value FROM base_constant
			WHERE const_term = 'MESSAGE_SUP_TO_FAC'";
			
			$result_sqlmsgfac = $dbg->query($sqlmsgfac); 
			$dbg->next_record();
			$row_cnt_msg_face = mysql_num_rows($result_sqlmsgfac);
			$msgConstValueFac = $dbg->f('const_value');
			
			if ($msgConstValueFac == 'Y')
			{
				
				include("../../../app/application/inbox/viva/review_amendment_confirm_faculty.php");
			}
			...
			{
				include("../../../app/application/email/viva/email_review_amendment_confirm_2.php");
			}
			...
			if ($emailConstValue == 'Y')
			{
				include("../../../app/application/email/viva/email_review_amendment_confirm_faculty.php");
			}
...
if(isset($_POST['btnReq']) && ($_POST['btnReq'] <> ""))
{
	...
	if(empty($confirmIdFromOthers))
	{
		...
		$sqlamend = "UPDATE pg_amendment
					SET modify_by = '$user_id', modify_date = '$curdatetime', ref_req_no = '$reqChangesId', status = 'ARC', amendment_status = 'REQ1'
					WHERE id = '$amendmentId'";
		$dba->query($sqlamend);
		
		$sqlamend = "INSERT INTO pg_amendment
		(id, pg_thesis_id, reference_no, pg_proposal_id, status, amendment_status, student_matrix_no, insert_by, insert_date, submit_date, submit_status, pg_viva_id, pg_calendar_id, confirm_date, confirm_by, confirm_status)
		VALUES
		('$amendmentIdNew', '$thesisId', '$referenceNo', '$proposalId', 'A', 'REQ1', '$matrixNo', '$user_id', '$curdatetime', '$submitDate', 'SUB1', '$pg_viva_id', '$scheId', '$confirm_date', '$user_id', 'CON')";
		$dba->query($sqlamend);
		//////////////////////////////////////END////////////////////////////////////////////////////////////////////////
		...
		//////////////update amendmentid file/attachment student//////////////
		...
		////////////////////////END////////////////////////
		//////////////update amendmentid file/attachment supervisor//////////////
		$sqlUpload="SELECT * FROM file_upload_amendment
		WHERE amendment_id = '$amendmentId' 
		...
		$sqlamend = "SELECT a.id,a.amendment_by_examiner AS amendment, a.feedback_by_examiner, a.amendment_by_examiner,
		...
		AND a.amendment_detail_status IN ('SUB1','CON', 'CON1') 
		...
		do{
			...
			$amendment_by_examiner = $dbb->f('amendment_by_examiner');
			...
			(id, pg_amendment_id, feedback_by_examiner, amendment_by_examiner, status, insert_by, insert_date, pg_thesis_id, student_matrix_no, amendment_detail_status, amendment_confirm_status, confirm_by, confirm_date)
			VALUES
			('$amendmentDetailId', '$amendmentIdNew', '$feedbackByExaminer', '$amendment_by_examiner', 'A', '$user_id', '$curdatetime', '$thesisId', '$matrixNo', 'SUB', '$amendment_confirm_status', '$confirm_by', $confirm_date)";
	...
	$save = "1";
		echo "<script>window.location = 'view_amendment_staff_after.php?tid=".$thesisId."&pid=".$proposalId."&save=".$save."&rid=".$referenceNo."&mn=".$matrixNo."&role=".$rolestatus."&mid=".$amendmentIdNew."';</script>";			
		...
		do{
			...
			WHERE pg_amend_detail_id = '$id'
			AND a.empid = '$user_id'
			AND a.comment_status IN ('SAV', 'NULL')
			AND a.status = 'A' OR a.status IS NULL";
			$db2 = $db;
			$result_sql13 = $db2->query($sql13); 
			$db2->next_record();
		...
		if (sizeof($_POST['amendCheck'])>0) {
			while (list ($key,$val) = @each ($_POST['amendCheck'])) 
			{
				...
				if($rolestatus == 'PRI')
				{
					...
					SET amendment_confirm_status = 'CON1', modify_by = '$user_id', modify_date = '$curdatetime', confirm_by = '$user_id', confirm_date = '$curdatetime'
				...
				{
					$sql1 = "UPDATE pg_amendment_review
					SET confirm_status = 'CON1', modify_by = '$user_id', modify_date = '$curdatetime'
					WHERE pg_amend_detail_id = '$amendmendIdDetail'
					AND empid = '$user_id'
					AND comment_status = 'SAV'
					AND id = '$commentId'";
					...
					(id, pg_amendment_id, empid, pg_amend_detail_id, comment, comment_date, status, comment_status, insert_by, insert_date, confirm_status)
					VALUES
					('$reviewId', '$amendmentId', '$user_id',  '$amendmendIdDetail', '$comment', '$curdatetime', 'A', 'SAV', '$user_id', '$curdatetime', 'CON1')";
...
<? } ?>
	...
	{
		var ask = window.confirm("Are you sure to confirm all the amendment? \nClick OK to proceed or CANCEL to stay on the same page.");
		if (ask) 
		{
...
.thetable td {
	...
	<table width="91%" class="thetable" border="1">
		<tr>
		 ...
		 <input type="hidden" name="confirm_date" id="confirm_date" value="<?=$confirm_date?>" /> </th>
			<th align="center" width="3%">No</th>
			<th align="left" width="32%">Feedback of External Examiner</th>
			<th align="left"width="32%">Amendmenst Based on the comment<br />from External Examiner<br />(Please specify the page number)</th>
		...
		<? if($row_cnt5>0) {?>
			...
			<tr>
				...
				<!--<textarea cols="40" name="amendmentSave[]" id="amendmentSave"></textarea>
				...
				<td>
				<? if($amendmentConfirmStatusArray[$i] == 'CON1') {?>
				<textarea cols="26" rows="4" name="comment[]" id="comment" readonly="readonly"><?=$comment?></textarea>
				<? } else { ?>
				<textarea cols="26" rows="4" name="comment[]" id="comment"><?=$comment?></textarea>
				<? } ?>
				</td>
				<? if($amendmentConfirmStatusArray[$i] == 'CON1') { $confirmDesc = 'Confirmed';?>
				...
				<? } else if ($amendmentConfirmStatusArray[$i] == 'CON') { $confirmDesc = 'Not Confirmed';?>
				<td width="7%" align="center"><?=$confirmDesc?></td>
				<? } ?>
			</tr>
			<? } ?>
...
?>
	<table>
		<tr>
			...
			<td><button type="button" name="btnAttachment" value="Attachment" onClick="return newAttachment('<?=$amendmentId?>', '<?=$thesisId?>', '<?=$proposalId?>', '<?=$referenceNo?>', '<?=$matrixNo?>','<?=$rolestatus?>', '<?=$mid?>')" >
