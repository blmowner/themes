...
{
	$msg[] = "<div class=\"success\"><span>The thesis has been confirmed successfully.</span></div>";
}
...
<?
		...
		$sqlUpload1="SELECT * FROM file_upload_viva
		WHERE pg_viva_id = '$pgVivaId' 
		AND pg_viva_detail_id = '$pgVivaDetailId'
		AND insert_by = '$user_id'";			
		$dbh1 = $dbf;
		$result_sqlUpload1 = $dbh1->query($sqlUpload1); 
		$dbh1->next_record();
		$row_staff = mysql_num_rows($result_sqlUpload1);
		$is = 0;
		$incs = 0;
		do{
			$namefile1 = $dbh1->f('fu_document_filename');
			$fu_cd1 = $dbh1->f('fu_cd');
			$namefileStaff[$is] = $namefile1;
			$fu_cdStaff[$is] = $fu_cd1;
			$is++;
			$incs++;
...
?>
	...
	<table>
		...
		<tr>
			...
			<a href="downloadviva.php?id=<?=$fu_cdStaff[$is];?>" target="_blank"><?=$namefileStaff[$is] ?>
		 	<img src="../images/download.png" width="20" height="19" style="border:0px;" title="Download <?=$namefileStaff[$is]?>"></a>
