...
if(isset($_POST['btnSubmit']) && ($_POST['btnSubmit'] <> ""))
{
	...
	if(empty($msg)) 
	{
		if ($evaluationDetailStatus=="IN1") {
			...
			$msg[] = "<div class=\"success\"><span>Work Completion Report has been submitted successfully.</span></div>";
			...
			if ($supervisorTypeId =="EC") {
				...
				{
					include("../../../app/application/inbox/work/submit_work_school.php");
				}
				else {
					echo "Not sent";
				}
		...
		if ($evaluationDetailStatus1=="IN1" || $evaluationDetailStatus1=="SV1") {?>
			...
			<table>
				...
				<tr>
					...
					<td>:</td>
					<td><strong><?=$examinerDesc?>
					  <input type="hidden" name="roleType" id="roleType" value="<?=$supervisorTypeId?>" />
					</strong><? echo "LOL : $supervisorTypeId"?></td>
				</tr>				
				<tr>
					<td>Last Update</td>
					<td>:</td>
				...
				<?				
					...
					?>				
				<td><label><?=$sessionTypeDesc?> - <?=$defenseDate?>, <?=$defenseSTime?> to <?=$defenseETime?>, <?=$venue?>
				    <input type="hidden" name="calendarId" id="calendarId" value="<?=$recommendedId?>" />
				</label></td>				
			</tr>   				
			</table>				
			</br>
