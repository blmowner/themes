...
if(isset($_POST['btnSubmit']) && ($_POST['btnSubmit'] <> ""))
{
	...
	/////end D////////////
	$flexEndDate = date("Y-m-d H:i:s", strtotime($endDate));
	
	$date = strtotime("+".$dayEnd." day");
	$defaultEndDate = date('Y-m-d H:i:s', $date);
	$curdatetime = date("Y-m-d H:i:s");
	$curdatetime1 = date("Y-m-d H:i:s");
	
	$msg = array();
	if ($recCheck == 'FAI' && (empty($remark) || $remark == '')) $msg[] = "<div class=\"error\"><span>Please insert remark.</span></div>";
	...
	if(empty($msg)) {
		if($recCheck == 'FAI') {
			...
			if(!empty($endDate)) {
				$sql7 = "UPDATE pg_evaluation_viva SET final_result = '$recCheck',
				final_result_date = '$curdatetime',
				final_respond = '$user_id', school_remark = '$remark',
				end_appeal_date = '$flexEndDate'
				...
				final_result_date = '$curdatetime',
				final_respond = '$user_id', school_remark = '$remark',
				end_appeal_date = '$defaultEndDate'
				WHERE id = '$evaluationId'
				AND pg_thesis_id = '$thesisId'
				AND student_matrix_no = '$matrixNo'
				AND STATUS = 'A'";
		...
		if($recCheck == 'PAS')
		{
			...
			if ($row_cnt_amend>0){
				...
				SET status = 'ARC1', modify_date = '$curdatetime', modify_by = '$user_id'
				WHERE pg_thesis_id = '$thesisId'
				AND pg_proposal_id = '$proposalId'
				AND pg_viva_id = '$pg_viva_id'
				AND STATUS = 'A'
				...
				(id, pg_thesis_id, reference_no, pg_proposal_id, status, amendment_status, student_matrix_no, insert_by, insert_date, pg_viva_id)
				VALUES
				('$amendmentIdNew', '$thesisId', '$reference_no', '$proposalId', 'ARC1', 'SUB', '$matrixNo', '$user_id', '$curdatetime', '$pg_viva_id')";
		...
		if($recCheck == 'PMR')
		{
			$sql7 = "UPDATE pg_viva SET viva_status = '$recCheck',
		...
		if($row_cnt_result_sqlforsenate > 0) {
			...
			if($msgConstValue == 'Y')
			{
				include("../../../app/application/inbox/viva/school_senate.php");
			}
			...
			{
				//echo "sadsa";
				include("../../../app/application/email/viva/email_school_senate.php");
			}
		...
		WHERE const_term = 'MESSAGE_SCH_TO_FAC'";
		...
		if($msgConstValue == 'Y')
		{
			include("../../../app/application/inbox/viva/report_school_evaluation.php");
		}
		
		$sqlemail1 = "SELECT const_value FROM base_constant
		WHERE const_term = 'EMAIL_SCH_TO_FAC'";
		$dbg2 = $dbg;
		$result_sqlemail1 = $dbg2->query($sqlemail1); 
		$dbg2->next_record();
		$row_cnt_email1 = mysql_num_rows($result_sqlemail1);
		$emailConstValue1 = $dbg2->f('const_value');
		
		if ($emailConstValue1 == 'Y')
		{
			//echo "sadsa";
			include("../../../app/application/email/viva/email_report_school_evaluation.php");
		}
			
		
		
		$save = "1";
		echo "<script>window.location = 'view_school_evaluation.php?tid=".$thesisId."&pd=".$pd."&save=".$save."&ed=".$evaluationId."&mn=".$matrixNo."'</script>";
...
$sql4 = "SELECT a.id, b.id as pg_viva_id
FROM pg_calendar a
			...
			<table border="1" style="border-collapse:collapse;" cellpadding="2" cellspacing="1" width="90%" class="thetable">
              ...
              <tr>
				...
				<? if($report_status == 'SUB') {?>
                    <a href="view_edit_viva_report_school.php?tid=<?=$thesisId?>&type=<?=$ref_supervisor_type_id?>&mn=<?=$matrixNo?>&role=<?=$role_status?>&pd=<?=$otherDetailId?>&empid=<?=$employeeId?>&other=<?=$evaluationId?>&othertype=<?=$roleType?>" name="thesisId" value="<?=$thesisId?>" title="View feedback"><img src="../images/view.jpg" width="45" height="30" style="border:0px;" title="View feedback" /></a>
                    <? } ?>
				</td>
              </tr>
...
?>
	...
	<table class="thetable" border="1">
		...
		<tr>
			...
			<input type="radio" name="recCheck" id="recCheck" checked="checked" value="<?=$idRecArray[$no]?>" required/>
			<? } else { ?>
			<input type="radio" name="recCheck" id="recCheck" value="<?=$idRecArray[$no]?>" required/>
			<? } ?>
			</td>
...
?>
	<fieldset><legend><strong>Appeal Date</strong></legend>
	<div style="margin-left:15px;">
	<table>
		<tr>
			<td>End Date</td>
			<td>:</td>
			<td><input type="text" id="endDate" name="endDate" value="<?=$defaultEndDate?>" readonly=""/> 
			<input type="hidden" id="dayEnd" name="dayEnd" value="<?=$noOfDay?>"/>
			<?	$curdatetime22 = date("Y-m-d H:i:s");
				$jscript .= "\n" . '
		...
		<tr>
			<td colspan="3"><span style="color:red;">Notes</span>:<br />
			If the result for the thesis is fail, please choose last date for the student to request for appeal.<br />
			By default, the end date of the appeal will be <?=$noOfDay?> calendar days after the result submitted.</td>
		</tr>
	</table>
	</div>
	</fieldset>
	...
	<table>
		<tr>
			<td><textarea name="remark" id="remark" class="ckeditor"></textarea></td>
		</tr>
	</table>
	</fieldset>
	<table>
		<tr>
			<td colspan="2"><label><span style="color:#FF0000">Notes:</span><br/>
					1. Submit button is for submit final result for the thesis for senate endorsement.</label></td>
		</tr>
		<tr>
		  <td colspan="2">
