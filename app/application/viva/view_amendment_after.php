...
<? 
	...
	AND amendment_status IN ('SUB', 'SUB1')
...
<? // Call list of amendment
	...
	AND student_matrix_no = '$user_id'
	AND amendment_detail_status IN ('SUB', 'SUB1')";
	...
	<table width="90%" class="thetable" border="1">
		...
		<? if($row_cnt5>0) {?>
			...
			<tr>
				...
				<td align="left"><p style="width:98%;"><?=$amendmentArray[$i]?></p></td>
			</tr>
			<? } ?>
		<? } else { ?>
	...
	<table>
		<tr>
			<td>
			 ...
			 <td> <input type="submit" name="submit" value="Print PDF" onclick="javascript:open_win('pdf_viva_amendment_student_after.php?tid=<?=$thesisId?>&pid=<?=$proposalId?>&rid=<?=$referenceNo?>&mn=<?=$user_id?>&ad=<?=$amendmentId;?>',1000,760,0,0,0,1,0,1,1,0,5,'winupload'); "/></td>
		</tr>	
	</table>
	</fieldset>
