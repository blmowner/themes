					...
					if ($row_cnt_supervisor>0) {
						do {
							...
							<tr>
								...
								if ($amendment_status == '') {
								?>
									<td align="left"><label>Expecting for Amendment</label></td>
								<?}
								else if ($amendment_status == 'CON') {
								?>
									<td align="left"><label><span style="color:#FF0000"></span>Confirmed</label></td>
								<?}
								else if ($amendment_status == 'REQ'){
									?>								
...
<? 
	...
	AND amendment_status IN ('SUB', 'REQ')
...
.thetable td {
	...
	<table width="95%" class="thetable" border="1">
		<tr>
			...
			<th width="8%">Status<input type="hidden" name="totaldetail" id="totaldetail" value="<?=$inc?>" />
