...
?>
	...
	<input type="hidden" id="evaVivaId" name="evaVivaId" value="<?=$evaVivaId?>" />
		...
		<tr>
			...
			<td width="3%" height="40"><?=$iq+1?><input type="hidden" name="overallStyle[]" id="overallStyle" value="<?=$idArray[$iq]?>" />
...
?>
	...
	<table class="thetable" border="1" width="79%">
		...
		<tr>
			<td><?=$no1+1?><input type="hidden" name="sectionEid[]" id="sectionEid" value="<?=$idOrArray[$no1]?>" />
	...
	<table>
		...
		<tr>
		  <td colspan="2">	  
		  <input type="button" name="btnBack" id="btnBack" value="Back" onClick="javascript:document.location.href='school_evaluation.php?tid=<?=$thesisId?>&pid=<?=$proposalId[$j]?>&mn=<?=$matrixNo?>&ed=<?=$other?>';"/>
		  <input type="submit" name="submit" value="Print" onclick="javascript:open_win('print_school_eva_other.php?tid=<?=$thesisId?>&mn=<?=$matrixNo?>&pd=<?=$pd?>&empid=<?=$empid?>&othertype=<?=$roleType?>',950,900,0,0,0,1,0,1,1,0,5,'winupload'); "/></td>
