...
?>
	...
	<table class="thetable" border="1" width="66%">
		<tr>
			<th width="25" align="center">No</td>
			<th width="487" align="left">Components</td>
		  <th width="222" align="left">Ratings</td>		
	  </tr>
	...
	<table>
		...
		<td>
		   ...
		   <input type="button" name="btnBack" id="btnBack" value="Back" onClick="javascript:document.location.href='view_school_evaluation.php?tid=<?=$thesisId?>&pid=<?=$proposalId[$j]?>&mn=<?=$matrixNo?>&ed=<?=$other?>';"/>
		  <input type="button" name="btnView" id="btnView" value="Print PDF"onclick="javascript:open_win('pdf_summary_viva_report.php?tid=<?=$thesisId?>&pid=<?=$proposalId?>&type=<?=$roleType?>&mn=<?=$matrixNo?>&role=<?=$rolestatus?>&pd=<?=$pd?>&empid=<?=$empid?>',800,700,0,0,0,1,0,1,1,0,5,'winupload'); "/>
		  
			</td>
		 <!-- <td>	  
		  <input type="button" name="btnBack" id="btnBack" value="Back" onClick="javascript:document.location.href='view_school_evaluation.php?tid=<?=$thesisId?>&pid=<?=$proposalId[$j]?>&mn=<?=$matrixNo?>&ed=<?=$other?>';"/>
		  <td><input type="submit" name="submit" value="Print" onclick="javascript:open_win('print_school_eva_other.php?tid=<?=$thesisId?>&type=<?=$roleType?>&mn=<?=$matrixNo?>&role=<?=$rolestatus?>&pd=<?=$pd?>&empid=<?=$empid?>&other=<?=$other?>&othertype=<?=$othertype?>',1000,760,0,0,0,1,0,1,1,0,5,'winupload'); "/></td>-->
		</tr>
	</table>		
	</form>
	<script>
