...
{
	$msg[] = "<div class=\"success\"><span>VIVA report successfully submitted.</span></div>";
	/*echo $thesisId = $_GET['tid']; 
	echo "<br>";
	echo $matrixNo=$_GET['mn'];
	echo "<br>";
	echo $rolestatus=$_GET['role'];
	echo "<br>";
	echo $pd=$_GET['pd'];
	echo "<br>";
	echo$roleType=$_GET['type'];*/
...
<table border="1" style="border-collapse:collapse;" cellpadding="2" cellspacing="1" width="80%" class="thetable">			
					<tr>
						...
						<th width="5%" align="left">Feedback</th>
					</tr>
					...
					if ($row_cnt_supervisor>0) {
						do {
							...
							<tr>
								...
								<a href="view_viva_report_other_staff.php?tid=<?=$thesisId?>&type=<?=$ref_supervisor_type_id?>&mn=<?=$matrixNo?>&role=<?=$role_status?>&pd=<?=$otherDetailId?>&empid=<?=$employeeId?>&other=<?=$pd?>&othertype=<?=$roleType?>" name="thesisId" value="<?=$thesisId?>" title="View feedback"><img src="../images/view.jpg" width="45" height="30" style="border:0px;" title="View feedback"></a>
								<? } ?></td>
					...
					<? 	} while($db_klas2->next_record());
						...
						?>
						<table>				
							<tr><td><span style="color:#FF0000;">Notes</span>: <br/>No Evaluation Panel in the list.
										Possible Reasons:-<br/>
										1. If already assigned, it could be the Evaluation Panel pending to accept<br/><br/>
...
.table1 td {
	...
	<input type="hidden" id="evaVivaId" name="evaVivaId" value="<?=$evaVivaId?>" />
		<table class="thetable" width="95%" border="1">
			<tr>
				...
				<th width="15%" align="left">Rate</td>
	...
	<table class="table1" width="52%">
		<tr>
			<td height="80" width="618" ><p style="width:98%;"><?=$major_revision?></p></td>
		</tr>
	</table>
	...
	<table class="table1" width="52%">
		<tr>
			<td height="80" width="618"><p style="width:98%;"><?=$other_comment?></p></td>
		</tr>
	</table>
...
?>
	...
	<table class="table1" width="52%">
		...
		<tr>
			<td colspan="3" height="80"><p style="width:98%;"><?=$commentSecE?></p></td>
		</tr>
	</table>
	</fieldset>
	<table>
		...
		<tr>
		  <td colspan="3">
		 <?   
		 if($roleType=='EC' || $roleType=='CC') {?>
		  <input type="button" name="btnBack" id="btnBack" value="Back" onClick="javascript:document.location.href='viva_evaluation.php';"/>
		 <!-- <input type="submit" name="submit" value="VIVA VOCE" onclick="javascript:open_win('view_voce.php?vid=<?=$evaVivaId;?>&tid=<?=$thesisId;?>&mn=<?=$matrixNo;?>',800,700,0,0,0,1,0,1,1,0,5,'winupload'); "/>-->
		   <input type="submit" name="submit" value="Print VOCE" onclick="javascript:open_win('pdf_view_voce.php?vid=<?=$evaVivaId;?>&tid=<?=$thesisId;?>&mn=<?=$matrixNo;?>',800,700,0,0,0,1,0,1,1,0,5,'winupload'); "/>
		   
		   <input type="button" name="btnView" id="btnView" value="Print PDF Report"onclick="javascript:open_win('pdf_viva_report.php?tid=<?=$thesisId?>&pid=<?=$proposalId?>&type=<?=$roleType?>&mn=<?=$matrixNo?>&role=<?=$rolestatus?>&pd=<?=$pd?>',800,700,0,0,0,1,0,1,1,0,5,'winupload'); "/><? }
			else {?>
						<input type="button" name="btnBack" id="btnBack" value="Back" onClick="javascript:document.location.href='viva_evaluation.php';"/>
						 <input type="button" name="btnView" id="btnView" value="Print PDF Report" 
						onclick="javascript:document.location.href='pdf_viva_report.php?tid=<?=$thesisId?>&pid=<?=$proposalId?>&type=<?=$roleType?>&mn=<?=$matrixNo?>&role=<?=$rolestatus?>&pd=<?=$pd?>';"/>
						
						<? } ?> </td>
	<!--	 <input type="button" name="btnView" id="btnView" value="Print PDF Summary Report" 
						onclick="javascript:document.location.href='pdf_summary_viva_report.php?tid=<?=$thesisId?>&pid=<?=$proposalId?>&type=<?=$roleType?>&mn=<?=$matrixNo?>&role=<?=$rolestatus?>&pd=<?=$pd?>';"/></td>-->
		</tr>
	</table>	
	</form>
	<script>
