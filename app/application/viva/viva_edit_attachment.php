...
{					
	if (sizeof($_POST['attachment_checkbox'])>0) {
		...
		while (list ($key,$val) = @each ($_POST['attachment_checkbox'])) 
		{
			...
			$sql1 = "DELETE FROM file_upload_viva
			WHERE fu_cd = '$attachment_id'
			AND student_matrix_no = '$user_id'";
...
?>
  ...
  <form id="form1" name="form1" method="post" enctype="multipart/form-data" onsubmit="return saveRec();">
	 ...
	 <fieldset>
		...
		<table>
			<tr>
				<td><input type="submit" name="submit" value="Add Attachment" onclick="javascript:open_win('edit_viva_upload.php?vid=<?=$vivaId;?>&tid=<?=$pgThesisId;?>&pid=<?=$proposalId;?>',480,280,0,0,0,1,0,1,1,0,5,'winupload'); "/></td>
			</tr>
		</table>
		<table>
		<tr>							
				...
				if ($row_cnt > 0) {	
					...
					{ 
						...
						?><tr>
								...
								<td><a href="downloadviva.php?id=<?=$row["fu_cd"];?>&amp;al=S" target="_blank" onMouseOver="toolTip('<?=$row["fu_document_filename"];?>', 300)" onMouseOut="toolTip()" align="center">
	...
	<table>
		<tr>
			...
			<td>
			<? if ($save==1 || !empty($save)) { ?>
			<input type="button" name="btnBack" value="Back" onclick="javascript:document.location.href='../viva/edit_viva_save.php?tid=<?=$pgThesisId;?>&pid=<?=$proposalId;?>&vid=<?=$vivaId?>';" />
			<? } else { ?>
			<input type="button" name="btnBack" value="Back" onclick="javascript:document.location.href='../viva/edit_viva.php?tid=<?=$pgThesisId;?>&pid=<?=$proposalId;?>&vid=<?=$vivaId?>';" />
			<? }?>
			</td>		
