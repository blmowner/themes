  ...
  <form id="form1" name="form1" method="post" enctype="multipart/form-data" onsubmit="return saveRec();">
	 ...
	 <fieldset>
		...
		<table>
			<tr>
				...
				<td><input type="submit" name="submit" value="Add Attachment" 
				onclick="javascript:open_win('viva_rev_upload.php?tid=<?=$pgThesisId;?>&pid=<?=$proposalId;?>&mn=<?=$matrixNo;?>&role=<?=$role;?>&vid=<?=$pgVivaId;?>&vd=<?=$pgVivaDetailId;?>',480,280,0,0,0,1,0,1,1,0,5,'winupload'); "/></td>
			</tr>
		</table>
		<table>
		<tr>							
				...
				if ($row_cnt > 0) {	
					...
					{ 
						...
						?><tr>
								...
								<td><a href="downloadviva.php?id=<?=$row["fu_cd"];?>" target="_blank" onMouseOver="toolTip('<?=$row["fu_document_filename"];?>', 300)" onMouseOut="toolTip()" align="center">
	...
	<table>
		<tr>
			...
			<td><input type="button" name="btnBack" value="Back" onclick="javascript:document.location.href='../viva/review_viva_thesis.php?tid=<?=$pgThesisId;?>&pid=<?=$proposalId;?>&mn=<?=$matrixNo;?>&role=<?=$role;?>&vid=<?=$pgVivaId;?>&vd=<?=$pgVivaDetailId;?>';" /></td>		
