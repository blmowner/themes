...
if (isset($_GET['vid'])) 
{
$pgVivaId = (get_magic_quotes_gpc()) ? $_GET['vid'] : addslashes($_GET['vid']);
}	
...
$xpic_desc = clean($xpic_desc, 50);
	...
	if (is_uploaded_file($xpic_file))
	{
		...
		$upload_id = runnum('fu_cd','file_upload_viva');
		...
		{
			$sql2 = "INSERT INTO file_upload_viva (
			fu_cd,  
			...
			if (($db->query($sql2)) && @ mysql_affected_rows() == 1) 
			{
				...
				echo "opener.location.href=\"viva_rev_attachment.php?tid=$pgThesisId&pid=$proposalId&mn=$matrixNo&role=$role&vid=$pgVivaId&vd=$pgVivaDetailId\";";
				echo "document.location.href=\"viva_rev_receipt.php?data=$fileContent&status=T&userId=$user_id&fucd=$upload_id\";";
				echo "</script>";
			...
			{
				header("Location: viva_rev_receipt.php?status=F&userId=$user_id&fucd=$upload_id");
			}
		}
		else
		{
...
<form name="frmEmpUpdPic" method="post" action="../viva/viva_rev_upload.php?tid=<?=$pgThesisId?>&pid=<?=$proposalId?>&mn=<?=$matrixNo?>&role=<?=$role?>&vid=<?=$pgVivaId?>&vd=<?=$pgVivaDetailId?>" enctype="multipart/form-data">
