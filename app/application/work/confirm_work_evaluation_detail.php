...
// Program Name: confirm_work_evaluation_detail.php
//
// Created by: Zuraimi
...
{
	$evaluationPanelArray = $_POST['evaluationPanelArray'];
	$supervisorEmpId = $_POST['supervisorEmpId'];
	$confirmRemark = $_POST['confirmRemark'];
	$refWorkMarksId = $_POST['refWorkMarksId'];
	$curdatetime = date("Y-m-d H:i:s");
	
	$sql19 = "UPDATE pg_work_evaluation
	...
	WHERE id = '$workEvaluationId'";
	
	$db->query($sql19);
	$db->next_record();
	
	if ($refWorkMarksId=="NSA") {
		$workAmendmentId = runnum('id','pg_work_amendment');
		$sql8 = "INSERT INTO pg_work_amendment 
		(id, pg_thesis_id, pg_proposal_id, student_matrix_no, reviewer_id, pg_work_evaluation_id, pg_calendar_id, 
		feedback_status, status, insert_by, insert_date)
		VALUES ('$workAmendmentId', '$thesisId', '$proposalId', '$studentMatrixNo', '$supervisorEmpId', '$workEvaluationId', '$calendarId',
		'PND',null,'$user_id','$curdatetime')";
		
		$dba->query($sql8);
		
		$arr_length = count($evaluationPanelArray);
		if ($arr_length > 0) {
			for ($j=0;$j<$arr_length;$j++) {
				$workAmendmentDetailId = runnum('id','pg_work_feedback');
				$sql13 = "INSERT INTO pg_work_feedback
				(id, work_amendment_id, panel_employee_id,  
				feedback_status, insert_by, insert_date, modify_by, modify_date, archived_status, archived_date)
				VALUES ('$workAmendmentDetailId', '$workAmendmentId', '$evaluationPanelArray[$j]', 
				'PND', '$user_id', '$curdatetime', '$user_id', '$curdatetime', null, null)";
				$dbg->query($sql13);
			}
		}
	}
	
	$msg[] = "<div class=\"success\"><span>The Work Completion report has been approved successfully.</span></div>";	
}
...
if(isset($_POST['btnDisapproved']) && ($_POST['btnDisapproved'] <> ""))
{
	...
	$supervisorEmpId = $_POST['supervisorEmpId'];
	$refWorkMarksId = $_POST['refWorkMarksId'];
	$proposedDefenseMarks = $_POST['proposedWorkMarks_cb'];
	$confirmRemark = $_POST['confirmRemark'];
	$workEvaluationId = $_POST['workEvaluationId'];
	$curdatetime = date("Y-m-d H:i:s");
	
	$msg = Array();
	
	if ($confirmRemark == "") {
		$msg[] = "<div class=\"error\"><span>Please provide your remarks for the decision you have taken for this Work Completion status.</span></div>";
		$confirmRemarkAlert = true;
	}
	if ($proposedDefenseMarks == "") {
		$msg[] = "<div class=\"error\"><span>Please provide your proposed mark for the decision you have taken for this Work Completion status.</span></div>";
		$proposedMarkAlert = true;
	}
	...
	if(empty($msg)) {
		...
		if (($refWorkMarksId=="SUB" && $proposedDefenseMarks=="NSA") || ($refWorkMarksId=="SAT" && $proposedDefenseMarks=="NSA")){
			$workAmendmentId = runnum('id','pg_work_amendment');
			$sql8 = "INSERT INTO pg_work_amendment 
			(id, pg_thesis_id, pg_proposal_id, student_matrix_no, reviewer_id, pg_work_evaluation_id, pg_calendar_id, 
			feedback_status, status, insert_by, insert_date, modify_by, modify_date)
			VALUES ('$workAmendmentId', '$thesisId', '$proposalId', '$studentMatrixNo', '$supervisorEmpId', '$workEvaluationId', '$calendarId',
			'PND',null,'$user_id','$curdatetime', '$user_id','$curdatetime')";
			...
			if ($arr_length > 0) {
				for ($j=0;$j<$arr_length;$j++) {
					$workAmendmentDetailId = runnum('id','pg_work_feedback');
					$sql13 = "INSERT INTO pg_work_feedback
					(id, work_amendment_id, panel_employee_id, feedback_status, 
					insert_by, insert_date, modify_by, modify_date, archived_status, archived_date)
					VALUES ('$workAmendmentDetailId', '$workAmendmentId','$evaluationPanelArray[$j]', 'PND',
					'$user_id', '$curdatetime', '$user_id', '$curdatetime', null, null)";
					$dbg->query($sql13);
				}
			}
		}
		$msg[] = "<div class=\"success\"><span>The selected Work Completion report has been disapproved accordingly.</span></div>";	
	}
}
...
b.major_revision, b.other_comment, b.ref_work_marks_id, c3.description as defense_marks_desc,
a.remark_evaluation
...
LEFT JOIN ref_work_marks c3 ON (c3.id = b.ref_work_marks_id)
WHERE a.id = '$workEvaluationId'
...
$refWorkMarksId1=$dbg->f('ref_work_marks_id');
...
$sql_sv = "SELECT pg_employee_empid
FROM pg_supervisor  
WHERE pg_student_matrix_no = '$studentMatrixNo'
AND pg_thesis_id = '$thesisId'
AND ref_supervisor_type_id = 'SV'
AND role_status = 'PRI'
AND status = 'A'";
...
$db->next_record();
$supervisorEmpId = $db->f('pg_employee_empid');
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<?if ($evaluationStatus == "IN1") {?>
		...
		<table>
			<tr>
				<td><input type="button" name="btnBack" value="Back" onClick="javascript:document.location.href='../work/work_completion_confirm.php';" /></td>
			</tr>
		</table>	
		</br>
		<?
	...
	<legend><h3><strong>Work Achievement</strong></h3></legend>
		...
		$i=0;
		do {
			$workMarksIdArray[$i] = $db->f('id');
			$workMarksDescArray[$i] = $db->f('description');
		...
		<table width="60%" border="1" cellpadding="3" cellspacing="1" style="border-collapse:collapse;" class="thetable">			
				...
				<?for ($i=0;$i<$row_cnt16;$i++) {
					if ($refWorkMarksId1!="") {?>
					<tr>
						<?if ($workMarksIdArray[$i] == $refWorkMarksId1) {?>
							<td align="center"><input name="workMarks_cb" type="radio" id="workMarks_cb" value="<?=$workMarksIdArray[$i];?>" checked="checked" disabled="true"></td>
						<?}
						else {
							?>
							<td align="center"><input name="workMarks_cb" type="radio" id="workMarks_cb" value="<?=$workMarksIdArray[$i];?>" disabled="true"></td>
							<?
						}?>
						
						<td align="center"><label><?=$i+1?>.</label></td>
						<td><label><strong><?=$workMarksDescArray[$i]?></strong> <?=$defenseMarksRemarksArray[$i]?></label></td>
					</tr>
					<?}
					else {?>
					<tr>
						<?if ($workMarksIdArray[$i] == $_POST['workMarks_cb']) {?>
							<td align="center"><input name="workMarks_cb" type="radio" id="workMarks_cb" value="<?=$workMarksIdArray[$i];?>" checked="checked"></td>
						<?}
						else {
							?>
							<td align="center"><input name="workMarks_cb" type="radio" id="workMarks_cb" value="<?=$workMarksIdArray[$i];?>"></td>
							<?
						}?>
		...
		<br/>
		<fieldset>
		<legend><strong>WORK COMPLETION CONFIRMATION By SCHOOLBOARD</strong></legend>	
			<table>
				<tr>
		...
		<table width="60%" border="1" cellpadding="3" cellspacing="1" style="border-collapse:collapse;" class="thetable">			
			...
			<?for ($i=0;$i<$row_cnt16;$i++) {
				...
				<?
				if ($workMarksIdArray[$i] == $refWorkMarksId1) {?>
					<td align="center"><input name="proposedWorkMarks_cb" type="radio" id="proposedWorkMarks_cb" disabled="true" value="<?=$workMarksIdArray[$i]?>"></td>
				<?}
				else if ($_POST['proposedWorkMarks_cb']==$workMarksIdArray[$i]) {?>
					<td align="center"><input name="proposedWorkMarks_cb" type="radio" id="proposedWorkMarks_cb" checked="true" value="<?=$workMarksIdArray[$i]?>"></td>
				<?}
				else {
					?>
					<td align="center"><input name="proposedWorkMarks_cb" type="radio" id="proposedWorkMarks_cb" value="<?=$workMarksIdArray[$i]?>"></td>
					<?
				}?>
					<td align="center"><label><?=$i+1?>.</label></td>
					<td><label><strong><?=$workMarksDescArray[$i]?></strong> <?=$defenseMarksRemarksArray[$i]?></label></td>
	...
	<br>
	<table>
		<input type="hidden" name="refWorkMarksId" id="refWorkMarksId" value="<?=$refWorkMarksId1; ?>">
		<input type="hidden" name="workEvaluationId" id="workEvaluationId" value="<?=$workEvaluationId; ?>">
		<tr>
