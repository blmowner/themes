...
// Program Name: confirm_work_evaluation_detail_view.php
//
// Created by: Zuraimi
...
DATE_FORMAT(a.confirmed_date, '%d-%b-%Y %h:%i %p') as confirmed_date, a.confirmed_remark, a.confirmed_by,
a.remark_evaluation, a.proposed_marks_id
...
LEFT JOIN ref_work_marks c3 ON (c3.id = a.ref_work_marks_id)
...
$proposedMarksId1=$dbg->f('proposed_marks_id');
$refDefenseMarksDesc=$dbg->f('defense_marks_desc');
$confirmedDate=$dbg->f('confirmed_date');
$confirmedRemark=$dbg->f('confirmed_remark');
$confirmedBy=$dbg->f('confirmed_by');
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<table>
		<tr>
			<td><strong>School Board Confirmation Status</strong></td>
		</tr>
	</table>
	<table>
		<tr>
		...
		$sql19 = "SELECT name
		from new_employee
		WHERE empid = '$confirmedBy'";
		
		if (substr($confirmedBy,0,3) != 'S07') { 
			$dbConn= $dbc; 
		} 
		else { 
			$dbConn=$dbc1; 
		...
		?>
		<tr>
			<td><label>Confirmed By</label></td>
			<td>:</td>
			<td><?=$employeeName?> (<?=$confirmedBy?>)</td>
		</tr>
		<tr>
			<td><label>Confirmation Date</label></td>
			<td>:</td>
			<td><?=$confirmedDate?></td>
		</tr>		
	</table>
	<table>
			<tr>
				<td><label><strong>Proposed Recommendation </strong> - If you choose to <strong>Disapproved</strong></label></td>
			</tr>
		</table>
		<?
		...
		<table width="60%" border="1" cellpadding="3" cellspacing="1" style="border-collapse:collapse;" class="thetable">			
			...
			<?for ($i=0;$i<$row_cnt16;$i++) {
				...
				<tr>
				<?
				if ($defenseMarksIdArray[$i] == $proposedMarksId1) {?>
					<td align="center"><input name="proposedDefenseMarks_cb" type="radio" id="proposedDefenseMarks_cb" disabled="true" checked="true" value="<?=$defenseMarksIdArray[$i]?>"></td>
				<?}
				else {
					?>
	...
	<table width="90%">
		<tr>
			<td><label>Confirmation Remark:</label></td>
		</tr>
		<tr>
			<td><?=$confirmedRemark?></td>
		</tr>
	</table>
	<br/>
	<table width="90%">
		<tr>
			<td><label>Remark by Supervisor for Schoolboard Consideration:</label></td>
		</tr>
		<tr>
			<td><?=$remarkEvaluation?></td>
		</tr>
	</table>
	<br/>
	<table>
