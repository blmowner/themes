...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<title>Edit Defense Proposal</title>
...
if(isset($_POST['btnSave']) && ($_POST['btnSave'] <> ""))
{
	...
	if ($_POST['defenseSchedule'] == "") $msg[] = "<div class=\"error\"><span>Please select Evaluation Schedule for Work Completion.</span></div>";
	...
	if(empty($msg))  
	{
		if ($workStatus=="") {
			...
			{
				$newDefenseDetailId = runnum('id','pg_work_detail');
			...
			{	
				...
				(id, work_id, reference_no, pg_thesis_id, pg_proposal_id, student_matrix_no, pg_chapter_id, pg_subchapter_id,  
				...
				STR_TO_DATE('$plannedDate[$k]','%d-%M-%Y'), STR_TO_DATE('$completionDate[$k]','%d-%M-%Y'), '$contentDescriptionArray[$k]', 
				'$user_id', '$curdatetime', '$user_id', '$curdatetime')";
			...
			{	
				...
				if ($row_cnt_sql6 == 0) {
					...
					$discussionIdArray[$k] = $achievementId;
					$sql6_1 = "UPDATE pg_work_achievement
					SET content_discussion = '$contentDescriptionArray[$k]',
					planned_date = STR_TO_DATE('$plannedDate[$k]','%d-%M-%Y'), 
					completion_date = STR_TO_DATE('$completionDate[$k]','%d-%M-%Y')
					WHERE id = '$achievementId'
					AND work_id = '$defenseId'
					AND pg_thesis_id = '$thesisId'
					AND pg_proposal_id = '$proposalId'
					AND reference_no = '$referenceNo'
					AND student_matrix_no = '$user_id'
			...
			$sql6 = "UPDATE pg_defense_publication
			SET pg_defense_id = '$defenseId', add_status = 'CFM', 
			...
			$sql7 = "UPDATE pg_defense_conference
			SET pg_defense_id = '$defenseId', add_status = 'CFM', modify_by = '$user_id', modify_date = '$curdatetime'
			WHERE pg_defense_id IS NULL
			AND pg_thesis_id = '$thesisId'
			AND pg_proposal_id = '$proposalId'
			AND student_matrix_no = '$user_id'
...
if(isset($_POST['btnSubmit']) && ($_POST['btnSubmit'] <> ""))
{
	...
	if(empty($msg)) 
	{
		if ($workStatus=="") {
			...
			status, submit_status, respond_status,
			insert_by, insert_date, modify_by, modify_date )
			VALUES ('$defenseId', '$referenceNo', '$defenseScheduleId', '$curdatetime', '$user_id', '$thesisId', '$proposalId', 
			...
			$msg[] = "<div class=\"success\"><span>Your Monthly Defence Report has been submitted successfully.</span></div>";		
		}
		else if ($workStatus=="SAV") {//SAV
			...
			{	
				...
				if ($row_cnt_sql6 == 0) {
					...
					planned_date, completion_date, content_discussion, 
					insert_by, insert_date, modify_by, modify_date )
					VALUES ('$newAchievementId', '$defenseId', '$referenceNo', '$thesisId', '$proposalId', '$user_id', 
					'$chapterId[$k]', '$subchapterId[$k]',  
					STR_TO_DATE('$plannedDate[$k]','%d-%M-%Y'), STR_TO_DATE('$completionDate[$k]','%d-%M-%Y'), '$contentDescriptionArray[$k]', 
					'$user_id', '$curdatetime', '$user_id', '$curdatetime')";
			...
			$msg[] = "<div class=\"success\"><span>Your Work Completion report has been submitted successfully.</span></div>";
...
a.pg_thesis_id, a.pg_proposal_id, a.status as work_status, 
...
AND status = 'A'
AND id NOT IN (SELECT pg_calendar_id 
FROM pg_work)
AND archived_status IS NULL
ORDER BY defense_date ASC";
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	function newPublication(pid, tid, pgid, ref) 
	{
		...
		if (ask) 
		{
			document.location.href = "../work/edit_work_publication.php?pid=" + pid + "&tid=" + tid + "&pgid=" + pgid + "&ref=" + ref;
			return true;
		}
		return false;
	}
	...
	function newConference(pid, tid, pgid, ref) 
	{
		...
		if (ask) 
		{
			document.location.href = "../work/edit_work_conference.php?pid=" + pid + "&tid=" + tid + "&pgid=" + pgid + "&ref=" + ref;
			return true;
		}
		return false;
	}
	...
	if ($row_cnt4 > 0) {
				...
				<tr>
					...
					if ($calendarId =="") {
						...
						for ($j=0;$j<$i;$j++) {
							...
							?>								
								<option value="<?=$recommendedIdArray[$j].$calendarStatusArray[$j].$recommStatusArray[$j]?>" selected="selected"><label><?=$sessionTypeDescArray[$j]?> - <?=$defenseDateArray[$j]?>, <?=$defenseSTimeArray[$j]?> to <?=$defenseETimeArray[$j]?>, <?=$venueArray[$j]?></label>
								<?
								if ($calendarStatusArray[$j]=="I" || $recommStatusArray[$j]=="CAN") {
									?>
					...
					if ($row_cnt_supervisor>0) {
						do {
							...
							<tr>
								...
								if ($row_cnt12>0) {
								?>
								<td align="center"><a href="edit_work_view_feedback.php?tid=<?=$thesisId;?>&pid=<?=$proposalId;?>&eid=<?=$employeeId;?>&id=<?=$id;?>&ref=<?=$referenceNo;?>&cid=<?=$calendarId?>">View</a></td>	
								<?}
								else {
									?>
									<td></td>
		...
		$msg = "<div class=\"error\"><span>Work Completion submission would require proposal defence date. Please advice your your preference date to your main Supervisor.</span></div>";
		echo $msg;
		?>
