...
{	
	...
	$prevReferenceNo = $_POST['prevReferenceNo'];
	...
	if ($_POST['defenseSchedule'] == "") $msg[] = "<div class=\"error\"><span>Please select recommended date for Work Completion.</span></div>";
	if ($defenseScheduleStatus == "I") $msg[] = "<div class=\"error\"><span>The selected Work Completion Date is already cancelled. Please select other date.</span></div>";
	if ($defenseRecommStatus == "CAN") $msg[] = "<div class=\"error\"><span>The selected Work Completion Date is already cancelled. Please select other date.</span></div>";
	...
	if(empty($msg)) {
		if ($workStatus=="") {
			...
			AND reference_no = '$prevReferenceNo'
			AND archived_status IS NULL";

			$dba->query($sql9);

			$workId = runnum('id','pg_work');
			$referenceNo = "W".runnum2('reference_no','pg_work');
			
			$sql1 = " INSERT INTO pg_work
			(id, pg_defense_id, pg_evaluation_id, reference_no, pg_calendar_id, submit_date, student_matrix_no, pg_thesis_id, pg_proposal_id,
			status, submit_status, respond_status,
			insert_by, insert_date, modify_by, modify_date )
			VALUES ('$workId', '$defenseId','$evaluationId','$referenceNo', '$defenseScheduleId', '$curdatetime', '$user_id', '$thesisId', 
			'$proposalId', 'SAV','SAV', 'N', '$user_id', '$curdatetime', '$user_id', '$curdatetime')";
		
			$dba->query($sql1); 
			$newWorkDetailIdArray = Array();
			while (list ($key,$val) = @each ($supervisorIdArray)) 
			{
				$newWorkDetailId = runnum('id','pg_work_detail');
				$newWorkDetailIdArray[$key] = $newWorkDetailId;
				
				$sql2 = " INSERT INTO pg_work_detail
				(id, pg_work_id, pg_employee_empid, status, submit_date,
				insert_by, insert_date, modify_by, modify_date )
				VALUES ('$newWorkDetailId', '$workId', '$val', 'SAV', '$curdatetime',
			...
			{	
				...
				$newAchievementId = runnum('id','pg_work_achievement');

				$sql3 = " INSERT INTO pg_work_achievement
				(id, work_id, reference_no, pg_thesis_id, pg_proposal_id, student_matrix_no, pg_chapter_id, 
				pg_subchapter_id, discussed_status, 
				planned_date, completion_date, content_discussion, 
				insert_by, insert_date, modify_by, modify_date )
				VALUES ('$newAchievementId', '$workId', '$referenceNo', '$thesisId', '$proposalId', '$user_id', '$chapterId[$k]', 
			...
			SET pg_defense_id = '$workId', reference_no = '$referenceNo', 
			...
			$msg[] = "<div class=\"success\"><span>Your Work Completion Report has been saved successfully.</span></div>";
		}
		else if ($workStatus=="SAV") {//SAV
			$sql5 = " UPDATE pg_work
			...
			while (list ($key,$val) = @each ($supervisorIdArray)) 
			{
				$sql5_4 = "SELECT id, pg_work_id, pg_employee_empid, status, responded_status, responded_date, 
				submit_date, insert_by, insert_date, modify_by, modify_date
				FROM pg_work_detail
				WHERE pg_work_id = '$workId'
				...
				$defenseDetailIdArray[$key] = $pg_work_detail_id;
				$row_cnt_5_4 = mysql_num_rows($result_5_4);
				if ($row_cnt_5_4 > 0) {
					if ($status != 'APP') {
						$sql5_5 = " UPDATE pg_work_detail
					...
					(id, pg_work_id, pg_employee_empid, status, submit_date, 
					insert_by, insert_date, modify_by, modify_date )
					VALUES ('$newWorkDetailId', '$workId', '$val', 'SAV', 
			...
			{	
				...
				if ($row_cnt_sql6 == 0) {
					...
					$sql3 = " INSERT INTO pg_work_achievement
					(id, work_id, reference_no, pg_thesis_id, pg_proposal_id, student_matrix_no, pg_chapter_id, pg_subchapter_id, discussed_status, planned_date, completion_date, 
					...
					$sql6_1 = "UPDATE pg_work_achievement
	...
	if(empty($msg)) {
		if ($workStatus=="") {
			...
			while (list ($key,$val) = @each ($supervisorIdArray)) 
			{
				...
				(id, pg_work_id, pg_employee_empid, status, submit_date,
				insert_by, insert_date, modify_by, modify_date )
				VALUES ('$newWorkDetailId', '$workId', '$val', 'IN1', '$curdatetime',
				'$user_id', '$curdatetime', '$user_id', '$curdatetime')";
				...
				$sqlspdata = "SELECT name, email from new_employee where empid = '$val'";
				$dbcspdata = $dbc;
				$result_sqlspdata = $dbcspdata->query($sqlspdata); 
				$dbcspdata->next_record();
				$spName = $dbcspdata->f('name');
				$spEmail = $dbcspdata->f('email');
				
				$sqlstudentdata = "SELECT name, email from student WHERE matrix_no = '$user_id'";
				$dbcstudata = $dbc;
				...
				{
					include("../../../app/application/inbox/work/submit_work.php");
				}
				...
				if ($emailConstValue == 'Y' && $emailConstValue1 == 'Y' && !empty($spEmail) && !empty($studentEmail))
				{
					include("../../../app/application/email/work/email_submit_work.php");
				}			
				////////////////////end notification/////////////////////////////////
			...
			$msg[] = "<div class=\"success\"><span>Your Work Completion Report has been submitted successfully to your Supervisor / Co-Supervisor.</span></div>";	
			...
			while (list ($key,$val) = @each ($supervisorIdArray)) 
			{
				...
				if ($row_cnt_5_4 > 0) {					
					...
					(id, pg_work_id, pg_employee_empid, status, submit_date, 
					insert_by, insert_date, modify_by, modify_date )
					VALUES ('$newWorkDetailId', '$workId', '$val', 'IN1', 
					'$submitDate', '$user_id', '$curdatetime', '$user_id', '$curdatetime')";
			...
			{	
				...
				if ($row_cnt_sql6 == 0) {
					...
					(id, work_id, reference_no, pg_thesis_id, pg_proposal_id, student_matrix_no, pg_chapter_id, pg_subchapter_id, discussed_status, planned_date, completion_date,
					content_discussion, insert_by, insert_date, modify_by, modify_date )
					VALUES ('$newAchievementId', '$workId', '$referenceNo', '$thesisId', '$proposalId', '$user_id', '$chapterId[$k]', '$subchapterId[$k]',   
...
$id=$dba->f('id');
$workStatus=$dba->f('defense_status');
...
if ($check == true) {
	$sql11 = "SELECT a.id as defense_id, b.id as evaluation_id, a.status as defense_status, 
	b.ref_defense_marks_id, b.proposed_marks_id, b.status as evaluation_status, 
	...
	$result_sql11 = $dbg->query($sql11); 
	$dbg->next_record();
	$row_sql11 = mysql_num_rows($result_sql11);
	
	$defenseId = $dbg->f('defense_id');
	$evaluationId = $dbg->f('evaluation_id');
	$defenseEvaluationStatus = $dbg->f('evaluation_status');
	$defenseMarksId = $dbg->f('ref_defense_marks_id');
	$defenseProposedMarksId = $dbg->f('proposed_marks_id');
	$defenseRespondStatus = $dbg->f('respond_status');
	$defenseConfirmedStatus = $dbg->f('confirmed_status');
	

	if ($defenseEvaluationStatus == "APP") {
		if ($defenseMarksId == "SAT") {
			$submitWCReport = true;
		}
		else {								
			$errorNo = 101;
			$submitWCReport = false;
			$msg[] = "<div class=\"error\"><span>Submission of new Work Completion Report is currently not allowed.</span></div>";	
		}		
	}
	else if ($defenseEvaluationStatus == "DIS") {
		if ($defenseMarksId == "SAT") {
			if ($defenseProposedMarksId == "NSA") {
				$errorNo = 101;				
				$submitWCReport = false;
				$msg[] = "<div class=\"error\"><span>Submission of new Work Completion Report is currently not allowed.</span></div>";	
			}
			else if ($defenseProposedMarksId == "FAI") {
				$errorNo = 101;				
				$submitWCReport = false;
				$msg[] = "<div class=\"error\"><span>Submission of new Work Completion Report is currently not allowed.</span></div>";					
			}
		}
		else if ($defenseMarksId == "NSA") {
			if ($defenseProposedMarksId == "SAT") {
				$submitWCReport = true;
			}
			else if ($defenseProposedMarksId == "FAI") {
				$errorNo = 101;				
		...
		else if ($defenseMarksId == "FAI") {
			if ($defenseProposedMarksId == "SAT") {
				$submitWCReport = true;
			}
			else if ($defenseProposedMarksId == "NSA") {
				$errorNo = 101;	
				$submitWCReport = false;
				$msg[] = "<div class=\"error\"><span>Submission of new Work Completion Report is currently not allowed.</span></div>";	
			}
	...
	if ($submitWCReport == true) {
		
		$sql10 = "SELECT a.id, a.reference_no, a.status as work_status
		FROM pg_work a
		WHERE a.student_matrix_no = '$user_id'
		AND a.pg_thesis_id = '$thesisId'
		AND a.pg_proposal_id = '$proposalId'
		AND a.status IN ('SAV', 'IN1', 'REC', 'NRE')
		AND a.submit_status IN ('SAV', 'INP')
		AND a.archived_status IS NULL
		ORDER BY a.submit_date DESC
		LIMIT 1";
		...
		$workStatus1 = $dbg->f('work_status');
		$referenceNo1 = $dbg->f('reference_no');
		
		$msg = Array();
		$errorNo = 0;

		if ($row_cnt10 > 0) { //Check defence proposal status
			if ($workStatus1 == "REC") {
				$sql8 = "SELECT b.id as work_evaluation_id, a.pg_calendar_id, a.status as work_status, 
				b.ref_work_marks_id, b.proposed_marks_id, 
				b.status as evaluation_status, b.respond_status, b.confirmed_status
				FROM pg_work a
				LEFT JOIN pg_work_evaluation b ON (b.pg_work_id = a.id)
				WHERE a.student_matrix_no = '$user_id'
				AND a.pg_thesis_id = '$thesisId'
				AND a.pg_proposal_id = '$proposalId'
				AND a.reference_no = '$referenceNo1'
				AND a.status = 'REC'
				AND a.submit_status = 'INP'
				AND a.respond_status = 'Y'
				AND a.archived_status IS NULL
				...
				$workEvaluationId = $dbg->f('work_evaluation_id');
				$theCalendarId = $dbg->f('pg_calendar_id');
				$workMarksId = $dbg->f('ref_work_marks_id');
				$proposedMarksId = $dbg->f('proposed_marks_id');
				$evaluationStatus = $dbg->f('evaluation_status');
				$respondStatus = $dbg->f('respond_status');
				$confirmedStatus = $dbg->f('confirmed_status');
				...
				if ($row_cnt8 > 0) {
					...
					AND pg_proposal_id = '$proposalId'
					AND student_matrix_no = '$user_id'
					AND pg_work_evaluation_id = '$workEvaluationId'
					AND pg_calendar_id = '$theCalendarId'
					AND feedback_status = 'FVE'					
					AND status = 'AVE'
					AND archived_status IS NULL";
					
					$result_sql13 = $dba->query($sql13);
					$dba->next_record();
					...
					if ($row_cnt13 > 0) { 
						if ($respondStatus == "Y") {
							if ($confirmedStatus == "Y") {
								if ($evaluationStatus == "APP") {							
									if ($workMarksId == "SUB") {
										$resubmitReport = false;
										$errorNo = 1;
										$msg[] = "<div class=\"error\"><span>Submission of new Work Completion Report is currently not allowed.</span></div>";
									}
									else if ($workMarksId == "SAT") {								
										$errorNo = 1;
										$msg[] = "<div class=\"error\"><span>Submission of new Work Completion Report is currently not allowed.</span></div>";
									}
									else if ($workMarksId == "NSA") {
										$resubmitReport = true;				
									}
								}
								else if ($evaluationStatus == "DIS") {
									if ($workMarksId == "SUB") {
										if ($proposedMarksId == "SAT") {
											$errorNo = 1;
											$msg[] = "<div class=\"error\"><span>Submission of new Work Completion Report is currently not allowed.</span></div>";
										}
										else if ($proposedMarksId == "NSA") {
											$resubmitReport = true;	
										}
									}
									else if ($workMarksId == "SAT") {
										if ($proposedMarksId == "SUB") {
											$errorNo = 2;
											$resubmitReport = false;
											$msg[] = "<div class=\"error\"><span>Submission of new Work Completion Report is currently not allowed.</span></div>";	
										}
							...
							else if ($confirmedStatus == "N") {
								$errorNo = 4;
								$resubmitReport = false;
								$msg[] = "<div class=\"error\"><span>Submission of new Work Completion Report is currently not allowed.</span></div>";
							}
						}
						else if ($respondStatus == "N") { 
							$errorNo = 5;
							$resubmitReport = false;
							$msg[] = "<div class=\"error\"><span>Submission of new Work Completion Report is currently not allowed.</span></div>";	
						}
			...
			else if ($workStatus1 == "IN1") {
				$errorNo = 6;
				$resubmitReport = false;
				$msg[] = "<div class=\"error\"><span>Submission of new Work Completion Report is currently not allowed.</span></div>";
			}
			else if ($workStatus1 == "NRE") {					
				$resubmitReport = true;
			}
		}
		else {
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	function newPublication(pid, tid, pgid, ref) 
	{
		...
		if (ask) 
		{
			document.location.href = "../work/new_work_publication.php?pid=" + pid + "&tid=" + tid + "&pgid=" + pgid + "&ref=" + ref;
			return true;
		}
		return false;
	}
	...
	function newConference(pid, tid, pgid, ref) 
	{
		...
		if (ask) 
		{
			document.location.href = "../work/new_work_conference.php?pid=" + pid + "&tid=" + tid + "&pgid=" + pgid + "&ref=" + ref;
			return true;
		}
		return false;
	}
	...
	if ($resubmitReport == true) {
		if ($row_cnt4 > 0) {
			if ($workStatus == "" || $workStatus == "SAV") {?>
				<table border="0"> 
					<tr>
					<td><h3><strong>Work Completion Details </strong><h3></td>
					</tr>
				</table>
				<table width="100%">
					<tr>
					...
					<tr>
						...
						<?if ($_POST['defenseId']!="") $defenseId=$_POST['defenseId'];?>
						<input type="hidden" name="defenseId" id="defenseId" value="<?=$defenseId; ?>">
						<?if ($_POST['evaluationId']!="") $evaluationId=$_POST['evaluationId'];?>
						<input type="hidden" name="evaluationId" id="evaluationId" value="<?=$evaluationId; ?>">
						<input type="hidden" name="prevReferenceNo" id="prevReferenceNo" value="<?=$referenceNo1; ?>">
					</tr>
					<tr>
				...
				<table>
					<tr>
						...
						1. The recommended date to have Work Completion is after <?=$defenseDurationParam?> days starting from submission date.</label>
						</td>
					</tr>
				</table>			
				<?
				...
				<table>
					<tr>
						...
						<td><input type="button" name="btnBack" value="Back" onclick="javascript:document.location.href='work_completion.php';" /></td>		
					</tr>
				</table>
				<br/>
						...
						if ($row_cnt_supervisor>0) {
							do {
								...
								<tr>
									...
									if ($row_cnt12>0) {
									?>
									<td align="center"><a href="new_work_view_feedback.php?tid=<?=$thesisId;?>&pid=<?=$proposalId;?>&eid=<?=$employeeId;?>&id=<?=$id;?>&ref=<?=$referenceNo;?>&cid=<?=$calendarId?>">View</a></td>	
									<?}
									else {
										?>
										<td></td>
				...
				$subchapterId= Array();
				$subchapterNo= Array();
				$subchapterDesc= Array();
				$chapterDiscussedStatus= Array();
				$subchapterDiscussedStatus= Array();
				$no=0;
				do {											
				...
				<table>
					<tr>
						<td><label><span style="color:#FF0000">Note:</span></td>
					</tr>
					<tr>
			...
			$msg = "<div class=\"error\"><span>Work Completion submission would require Work Completion date. Please advice your main Supervisor to recommend the date.</span></div>";
			echo $msg;
			?>
			<table>
				<tr>		
					<td><label>Possible Reason:-<br>
					1. Work Completion submission would require work completion date. Please check your preference date with your main Supervisor.</label></td>		
				</tr>
			</table>		
			<table>
				<tr>		
		...
		<table>
			<tr>		
				<td><label>Possible Reason:-<br>
				1. Your Work Completion report has already been approved by Evaluation Committee and Academic Committee, please proceed with the <a href="">VIVA</a> report submission<br>
				</label></td>		
			</tr>
		</table>
		<?
		...
		<table>
			<tr>		
				<td><label>Possible Reason:-<br>
				1. Your Work Completion report has already been approved by Academic Committee, please proceed with the <a href="">VIVA</a> report submission<br>
				</label></td>		
			</tr>
		</table>
		<?
		...
		<table>
			<tr>		
				<td><label>Possible Reason:-<br>
				1. Your Work Completion report is currently <a href="../work/work_completion.php">pending approval</a> by Academic Committee, please check again later.<br>
				</label></td>		
			</tr>
		</table>
		<?
		...
		<table>
			<tr>		
				<td><label>Possible Reason:-<br>
				1. Your Work Completion report is currently <a href="../work/work_completion.php">pending approval</a> by Evaluation Committee, please check again later.<br>
				</label></td>		
			</tr>
		</table>
		<?
		...
		<table>
			<tr>		
				<td><label>Possible Reason:-<br>
				1. Your Work Completion report is currently in <a href="../work/work_completion.php">review</a> by Evaluation Committee. Please check again later.</label></td>		
			</tr>
		</table>
		<?
		}
		...
		<table>
			<tr>		
				<td><label>Possible Reason:-<br>
				1. Your Work Completion report is currently in draft mode <a href="../work/work_completion.php">[Edit Mode]</a>, please complete and submit it first</label></td>		
			</tr>
		</table>
		<?
		}
		...
		<table>
			<tr>		
				...
				1. It could be your Work Completion has been approved. If so, please <a href="../viva/list_of_viva.php">submit the thesis</a> for your VIVA/Thesis Evaluation OR<br>
				2. You have yet to complete the required amendment as requested by the Schoolboard/Academic Committee. Please <a href="../work/work_amendment.php">complete</a> it first before resubmit a new Work Completion.</label></td>		
			</tr>
		</table>
		<?
		}
		...
		<table>
			<tr>		
				<td><label>Possible Reason:-<br>
				1. Your Defence Proposal report is still not yet approved. Please <a href="../defense/defense_proposal.php">complete</a> it first</label></td>		
			</tr>
		</table>
		<?
		}
		...
		<?
			...
			$theMsg = "<div class=\"error\"><span>New Work Completion report submission is currently not allowed. dddd</span></div>";	
			echo $theMsg;
			?>
			<table>
			<tr>		
				<td><label>Possible Reason:-<br>
				1. Ensure your <a href="../defense/defense_proposal.php">Defence Proposal report</a> is submitted and approved first.</label></td>		
			</tr>
			</table>
			<table>
				<tr>		
