...
// Program Name: new_work_conference.php
//
// Created by: Zuraimi
...
if(isset($_POST['btnAdd']) && ($_POST['btnAdd'] <> ""))
{
	...
	if (empty($_POST['add_conference_sdate']) || empty($_POST['add_conference_edate'])) {
		...
		if ($conferenceSDate4 > $currentDate4) {
			$msg[] = "<div class=\"error\"><span>Conference Start Date cannot be later than Current Date.</span></div>";
		}
		if ($conferenceEDate4 > $currentDate4) {
			$msg[] = "<div class=\"error\"><span>Conference End Date cannot be later than Current Date.</span></div>";
		}
	...
	if(empty($msg)) 
	{
		...
		if ($defense_id!="") {
			...
			VALUES ('$conference_id', '$defense_id', '$referenceNo', '$thesis_id', '$proposal_id', '$user_id', 'WCO',
			...
			conference, location, conference_sdate, conference_edate, presentation_status, presentation_title,
			add_status, insert_by, insert_date, modify_by, modify_date)
			VALUES ('$conference_id', null, null, '$thesis_id', '$proposal_id', '$user_id', 'WCO',
			'$myAddConference', '$myAddConferenceLoc', STR_TO_DATE('$myAddConferenceSDate','%d-%M-%Y'), 
			STR_TO_DATE('$myAddConferenceEDate','%d-%M-%Y'), '$myAddPresentationStatus', '$myAddPresentationTitle',
			'TMP', '$user_id', '$curdatetime', '$user_id', '$curdatetime')";
...
{	
	...
	while (list ($key,$val) = @each ($tmpConferenceCheckBox)) 
	{
		...
		$conferenceSDate1 = date('d-M-Y', strtotime($_POST['conference_sdate'][$val]));
		$conferenceSDate2 = new DateTime($conferenceSDate1);
		$conferenceSDate3 = $conferenceSDate2->format('d-M-Y');
		$conferenceSDate4 = new DateTime($conferenceSDate3);
		
		$conferenceEDate1 = date('d-M-Y', strtotime($_POST['conference_edate'][$val]));
		$conferenceEDate2 = new DateTime($conferenceEDate1);
		$conferenceEDate3 = $conferenceEDate2->format('d-M-Y');
		$conferenceEDate4 = new DateTime($conferenceEDate3);
		
		if ($conferenceEDate4 < $conferenceSDate4) $msg[] = "<div class=\"error\"><span>Conference Start Date for record no $no. cannot be later than Conference End Date.</span></div>";
		
		if ($conferenceSDate4 > $currentDate4) {
			$msg[] = "<div class=\"error\"><span>Conference Start Date for record no $no. cannot be later than Current Date.</span></div>";
		}
		if ($conferenceEDate4 > $currentDate4) {
			$msg[] = "<div class=\"error\"><span>Conference End Date for record no $no. cannot be later than Current Date.</span></div>";
		}		
	}
