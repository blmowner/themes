...
$sql22 = "SELECT id, description
		FROM ref_amendment_status
		WHERE id = '$theAmendmentStatus' ";
		$result_sql22 = $db->query($sql22);
		$db->next_record();
		$theAmendmentDesc = $db->f('description');
 
		$sql7 = "SELECT name
		FROM new_employee
		WHERE empid = '$theModifyBy'";
...
$sqlComment = "SELECT a.comment 
			...
			AND a.status NOT LIKE 'ASU'";
			
			$result_sqlComment = $dba->query($sqlComment);
			$dba->next_record();
			$row_cntComment = mysql_num_rows($result_sqlComment);
			
			$commentSVArray = Array();
			$q=0;
			do{
			$commentSVArray[$q] = $dba->f('comment');
			$q++;
			}while ($dba->next_record());
			...
			for ($j=0; $j<$row_cnt1; $j++)
				...
				$sql3 = "SELECT  comment
						...
						do {
							...
							$comment1[$i] = $dba->f('comment');
					
							$i++;
						}while ($dba->next_record());
						
					$htmlB1=$commentSVArray[$j];
					$tmpHTMLB1 = strip_tags($htmlB1);
					$html=$panelFeedbackArray[$j];
					$tmpHTML = strip_tags($html);
					$htmlB=$amendmentBeforeChgArray[$j];
					...
					$pdf->Row2(array(($j+1).".",$tmpHTML,"Amendment (Before Change): ".$tmpHTMLB."Page Before: ". $pageAffectedArray[$j]."\nAmendment (After Change): ".$tmpHTMLA."Page After: ".$pageAfterChgArray[$j],"Name: ".$supervisorName."\n(".$employeeId.")" ,"Name: ".$panelEmployeeName."\n(".$panelEmployeeId.")\nRemark: ".$tmpHTMLV));	
				}
			}
		else{}
...
else if($theAmendmentStatus == 'ACO'){if ($row_cnt1 > 0) {
			for ($j=0; $j<$row_cnt1; $j++)
				{ 
					$htmlB1=$commentSVArray[$j];
					$tmpHTMLB1 = strip_tags($htmlB1);
...
else if($theAmendmentStatus == 'AVE'){if ($row_cnt1 > 0) {
			for ($j=0; $j<$row_cnt1; $j++)
				{ 
					$htmlB1=$commentSVArray[$j];
					$tmpHTMLB1 = strip_tags($htmlB1);
					...
					$htmlV=$verifiedRemarkArray[$j];
					$tmpHTMLV = strip_tags($htmlV);
					$pdf->Row2(array(($j+1).".",$tmpHTML,"Amendment (Before Change): ".$tmpHTMLB."Page Before: ". $pageAffectedArray[$j]."\nAmendment (After Change): ".$tmpHTMLA."Page After: ".$pageAfterChgArray[$j],"Name: ".$supervisorName."\n(".$employeeId.")\nComment: ".$tmpHTMLB1."\nStatus: ".$theAmendmentDesc ,"Name: ".$panelEmployeeName."\n(".$theModifyBy.")\nRemark: ".$tmpHTMLV));	
				}
			}
		else
			{
