...
// Program Name: review_work_evaluation_detail_view.php
//
// Created by: Zuraimi
		...
		if ($row_cnt_examiner>0) {
			...
			do {
				...
				<tr>
					...
					?>
					<td><a href="../work/review_work_evaluation_partner_view.php?id=<?=$evaluationId?>&cid=<?=$calendarId?>&tid=<?=$thesisId;?>&pid=<?=$proposalId?>&eid=<?=$employeeId?>&mn=<?=$studentMatrixNo?>&ref=<?=$referenceNo?>&refd=<?=$referenceNoDefence?>&rol=<?=$partnerSupervisorTypeId?>&rolx=<?=$supervisorTypeId?>"><?=$partnerEvaluationDesc?></a><label></label></td>
				<?
...
<fieldset>
<legend>
<h3><strong>Work Achievement</strong>asdasd</h3>
</legend>
	<?
	$chapterArray = Array();
	$no=0;
	...
	<table>
		<tr>
			<td><input type="button" name="btnBack" value="Back" onClick="javascript:document.location.href='../work/work_completion_evaluate.php';" /></td>
			<td> <input type="button" name="btnView" id="btnView" value="Print PDF"onclick="javascript:open_win('pdf_work_completion_report.php?id=<?=$id?>&mn=<?=$studentMatrixNo?>&tid=<?=$thesisId?>&pid=<?=$proposalId?>&ref=<?=$referenceNo?>&refd=<?=$referenceNoDefence?>&rol=<?=$supervisorTypeId?>&cid=<?=$calendarId?> &did=<?=$defenseId?>',800,700,0,0,0,1,0,1,1,0,5,'winupload'); "/></td>
			
					<!--	<td><input type="button" name="btnPrint" value="Print me PDF" onClick="javascript:document.location.href='../work/pdf_work_completion_report.php?id=<?=$id?>&mn=<?=$studentMatrixNo?>&tid=<?=$thesisId?>&pid=<?=$proposalId?>&ref=<?=$referenceNo?>&refd=<?=$referenceNoDefence?>&rol=<?=$supervisorTypeId?>&cid=<?=$calendarId?> &did=<?=$defenseId?>';" /></td>-->
	<!--<td><input type="button" name="btnPrint" value="Print Amendments PDF" onClick="javascript:document.location.href='../work/pdf_list_amendments_work_completion.php?id=<?=$id?>&mn=<?=$studentMatrixNo?>&tid=<?=$thesisId?>&pid=<?=$proposalId?>&ref=<?=$referenceNo?>&refd=<?=$referenceNoDefence?>&rol=<?=$supervisorTypeId?>&cid=<?=$calendarId?> &did=<?=$defenseId?>';" /></td>
	-->
		</tr>
	</table>
