...
a.comment, DATE_FORMAT(a.comment_date,'%d-%b-%Y %h:%i%p') as comment_date,
...
do {
	...
	$amendmentDatekArray[$i] = $dba->f('amendment_date');
	  ...
	  <table>
       ...
       <table border="1" style="border-collapse:collapse;" cellpadding="2" cellspacing="1" width="100%" class="thetable">
        <tr>
		  ...
		  <th width="15%" align="left">Comment by Supervisor</th>
		  <th width="20%" align="left">Verified By</th>
		...
		if ($row_cnt1 > 0) {		
			for ($j=0; $j<$row_cnt1; $j++){	?>
				...
				<tr class="<?=$color?>">					
					...
					<td>
					<?if ($amendmentDatekArray[$j]=="") {?>
						<span style="color:#FF0000;">Last Update:</span>
					<?}
					else {
						?>
						Last Update: 
					<?}?>
					<?=$amendmentDatekArray[$j]?><br>Amendment (Before Change):<br><em><?=$amendmentBeforeChgArray[$j]?></em><br><br>Amendment (After Change):<br><em><?=$amendmentAfterChgArray[$j]?></em></td>
					<td>Last Update: <?=$commentDateArray[$j]?><br><?=$commentArray[$j]?></td>
					<td><?=$verifiedRemarkArray[$j]?></td>
					<td><a href="../work/work_amendment_update.php?wadid=<?=$workAmendmentDetailIdArray[$j]?>&wfid=<?=$workFeedbackId?>&eid=<?=$panelEmployeeId?>&seid=<?=$employeeId?>&waid=<?=$workAmendmentId?>&tid=<?=$thesisId?>&ref=<?=$referenceNo?>&pid=<?=$proposalId?>&cid=<?=$calendarId?>&wmid=<?=$workMarksId?>&wes=<?=$workEvaluationStatus?>&pmid=<?=$proposedMarksId?>">Update</a></td>
				</tr>		
			<? }			
		}		
		else {
