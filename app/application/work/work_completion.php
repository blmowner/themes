...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<title>Submit Work Completion</title>
...
// Program Name: work_completion.php
//
// Created by: Zuraimi
...
if(isset($_POST['btnDeleteReport']) && ($_POST['btnDeleteReport'] <> ""))
{
	...
	while (list ($key,$val) = @each ($tmp_report_checkbox)) 
	{
		if ($defenseStatus[$val] != 'SAV') {
			$msg[] = "<div class=\"error\"><span>The Work Completion $referenceNo[$val] is already submitted. It cannot be deleted from the list.</span></div>";
		}
		else {
			$sql1 = "SELECT id 
			FROM pg_work
			...
			if ($row_cnt_sql1 > 1) {
				$msg[] = "<div class=\"error\"><span>The Work Completion $referenceNo[$val] has the detail history already. It cannot be deleted from the list.</span></div>";
			}
	...
	if(empty($msg)) 
	{
		...
		{
			$sql1 = "DELETE FROM pg_work
			WHERE id = '$defenseId[$val]'";
			
			$db->query($sql1);
			$db->next_record();
			
			$sql2 = "DELETE FROM pg_work_detail
			WHERE pg_work_id = '$defenseId[$val]'";
...
f.ref_work_marks_id, g.description as defense_marks_desc, f.reference_no as reference_no_eval,
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	{
		document.location.href = "new_work.php?tid=" + tid + "&pid=" + pid;
		return true;
	}
	</script>
	<script type="text/javascript">
	...
	{
		var ask = window.confirm("Are you sure to delete this Work Completion? All proposal with the same Reference No. will be deleted. \nClick OK to proceed or CANCEL to stay on the same page.");
		if (ask) 
		{
			return true;
		}
	...
	<?if ($row_cnt1 > 0) {
		if ($row_cnt_supervisor > 0 ) {
				...
				if ($row_cnt2_1 > 0) {				
					?>
					<fieldset>
						<legend><strong>List of Work Completion</strong></legend>
						<table>
							<tr>
							...
							<? 	} while($db_klas2->next_record());
								...
								<table>				
									<tr><td>Notes: <br/>No Supervisor/Co-Supervisor in the list.
												...
												<span style="color:#FF0000"> Make sure your Supervisor has been assigned first before submit the Work Completion.</span></td>
									</tr>
								</table>
								<?
							}?>	
						...
						<table>
							<tr>
								<td><h3><strong>Work Completion Status</strong></h3>Searching Results - <?=$row_cnt2?> record(s) found.</td>
							</tr>						
						</table>
						<?if ($row_cnt2 <= 1) {
							?>
								...
								if ($row_cnt2 > 0) {
									...
									<tr>
										...
										if ($defenseStatus =='SAV') {
											?>
											<td align="center"><a href="../work/edit_work.php?pid=<?=$proposalId;?>&tid=<?=$thesisId;?>&ref=<?=$referenceNo;?>">Edit</a></td>
										<?}
										...
										<?}
											...
											?>
											<td align="center"><a href="../work/view_work.php?id=<?=$id;?>&pid=<?=$proposalId;?>&tid=<?=$thesisId;?>&ref=<?=$referenceNo;?>&cid=<?=$calendarId?>">View</a></td>
											<?
										}?>
									</tr>
									<?
						...
						<table>
							<tr>
								...
								1. Work Completion can only be deleted if it is not yet submitted to the Supervisor/Co-Supervisor (Edit Mode).<br/>
								2. The deletion of the Work Completion will be applied to the report with the same Reference No.</label></td>
							</tr>
						</table>
						<table>
							<tr>
								<td><input type="button" name="btnNewReport" value="New Work Completion" onclick="return newReport('<?=$thesisId?>','<?=$proposalId?>')"></td>
								<td><input type="submit" name="btnDeleteReport" value="Delete Work Completion" onclick="return deleteReport()"></td>
							</tr>
						</table>					
					</fieldset>
					<?				
					...
					$err = "<div class=\"error\"><span>Please define thesis chapter first before submit Work Completion.</span></div>";
					echo $err;	
				}
		}
		else {
			$err = "<div class=\"error\"><span>Submission of Work Completion is currently not allowed.</span></div>";
			echo $err;
			?>
			<table>
				<tr>
				...
				<tr>
					...
					2. Your Supervisor/Co-Supervisor has not been assigned yet or pending their invitation acceptance.<br>
					3. Your Defence Proposal Report is pending approval by Evaluation Committee.</label></td>
				</tr>
			</table>
			<?						
		}
			...
			$err = "<div class=\"error\"><span>You are not able to submit your <strong>Work Completion</strong>. Ensure your thesis proposal has been submitted and approved first by the Senate.</span></div>";
			echo $err;	
	}	?>
	<script>
		<?=$jscript;?>	
