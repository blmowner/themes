...
// Program Name: work_review_amendment.php
//
// Created by: Zuraimi
// Created Date: 17-September-2015
// Modified by: Zuraimi
...
?>
	...
	<table>
		...
		<tr>
			<td><strong>Note:</strong> <em>by default the list will contain the feedback from each Evaluation Panel for Supervisor review.</em></td>
	</table>
	<br>
	<table>
		<tr>
		...
		if ($row_cnt1 > 0) {		
			for ($j=0; $j<$row_cnt1; $j++){	
				...
				<tr class="<?=$color?>">
					...
					<td><?=$studentName?><br><?=$studentMatrixNoArray[$j]?></td>
					...
					<?if ($feedbackStatusArray[$j]=="FSU") {?>
						<td><?=$feedbackStatusDescArray[$j]?><br><?=$feedbackDateArray[$j]?><a href="../work/work_review_feedback_detail.php?waid=<?=$workAmendmentIdArray[$j]?>&tid=<?=$thesisIdArray[$j]?>&pid=<?=$proposalIdArray[$j]?>&ref=<?=$referenceNoArray[$j]?>&cid=<?=$calendarIdArray[$j]?>&wmid=<?=$workMarksIdArray[$j]?>&wes=<?=$workEvaluationStatusArray[$j]?>&pmid=<?=$proposedMarksIdArray[$j]?>&mn=<?=$studentMatrixNoArray[$j]?>" title="<?=$remarksArray[$j]?>"><br>Update</a></td>
					<?}
					else if ($feedbackStatusArray[$j]=="FVE"){
						?>
						<td><?=$feedbackStatusDescArray[$j]?><br><?=$feedbackDateArray[$j]?><a href="../work/work_review_feedback_detail.php?waid=<?=$workAmendmentIdArray[$j]?>&tid=<?=$thesisIdArray[$j]?>&pid=<?=$proposalIdArray[$j]?>&ref=<?=$referenceNoArray[$j]?>&cid=<?=$calendarIdArray[$j]?>&wmid=<?=$workMarksIdArray[$j]?>&wes=<?=$workEvaluationStatusArray[$j]?>&pmid=<?=$proposedMarksIdArray[$j]?>&mn=<?=$studentMatrixNoArray[$j]?>" title="<?=$remarksArray[$j]?>"><br>View</a></td>
						<?
					}
					else if ($feedbackStatusArray[$j]=="FRQ"){
						?>
						<td><?=$feedbackStatusDescArray[$j]?><br><?=$feedbackDateArray[$j]?><a href="../work/work_review_feedback_detail.php?waid=<?=$workAmendmentIdArray[$j]?>&tid=<?=$thesisIdArray[$j]?>&pid=<?=$proposalIdArray[$j]?>&ref=<?=$referenceNoArray[$j]?>&cid=<?=$calendarIdArray[$j]?>&wmid=<?=$workMarksIdArray[$j]?>&wes=<?=$workEvaluationStatusArray[$j]?>&pmid=<?=$proposedMarksIdArray[$j]?>&mn=<?=$studentMatrixNoArray[$j]?>" title="<?=$remarksArray[$j]?>"><br>View</a></td>
						<?
					}
					...
					<?if (($workAmendmentStatusArray[$j]=="") || ($workAmendmentStatusArray[$j]=="PND")){
						?>
						<td><?=$workAmendmentDescArray[$j]?></td>
						<?
					}
					else if ($workAmendmentStatusArray[$j]=="ASU"){
						?>
						<td><?=$workAmendmentDescArray[$j]?><br><?=$workAmendmentDateArray[$j]?><a href="../work/work_review_amendment_detail.php?waid=<?=$workAmendmentIdArray[$j]?>&tid=<?=$thesisIdArray[$j]?>&pid=<?=$proposalIdArray[$j]?>&ref=<?=$referenceNoArray[$j]?>&cid=<?=$calendarIdArray[$j]?>&wmid=<?=$workMarksIdArray[$j]?>&wes=<?=$workEvaluationStatusArray[$j]?>&pmid=<?=$proposedMarksIdArray[$j]?>&mn=<?=$studentMatrixNoArray[$j]?>"><br>Update</a></td>
					<?}
					else if (($workAmendmentStatusArray[$j]=="ACO") || ($workAmendmentStatusArray[$j]=="AVE") || ($workAmendmentStatusArray[$j]=="ARQ")){
						?>
						<td><?=$workAmendmentDescArray[$j]?><br><?=$workAmendmentDateArray[$j]?><a href="../work/work_review_amendment_detail.php?waid=<?=$workAmendmentIdArray[$j]?>&tid=<?=$thesisIdArray[$j]?>&pid=<?=$proposalIdArray[$j]?>&ref=<?=$referenceNoArray[$j]?>&cid=<?=$calendarIdArray[$j]?>&wmid=<?=$workMarksIdArray[$j]?>&wes=<?=$workEvaluationStatusArray[$j]?>&pmid=<?=$proposedMarksIdArray[$j]?>&mn=<?=$studentMatrixNoArray[$j]?>"><br>View</a></td>
					<?}?>
				</tr>		
			<? }			
		}		
