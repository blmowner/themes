...
// Program Name: work_review_feedback_change.php
//
// Created by: Zuraimi
...
?>
	...
	<br>
      ...
      <table border="1" style="border-collapse:collapse;" cellpadding="2" cellspacing="1" width="85%" class="thetable">
        <tr>
		  <th width="5%">No</th>
		  <th width="10%" align="left">Affected Page No</th>
		...
		if ($row_cnt3 > 0) {		
			for ($j=0; $j<$row_cnt3; $j++){	?>
				...
				<tr class="<?=$color?>">					
					...
					<td><em><?=$submitDateArray[$j]?></em><br><?=$panelFeedbackArray[$j]?></td>
					<td><em><?=$commentDateArray[$j]?></em><br><?=$commentArray[$j]?></td>
					<td align="left"><a href="../work/work_review_feedback_update.php?wfdid=<?=$workFeedbackDetailIdArray[$j]?>&wfid=<?=$workFeedbackId?>&eid=<?=$panelEmployeeId?>&waid=<?=$workAmendmentId?>&tid=<?=$thesisId?>&ref=<?=$referenceNo?>&pid=<?=$proposalId?>&cid=<?=$calendarId?>&wmid=<?=$workMarksId?>&wes=<?=$workEvaluationStatus?>&pmid=<?=$proposedMarksId?>&mn=<?=$studentMatrixNo?>">Update</a></td>									
				</tr>		
			<? }			
		}		
		else {
	...
	<table>				
		<tr>
			<td>Note:<br>
			If no comment to the feedback, it will be considered as verified and accepted by the Supervisor.
			</td>
		</tr>
	</table>
	<table>
		<tr>		
			<td><input type="button" name="btnBack" value="Back" onclick="javascript:document.location.href='../work/work_review_feedback_detail.php?waid=<?=$workAmendmentId?>&tid=<?=$thesisId?>&ref=<?=$referenceNo?>&pid=<?=$proposalId?>&cid=<?=$calendarId?>&wmid=<?=$workMarksId?>&wes=<?=$workEvaluationStatus?>&pmid=<?=$proposedMarksId?>&mn=<?=$studentMatrixNo?>'" /></td>			
		</tr>
	</table>
    </form>
</div>
