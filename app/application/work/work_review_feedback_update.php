...
// Program Name: work_review_feedback_update.php
//
// Created by: Zuraimi
...
if(isset($_POST['btnUpdate']) && ($_POST['btnUpdate'] <> ""))
{
	...
	if(empty($msg))  
	{
		...
		if ($status == 'A') {
			...
			submit_date, status, insert_by, insert_date, modify_by, modify_date)
			VALUE ('$newWorkFeedbackDetailId', '$workFeedbackId', '$panelFeedback', '$pageAffected', '$curdatetime', '$comment',
...
$sql3 = "SELECT page_affected, panel_feedback, 
...
?>
	...
	<table>
		<tr>
			...
			<td><input type="button" name="btnBack" value="Back" onclick="javascript:document.location.href='../work/work_review_feedback_change.php?waid=<?=$workAmendmentId?>&wfid=<?=$workFeedbackId?>&eid=<?=$panelEmployeeId?>&tid=<?=$thesisId?>&ref=<?=$referenceNo?>&pid=<?=$proposalId?>&cid=<?=$calendarId?>&wmid=<?=$workMarksId?>&wes=<?=$workEvaluationStatus?>&pmid=<?=$proposedMarksId?>&mn=<?=$studentMatrixNo?>'" /></td>
		</tr>		
	</table>
    </form>
</div>
