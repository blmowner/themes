...
// Program Name: work_verify_amendment.php
//
// Created by: Zuraimi
...
if(isset($_POST['btnSearch']) && ($_POST['btnSearch'] <> "")) {
	...
	echo $sql1 = "SELECT a.pg_thesis_id, a.pg_proposal_id, a.pg_calendar_id, a.status as work_status, a.reference_no, 
	a.student_matrix_no,
	...
	AND e.status = 'A'
	AND f.feedback_status = 'FVE'
	AND f.status IN ('ACO', 'AVE')
...
?>
	...
	<br>
	  ...
	  <table border="1" style="border-collapse:collapse;" cellpadding="2" cellspacing="1" width="100%" class="thetable">
			...
			for ($j=0; $j<$row_cnt1; $j++){	
				...
				<tr class="<?=$color?>">
					...
					<?if ($feedbackStatusArray[$j]=="FVE"){
						?>
						<td><?=$feedbackStatusDescArray[$j]?><br><?=$feedbackDateArray[$j]?><a href="../work/work_verify_feedback_detail.php?waid=<?=$workAmendmentIdArray[$j]?>&tid=<?=$thesisIdArray[$j]?>&pid=<?=$proposalIdArray[$j]?>&ref=<?=$referenceNoArray[$j]?>&cid=<?=$calendarIdArray[$j]?>&wmid=<?=$workMarksIdArray[$j]?>&wes=<?=$workEvaluationStatusArray[$j]?>&pmid=<?=$proposedMarksIdArray[$j]?>&mn=<?=$studentMatrixNoArray[$j]?>" title="<?=$remarksArray[$j]?>"><br>View</a><br></td>
						<?
					}?>
						
					<?if ($workAmendmentStatusArray[$j]=="ACO"){
						?>
						<td><?=$workAmendmentDescArray[$j]?><br><?=$workAmendmentDateArray[$j]?><a href="../work/work_verify_amendment_detail.php?waid=<?=$workAmendmentIdArray[$j]?>&tid=<?=$thesisIdArray[$j]?>&pid=<?=$proposalIdArray[$j]?>&ref=<?=$referenceNoArray[$j]?>&cid=<?=$calendarIdArray[$j]?>&wmid=<?=$workMarksIdArray[$j]?>&wes=<?=$workEvaluationStatusArray[$j]?>&pmid=<?=$proposedMarksIdArray[$j]?>&mn=<?=$studentMatrixNoArray[$j]?>"><br>Update</a></td>
					<?}
					else if ($workAmendmentStatusArray[$j]=="AVE"){?>
		...
		<table>				
			<tr>
				<td><label><strong><br>Note:</strong><label><br>
				Please verify this amendment to enable the student to resubmit the Work Completion.
				</td>
			</tr>
		</table>	
