...
// Program Name: work_verify_amendment_change.php
//
// Created by: Zuraimi
	  ...
	  <table>
       ...
       <table border="1" style="border-collapse:collapse;" cellpadding="2" cellspacing="1" width="100%" class="thetable">
        <tr>
		  ...
		  <th width="20%" align="left">Remark by Faculty</th>
		...
		if ($row_cnt1 > 0) {		
			for ($j=0; $j<$row_cnt1; $j++){	?>
				...
				<tr class="<?=$color?>">					
					...
					<td><a href="../work/work_verify_amendment_update.php?wadid=<?=$workAmendmentDetailIdArray[$j]?>&wfid=<?=$workFeedbackId?>&eid=<?=$panelEmployeeId?>&waid=<?=$workAmendmentId?>&tid=<?=$thesisId?>&ref=<?=$referenceNo?>&pid=<?=$proposalId?>&cid=<?=$calendarId?>&wmid=<?=$workMarksId?>&wes=<?=$workEvaluationStatus?>&pmid=<?=$proposedMarksId?>&mn=<?=$studentMatrixNo?>">Update</a></td>
				</tr>		
			<? }			
		}		
		else {
