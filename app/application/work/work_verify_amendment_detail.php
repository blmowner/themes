...
// Program Name: work_verify_amendment_detail.php
//
// Created by: Zuraimi
...
if(isset($_POST['btnVerify']) && ($_POST['btnVerify'] <> "")) {
	...
	{			
		...
		'$theCalendarId', '$theFeedbackDate', '$theFeedbackStatus', '$curdatetime', 'AVE', '$theInsertBy', '$theInsertDate', '$user_id', '$curdatetime')";
		...
		for ($i=0;$i<count($workFeedbackId);$i++) {
			...
			if ($row_cnt12 > 0) {
				...
				'$curdatetime', 'AVE', '$theInsertBy', '$theInsertDate', '$theModifyBy', '$theModifyDate')";
				$dba->query($sql9);
				...
				if ($row_cnt17 > 0) {
					...
					for ($l=0;$l<count($theWorkFeedbackDetailIdArray);$l++) {
						...
						'AVE', '$user_id', '$curdatetime', '$user_id', '$curdatetime')";
						$dba->query($sql20);
					...
					SET status = 'AVE', verified_by = '$user_id', verified_date = '$curdatetime', verified_status = 'Y',
				...
				VALUES ('$newWorkFeedbackId', '$newWorkAmendmentId', '$thePanelEmployeeId', '$theVerifiedDate', '$theFeedbackStatus',
				'$curdatetime', 'AVE', '$theInsertBy', '$theInsertDate', '$user_id', '$curdatetime')";
				$dba->query($sql9);
					...
					for ($l=0;$l<$row_cnt17;$l++) {
						...
						for ($b=0;$b<$a;$b++) {
							if ($statusArray[$b]  == 'ACO') {
								...
								'$user_id', '$curdatetime', 'Y', '$verifiedRemarkArray[$b]', 
								'AVE', '$user_id', '$curdatetime', '$user_id', '$curdatetime')";
								$dba->query($sql20);
								...
								SET status = 'AVE', work_amendment_id = '$newWorkAmendmentId', work_feedback_detail_id = '$newWorkFeedbackDetailId',
								modify_by = '$user_id', modify_date = '$curdatetime'
								WHERE id = '$workAmendmentDetailIdArray[$b]'
								AND status = 'NEW'
								AND archived_status IS NULL";
		...
		$msg[] = "<div class=\"success\"><span>The list of feedback for each Evaluation Panel has been submitted back to the Student successfully.</span></div>";
	}
}
...
?>
		...
		if ($row_cnt1 > 0) {		
			for ($j=0; $j<$row_cnt1; $j++){	
				...
				<tr class="<?=$color?>">
					...
					<?if ($statusArray[$j]=="ACO"){
						?>
						<td align="left"><?=$statusDescArray[$j]?><br><?=$amendmentDateArray[$j]?><a href="../work/work_verify_amendment_change.php?wfid=<?=$workFeedbackIdArray[$j]?>&eid=<?=$panelEmployeeIdArray[$j]?>&seid=<?=$employeeId?>&waid=<?=$workAmendmentId?>&tid=<?=$thesisId?>&ref=<?=$referenceNo?>&pid=<?=$proposalId?>&cid=<?=$calendarId?>&wmid=<?=$workMarksId?>&wes=<?=$workEvaluationStatus?>&pmid=<?=$proposedMarksId?>&mn=<?=$studentMatrixNo?>"><br>Update</a></td>
					<?}
					else if ($statusArray[$j]=="AVE"){
						?>
						<td align="left"><?=$statusDescArray[$j]?><br><?=$amendmentDateArray[$j]?><a href="../work/work_verify_amendment_view.php?wfid=<?=$workFeedbackIdArray[$j]?>&eid=<?=$panelEmployeeIdArray[$j]?>&seid=<?=$employeeId?>&waid=<?=$workAmendmentId?>&tid=<?=$thesisId?>&ref=<?=$referenceNo?>&pid=<?=$proposalId?>&cid=<?=$calendarId?>&wmid=<?=$workMarksId?>&wes=<?=$workEvaluationStatus?>&pmid=<?=$proposedMarksId?>&mn=<?=$studentMatrixNo?>"><br>View</a></td>
					<?}?>			
				</tr>		
			<? }			
		}		
	...
	<table>				
		<tr>
			...
			Once you are done with the verification, click Verification Done button to enable the student to resubmit the Work Completion.
			</td>
		</tr>
	</table>	
	<table>
