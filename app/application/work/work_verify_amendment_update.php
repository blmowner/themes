...
// Program Name: work_verify_amendment_update.php
//
// Created by: Zuraimi
...
if(isset($_POST['btnUpdate']) && ($_POST['btnUpdate'] <> ""))
{
	...
	if ($_POST['remark'] == "") $msg[] = "<div class=\"error\"><span>Please enter the remark. It cannot be empty.</span></div>";
	
	if(empty($msg))  
	{
		$sql1 = "SELECT id, work_amendment_id, work_feedback_detail_id, page_after_chg, amendment_before_chg, 
		amendment_after_chg, amendment_date, comment_date, comment, confirmed_date, confirmed_status, 
		...
		if ($status == 'ACO') {
			...
			'$amendmentDate', '$commentDate', '$comment', '$confirmedDate', '$confirmedStatus', '$user_id', '$curdatetime', 'Y',
			'$remark', 'NEW', '$insertBy', '$insertDate', '$modifyBy', '$modifyDate')";
			$dba->query($sql4);
			...
			SET verified_remark = '$remark', verified_date = '$curdatetime', verified_by = '$user_id',
...
?>
	...
	<table>
		<tr>
			<td><strong>Remark by Faculty</strong></td>
		</tr>
	</table>
	<table>
		<tr>
