...
// Program Name: work_verify_amendment_view.php
//
// Created by: Zuraimi
...
?>
		...
		if ($row_cnt1 > 0) {		
			for ($j=0; $j<$row_cnt1; $j++){	?>
				...
				<tr class="<?=$color?>">					
					...
					<td><em><?=$verifiedDateArray[$j]?></em><br><?=$verifiedRemarkArray[$j]?></td>
				</tr>		
			<? }			
		}		
		else {
	...
	<table>
		<tr>		
			<td><input type="button" name="btnBack" value="Back" onclick="javascript:document.location.href='../work/work_verify_amendment_detail.php?waid=<?=$workAmendmentId?>&tid=<?=$thesisId?>&ref=<?=$referenceNo?>&pid=<?=$proposalId?>&cid=<?=$calendarId?>&wmid=<?=$workMarksId?>&wes=<?=$workEvaluationStatus?>&pmid=<?=$proposedMarksId?>&mn=<?=$studentMatrixNo?>'" /></td>	
			
			<td> <input type="submit" name="submit" value="Print PDF" onclick="javascript:open_win('pdf_work_amendment_report_faculty.php?wfid=<?=$workFeedbackId?>&eid=<?=$panelEmployeeId?>&waid=<?=$workAmendmentId?>&tid=<?=$thesisId?>&ref=<?=$referenceNo?>&pid=<?=$proposalId?>&cid=<?=$calendarId?>&wmid=<?=$workMarksId?>&wes=<?=$workEvaluationStatus?>&pmid=<?=$proposedMarksId?>&mn=<?=$studentMatrixNo?>',1000,760,0,0,0,1,0,1,1,0,5,'winupload'); "/></td>
