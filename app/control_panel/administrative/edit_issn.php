...
{ 
    ...
    if($_POST['submit'] <> "") {
        ...
        if(empty($_POST['issn'])) $msg[] = "<div class=\"error\"><span>Please insert Name</span></div>";
        //if(empty($_POST['const_value'])) $msg[] = "<div class=\"error\"><span>Please insert constant value</span></div>";
        
        if(empty($msg)) 
		{
				$sql2 = "UPDATE pg_publication SET issn_no = '$issn', status = '$status', modify_by = '$user_id', modify_date= '$curdatetime'
				WHERE issn_no= '$id'";
				$db->query($sql2);
    ...
    <div class="padding-5 margin-5 outer">
		...
		<table>
			<tr>
			  ...
			  <td width="493"><input type="text" name="issn" size="40" value="<?=$issn_no?>" /></td>
			</tr>
			<tr>
				...
				{ ?> 
					...
					<option value="I" selected="selected">No</option>
				<? 
				} ?>
				</select>Status: Active or Not Active				</td>
			</tr>
		</table>
