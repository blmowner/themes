<?php
	...
	$publishStatus = $dbf->f('publish_status');
...
if($_POST['btnEdit'] <> ""){
	...
	{	
		...
		$sqlUpd = "UPDATE pg_announcement SET title = '$title', announcement = '".mysql_escape_string($announ)."', start_date = '$newStartDate', 
		end_date = '$newEndDate', publish_status = 'P',
		status= 'A', modify_by = '$user_id', modify_date = '$curdatetime'
		WHERE id='$id'";			
		...
		$lol = mysql_escape_string($sqlUpd);
		...
		$sqltracking = "INSERT INTO pg_announcement_tracking
				...
				VALUES
				('$trackId', 'E', '$user_id', '".$lol."' , '$id', 'A', '$curdatetime', 'D')";
...
if($_POST['btnUpdate'] <> ""){
	...
	{	
		...
		$sqlUpd1 = "INSERT INTO pg_announcement 
		(`id`,`title`,`announcement`,`insert_by`,`insert_date`, announcer, `status`, start_date, end_date, publish_status)
		VALUES
		('$Aid', '$title', '".mysql_escape_string($announ)."', '$user_id' , '$curdatetime', '$user_id', 'A', '$newStartDate', '$newEndDate', 'P')";			
		$process1 = $db->query($sqlUpd1);
...
if($_POST['btnsave'] <> "") 
{
	...
	{	
		...
		$sqltracking = "INSERT INTO pg_announcement_tracking
				...
				VALUES
				('$trackId', 'E', '$user_id', '".mysql_escape_string($sqlUpd)."' , '$Aid', 'A', '$curdatetime' , 'D', 'Edit by User<br>($user_id)')";
	...
	<table class="idd">
		...
		<tr>
		  <td colspan="4" align="right"><span style="color:#FF0000; font-family:Verdana;font-size:11px;">Note:</span> <span style="font-family:Verdana;font-size:11px;">Field marks with (</span><span style="color:#FF0000">*</span><span style="font-family:Verdana;font-size:11px;">) is compulsory.</span>
			  ...
			  <input type="submit" id="btnEdit" name="btnEdit" value = "Publish" />
			  <input type="submit" id="close" name="close" onClick="javascript: parent.$.fn.colorbox.close();" value = "Close" />
			  <? } else {?>
