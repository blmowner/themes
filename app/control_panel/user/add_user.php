<?php 
    ...
    if($_POST['submit'] <> "") {
        ...
        if(empty($msg)) {
            ...
            $checkUserExist = getValue_local('staff_id','user_acc','staff_id',$staff_id);
			...
			$sqlIns1 = "INSERT INTO user_acc (user_id, role_id, staff_id, user_pass, user_online_stat, user_status, user_type, lock_status, created_by, created_date) 
                       VALUES('".run_num('user_id','user_acc')."','$staff_role', '$staff_id', '".md5($staff_id)."','0','$staff_status',
					   '".$_POST['user_type']."', 'U', '".$_SESSION['user_id']."', now())";
            $result_sqlIns1 = $db->query($sqlIns1);		
                ...
                if($result_sqlIns1) {
					...
					VALUES ('$credentialId', '$staff_id', '".md5($staff_id)."', now())";
