<?php 
    ...
    if($_POST['submit'] <> "") {
        ...
        if(empty($msg)) {
            ...
            $checkUserExist = getValue('staff_id','user_acc','staff_id',$staff_id);
            if($staff_id == $checkUserExist) {
                $msg[] = "<div class=\"error\"><span>User already exist in a system!</span></div>";
            } else {
            //if user not in a system, then set them in!
            $sqlIns = "INSERT INTO user_acc (user_id, role_id, staff_id, user_pass, user_online_stat, user_status, created_by, created_date) 
                       VALUES('".run_num('user_id','user_acc')."','$staff_role', '$staff_id', '".md5($staff_id)."','0','$staff_status','".$_SESSION['user_id']."', now())";
