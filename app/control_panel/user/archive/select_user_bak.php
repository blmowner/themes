<?php 
    ...
    if(isset($_POST['search']) || isset($_POST['search_box'])) {
        $sql = "SELECT empid, name from siso_employee WHERE name LIKE '%".$_POST['search_box']."%' OR empid LIKE '%".$_POST['search_box']."%' LIMIT $startpoint, $perpage";

    } else {
        $sql = "SELECT empid, name from siso_employee LIMIT $startpoint, $perpage";
    }
...
<form method="post" id="form-set">
 ...
 <table cellpadding="3" cellspacing="3" width="100%" class="thetable">
    ...
    <tr class="<?php echo $color; ?>">
        <td align="center"><?php echo $dbs->f('empid'); ?></td>
        <?php
            $a = $dbs->f('empid');
        ?>
        <td><a href="#" onclick="closeThePop('<?php echo $a; ?>');"><?php echo $dbs->f('name'); ?></a></td>
    </tr>    
    <?php
            $inc++;
            $result = $dbs->next_record();
 ...
 <?php
    // count total number of result - without LIMIT
        ...
        $dbs->query($count_total_result);
        $a = $dbs->num_rows($count_total_result);
        //$dbs->free();
