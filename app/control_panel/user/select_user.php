...
foreach($_REQUEST as $key => $value)
{
    ...
    if(isset($_POST['search']) || isset($_POST['search_box'])) {
       $sql = "SELECT matrix_no empid, name, 'S' as user_type from student 
	   			WHERE (name LIKE '%".$_POST['search_box']."%' OR matrix_no LIKE '%".$_POST['search_box']."%') 
				AND student_status IN ('ACTIVE', 'LI', 'Project')
				UNION 
				SELECT empid, name, 'E' as user_type FROM new_employee
				WHERE (name LIKE'%".$_POST['search_box']."%' OR empid LIKE '%".$_POST['search_box']."%') 
				AND xemployee_status='AC' order by name LIMIT $startpoint, $perpage";
       ...
       $sql = "SELECT matrix_no empid, name, 'S' as user_type from student 
				...
				UNION
				SELECT empid, name, 'E' as user_type FROM new_employee
				WHERE xemployee_status='AC' order by name LIMIT $startpoint, $perpage";
				/*LIMIT $startpoint, $perpage*/
    }
...
<head>
    ...
    <script>
        function closeThePop(data1, data2) {
            parent.$.fn.colorbox.close(); //call the colorbox's close function
            parent.$.fn.getParameterValue(data1, data2); // pass the parameter to parent
        }
    </script>
</head>
...
<form method="post" id="form-set">
 ...
 <table cellpadding="3" cellspacing="3" width="100%" class="thetable">
    ...
    <tr class="<?php echo $color; ?>">
        ...
        <?php
            $a = $dbc->f('empid');
			$userType = $dbc->f('user_type');
        ?>
        <td><a href="#" onclick="closeThePop('<?php echo $a; ?>','<?php echo $userType; ?>');"><?php echo $dbc->f('name'); ?></a></td>
    </tr>    
    <?php
            $inc++;
            $result = $dbc->next_record();
 ...
 <?php
    // count total number of result - without LIMIT
        $count_total_result = "SELECT empid, name from new_employee WHERE name LIKE '%".$_POST['search_box']."%' OR empid LIKE '%".$_POST['search_box']."%' union SELECT matrix_no, name from student WHERE name LIKE '%".$_POST['search_box']."%' OR matrix_no LIKE '%".$_POST['search_box']."%'";
        ...
        doPages($perpage, 'select_user.php', $varParamSend, $a); 
 ?>
</body>
</html>